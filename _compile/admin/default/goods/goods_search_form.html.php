<?php /* Template_ 2.2.6 2022/03/07 12:32:25 /www/suns_firstmall_kr/admin/skin/default/goods/goods_search_form.html 000028602 */ 
...
<link rel="stylesheet" type="text/css" href="/admin/skin/default/css/goods_admin.css?v=<?php echo date('Ymd')?>" />
	...
	<tr data-fid='sc_provider' <?php if(!in_array('sc_provider',$TPL_VAR["sc_form"]["default_field"])){?>class='hide'<?php }?>>
		...
		<td>
			<select name="commission_type_sel">
				...
				<option value="SUPR" data-currency_symbol="<?php echo $TPL_VAR["basic_currency_info"]['currency_symbol']?>">공급가</option>
			</select>
		...
		<td colspan="3">
			<div class="sc_day_date date_range_form">
				<div style="display:inline-block !important">
					...
					<input type="text" name="sdate" value="<?php echo $TPL_VAR["sc"]["sdate"]?>" class="datepicker line sdate"  maxlength="10" default_none />
					-
					<input type="text" name="edate" value="<?php echo $TPL_VAR["sc"]["edate"]?>" class="datepicker line edate" maxlength="10" default_none   />
				</div>
...
<?php }?>
<?php if(serviceLimit('H_AD')){?>
	<tr data-fid='sc_confirm' <?php if(!in_array('sc_confirm',$TPL_VAR["sc_form"]["default_field"])){?>class='hide'<?php }?>>
		<th><span>승인</span></th>
		<td colspan='3'>
...
<?php if($TPL_VAR["openMarketSeach"]!="disable"){?>
	<tr data-fid='sc_openmarket' <?php if(!in_array('sc_openmarket',$TPL_VAR["sc_form"]["default_field"])){?>class='hide'<?php }?>>
...
<?php }?>
<?php if($TPL_VAR["catalog_page_gubun"]!="coupon"){?>
	<tr data-fid='sc_feed_status' <?php if(!in_array('sc_feed_status',$TPL_VAR["sc_form"]["default_field"])){?>class='hide'<?php }?>>
...
<?php if($TPL_I1&&$TPL_I1% 4== 0){?></tr><tr><?php }?>
		<td style="width:100px;height:30px;text-align:left;">
			<label class="resp_checkbox">
				...
				<img src="/data/icon/goods/<?php echo $TPL_V1["codecd"]?>.gif" border="0">
			</label>
		</td>
<?php }}?>
		</tr>
