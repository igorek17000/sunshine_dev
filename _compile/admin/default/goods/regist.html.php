<?php /* Template_ 2.2.6 2022/03/07 12:32:25 /www/suns_firstmall_kr/admin/skin/default/goods/regist.html 000130924 */ 
...
<script type="text/javascript" src="/app/javascript/plugin/editor/js/daum_editor_loader.js?dummy=<?php echo date('Ym')?>"></script>
...
<?php if($TPL_VAR["goods"]["goods_seq"]){?>
			<li class="relative">
				<button type="button" id="viewGoods" goods_seq="<?php echo $TPL_VAR["goods"]["goods_seq"]?>" class="resp_btn v2 size_L">상품보기<span class="arrowright"></span></button>
...
<?php }?>
		...
		<div class="page-buttons-left">
			<button type="button" class="resp_btn v3 size_L" onclick="document.location.href='../goods/<?php if($TPL_VAR["package_yn"]=='y'){?>package_<?php }?><?php if($TPL_VAR["socialcpuse"]){?>social_<?php }?>catalog?<?php echo $TPL_VAR["query_string"]?>';">리스트 바로가기</button>
<?php if($TPL_VAR["goods"]["goods_seq"]){?>
			<button type="button" id="manager_copy_btn"  class="resp_btn v2 size_L" goods_seq="<?php echo $TPL_VAR["goods"]["goods_seq"]?>">복사</button>
<?php }?>
		</div>
...
<?php if($TPL_VAR["provider_seq"]=='1'){?>
			<div class="ptc-title blue pdl0">본사 상품 (<?php echo $TPL_VAR["goods"]["goods_status_text"]?>, <?php echo $TPL_VAR["goods"]["provider_status_text"]?>)</div>
<?php }else{?>
			<div class="ptc-title red">
				<?php echo $TPL_VAR["provider"]["provider_id"]?>의 입점사 상품 (<?php echo $TPL_VAR["goods"]["goods_status_text"]?>, <?php echo $TPL_VAR["goods"]["provider_status_text"]?>)
...
<?php if($TPL_VAR["goods"]["provider_seq"]> 1){?>입점사<?php }else{?>본사<?php }?>
<?php }else{?>
					<div class='resp_radio'>
						<label class="mr20"><input type="radio" name="goods_gubun" value="admin" checked> 본사상품</label>
						<label><input type="radio" name="goods_gubun" value="provider"> 입점사 상품</label>
					</div>
<?php }?>
				</td>
			</tr>
			<tr class='tr_provider hide'>
				<th>입점사 <?php if(!$TPL_VAR["goods"]["goods_seq"]){?><span class="required_chk"></span><?php }?></th>
				<td>
...
<?php if($TPL_V1["provider_status"]=='N'){?>
						<option value="<?php echo $TPL_V1["provider_seq"]?>" disabled><?php echo $TPL_V1["provider_name"]?>(<?php echo $TPL_V1["provider_id"]?>) [종료]</option>
<?php }else{?>
...
<?php if(!$TPL_VAR["goods"]["provider_seq"]){?>
			<ul class='bullet_hyphen resp_message'>
				<li>선택한 본사 상품, 입점사 상품 구분 변경 시 입력한 상품 정보가 초기화 됩니다.</li>
			</ul>
<?php }?>
		</div>
	</div>
...
<?php }?>
						...
						<ul class='bullet_hyphen resp_message'>
							<li>자주 쓰는 추가정보를 
								<span class="underline blue hand" onclick="window.open('../setting/goods');">상품 코드</span>에서 등록할 수 있습니다.</li>
...
<?php }?>
						<!--<span class="btn small orange"><button type="button" class="goods_modify_helper" >안내) 상품 수정 시 자동 미승인 처리 기준</button></span>-->
					</td>
				</tr>
<?php }?>
				<tr>
...
<?php }?>
				...
				<tr>
					...
					<td colspan="3">
						...
						<div class="resp_radio">
							<label class="mr10"><input type="radio" name="tmp_goodsView" value="look" checked="checked"> 노출</label>
	...
	<div class="bx-lay" data-bxcode="options">
		...
		<div class="cont">
			<!--
				1. 상품 수정 페이지 open
					= 필수옵션 노출 영역
					#OPTION_HTML		: _option_for_regist.html			: (단일/멀티)
					* _resgist_popup_guide.html > #optionDefaultForm 	: (단일옵션) 기본 레이아웃 - 옵션 사용여부 사용함 했다가 사용 안함 클릭 시 불러옴.
					set_goods_options.html - #ONLY_VIEW(view_goods_options.html)	: (멀티) - 팝업에서 옵션 생성 후 확인(적용) 시 호출
				2. 필수옵션(멀티) 등록/수정 팝업 - set_goods_options.html
					- #EDIT_VIEW		: edit_goods_options.html		: 등록/수정 스킨
					- #CREATE_OPTION	: create_goods_options.html		: 필수옵션 신규 생성
				패키지 상품 연결
...
<?php if(!$TPL_VAR["socialcpuse"]){?>
	<div class="bx-lay" data-bxcode="suboptions">
		...
		<div class="cont">
			<!--
				SUBOPTION_HTML	: _suboption_for_regist.html		: 추가구성옵션 - 상품 수정 페이지 Open 시
				* _resgist_popup_guide.html > #optionDefaultForm 	: 필수옵션(단일) 기본 레이아웃 - 옵션 사용여부 사용함 했다가 사용안함 클릭 시 불러옴.
...
<?php if($TPL_VAR["goods"]["goods_seq"]){?><!-- 워터마크 :: START -->
...
<?php if(serviceLimit('H_FR')){?>
							...
							<span class="<?php echo serviceLimit('C1')?>"><button type="button" class="resp_btn v2" onclick="<?php echo serviceLimit('A1')?>">워터마크 적용하기</button></span>
							<span class="<?php echo serviceLimit('C1')?>"><button type="button" class="resp_btn v2" onclick="<?php echo serviceLimit('A1')?>">워터마크 제거하기</button></span>
<?php }else{?>
							<button type="button" class="resp_btn v2 waterMarkImageSetting">워터마크 설정</button>
							<button type="button" class="resp_btn v2 waterMarkImageApply">워터마크 적용하기</button>
							<button type="button" class="resp_btn waterMarkImageCancel">워터마크 제거하기</button>
<?php }?>
						</div>
<?php }?>
						<!-- 워터마크 :: END -->
...
<?php if($TPL_VAR["config_system"]["operation_type"]!='light'){?>
			<tr>
				...
				<td>
					...
					<ul class="bullet_hyphen resp_message">
						<li>
							외부마켓(11번가, G마켓, 옥션 등)에 판매 시 이미지 호스팅 서비스 이용을 권장 드립니다.
							<button type="button" class="resp_btn v2" id="openmarketimghostingftp">이미지 호스팅 설정</button>
						</li>
<?php if(serviceLimit('H_FR')){?>
						<li>
...
<?php }else{?>
	...
	<div class="bx-lay" data-bxcode="shipping">
		...
		<div class="cont">
			...
			<ul class="bullet_hyphen resp_message">
				<li>배송비는 <span class="hand blue underline" onClick="window.open('../setting/shipping_group')" target="_blank">배송비</span> 에서 설정할 수 있습니다.</li>
			</ul>
		</div>
	</div>
<?php }?>
	...
	<div class="bx-lay" data-bxcode="events">
		...
		<div class="cont">
			...
			<tr>
				...
				<td>
					<button type="button" class="resp_btn v2" id="gradeDiscountSet">선택</button>
			...
			<tr>
				...
				<td>
					<div class="resp_radio">
						...
						<label><input type="radio" name="multiDiscountSet" class="ml20" value="n" <?php if($TPL_VAR["goods"]["multi_discount_policy_count"]== 0){?>checked<?php }?>> 사용 안 함</label>
					</div>
...
<?php }?>
	...
	<div class="bx-lay" data-bxcode="etc_info">
		...
		<div class="cont">
			<table class="table_basic thl">
			<tr>
				<th class="left">결제수단 설정</th>
				<td>
					<div class="resp_radio">
						<label class="mr20"><input type="radio" name="possible_pay_type" value="shop" <?php if($TPL_VAR["goods"]["possible_pay_type"]!='goods'){?>checked<?php }?>> 통합 설정</label>
						<label><input type="radio" name="possible_pay_type" value="goods" <?php if($TPL_VAR["goods"]["possible_pay_type"]=='goods'){?>checked<?php }?>> 개별 설정</label>
<?php if(serviceLimit('H_NFR')){?>
						<button type="button" class="resp_btn v2" id="possible_pay_setting">설정</button>
<?php }?>
						<span id="possible_pay_td"><?php if($TPL_VAR["goods"]["possible_pay_type"]=="goods"){?>(<?php echo $TPL_VAR["possible_pay_str"]?>)<?php }?></span>
					</div>
					<ul class="bullet_hyphen resp_message">
						<li>결제 수단 통합 설정은 <a href="/admin/setting/pg" target="_blank"><span class="underline blue">전자결제</span></a>
							또는 <a href="/admin/setting/bank" target="_blank"><span class="underline blue">무통장</span></a>에서 가능합니다</li>
					</ul>
				</td>
			</tr>
			<tr>
...
<?php if($TPL_r_hscode_1){foreach($TPL_VAR["r_hscode"] as $TPL_V1){?><option value="<?php echo $TPL_V1["hscode_common"]?>"><?php echo $TPL_V1["hscode_name"]?>(<?php echo $TPL_V1["hscode_common"]?>)</option><?php }}?>
						...
						<ul class="bullet_hyphen resp_message fl" style="margin-top:3px;">
							<li>HS CODE 관리 <a href="../goods/hscode_setting" target="_blank"><span class="underline blue">바로 가기 &gt;</span></a></li>
						</ul>
					</div>
					<div class="cboth"></div>
...
<?php }}?>
							...
							<thead>
								<tr>
									...
									<th class="rate" colspan="<?php echo count($TPL_VAR["hscode"]["hscode_items"][ 0]["export_nation_key"])?>">수출국가별 수입국가 세율</th>
								</tr>
								<tr>
...
<?php if(serviceLimit('H_NFR')){?>
			<tr class="bigdataGoodsContainer">
				<th class="left">추천 상품 <span class="tooltip_btn" onClick="showTooltip(this, '/admin/tooltip/goods', '#regist_bigdata', 'sizeS')"></span></th>
				<td>
					<span class=""><button type="button" class="resp_btn active bigdataCriteriaButton displayCriteriaType " dp_id='displayCriteriaBigdata' kind='bigdata' auto_type='bigdata'>조건 설정</button></span>
					<!-- ## 빅데이터 -->
					<div class="wp70">
						...
						<!-- //추천상품 -->
						<ul class="bullet_hyphen resp_message">
							<li>추천 상품 <a href="https://www.firstmall.kr/customer/faq/1333" target="_blank"><span class="underline blue">예시 보기 &gt;</span></a></li>
						</ul>
					</div>
				</td>
			</tr>
<?php }?>
			<tr class="relationGoodsContainer">
				<th class="left">관련상품 <span class="tooltip_btn" onClick="showTooltip(this, '/admin/tooltip/goods', '#regist_relation', 'sizeS')"></span></th>
				<td>
					<div>
						<div class="resp_radio">
							<label><input type="radio" name="relation_type" class="relation_type" data-type="relation" value="AUTO" checked="checked" /> 자동</label>
							<label><input type="radio" name="relation_type" class="relation_type" data-type="relation" value="MANUAL" /> 직접 선정</label>
						</div>
						<!-- 관련상품 자동 -->
						<div id="relationGoodsAutoContainer" class="mt5 pdt5" style="border-top:1px dashed #ddd">
							...
							<button type="button" class="relationCriteriaButton displayCriteriaType resp_btn active" dp_id='relationCriteria' use_id='auto_condition_use' kind='relation' auto_type="auto" goods_seq="<?php echo $TPL_VAR["goods"]["goods_seq"]?>">조건 설정</button>
...
<?php }?>
						...
						<div id="relationGoodsSelectContainer" class="hide mt5 pdt5" style="border-top:1px dashed #ddd">
							<button type="button" class="btnSelectGoods resp_btn active" dp_id="relationGoods">상품 검색</button>
...
<?php }?>
									...
									<tbody>
										<tr>
											<th><label class="resp_checkbox"><input type="checkbox" name="chkAll" value="goods" title="전체선택" onClick="gGoodsSelect.exceptCheckAll(this)"></label></th>
<?php if(serviceLimit('H_AD')){?>
											<th>입점사명</th>
<?php }?>
											<th>상품명</th>
...
<?php }?>
						...
						<div id="relationSellerGoodsSelectContainer" class="hide mt5 pdt5" style="border-top:1px dashed #ddd">
							<button type="button" class="resp_btn active btnSelectGoods" dp_id="relationSellerGoods">상품검색</button>
							<button type="button" class="btnSelectGoodsDel resp_btn v3" selectType="goods">선택 삭제</button>
							<div class="mt5 wx600">
								<div class="goods_list_header">
								<table class="table_basic tdc">
...
<?php }?>
			...
			<ul class="bullet_hyphen resp_message">
				<li>관련 상품 기본 설정 <button type="button" class="resp_btn v2 btn_goods_default_set" data-type="relation">기본 설정</button></li>
				<li>추천 상품 디스플레이는 아이 디자인에서 설정해주세요. <span class="underline blue hand" onClick="window.open('/admin/design/main?setMode=pc')">아이 디자인 &gt; </span></li>
			</ul>
		</div>
	</div>
	<!-- 14. 추천 상품 종료 -->
	...
	<!-- 15. 오픈 마켓 연동 시작 -->
<?php if(!$TPL_VAR["socialcpuse"]){?>
	<a name="15" alt="오픈 마켓 연동"></a>
	<div class="bx-lay" data-bxcode="openmarket">
		<div class="bx-title">
			<div class="item-title">오픈 마켓 연동</div>
			<div class='right'></div>
		</div>
			...
			<table class="table_basic v7">
				...
				<tbody id="openmarketLists">
					...
					<tr class="openmarket-no-data">
						<td class="center" colspan="7">연동중인 마켓이 없습니다.</td>
					</tr>
				</tbody>
			</table>
		</div>
...
<?php }?>
	...
	<!-- 16. 입점 마케팅 시작 -->
<?php if(!$TPL_VAR["socialcpuse"]){?>
	<a name="16" alt="입점 마케팅"></a>
	<div class="bx-lay" data-bxcode="marketing">
		<div class="bx-title">
			<div class="item-title">입점 마케팅</div>
			<div class='right'></div>
		</div>
		<div class="cont">
			<table class="table_basic thl" id="feed_table">
				<tbody>
					<tr>
						<th class="left">상품 정보 전달</th>
						<td colspan="3">
							<div class="resp_radio">
								<!-- <?php if($TPL_VAR["goods"]["goods_seq"]){?> -->
								<label><input type="radio" name="feed_status" value="Y" <?php if($TPL_VAR["goods"]["feed_status"]=='Y'){?>checked="checked"<?php }?>  onclick="chgfeedinfoNew('Y');" <?php if($TPL_VAR["socialcpuse"]){?> disabled <?php }?>> 예</label>
								&nbsp;&nbsp;
								<label><input type="radio" name="feed_status" value="N" <?php if($TPL_VAR["goods"]["feed_status"]!='Y'||$TPL_VAR["socialcpuse"]){?>checked="checked"<?php }?>  onclick="chgfeedinfoNew('N');"> 아니오</label>
								<!-- <?php }else{?>  -->
								<label><input type="radio" name="feed_status" value="Y" <?php if(!$TPL_VAR["socialcpuse"]){?>checked="checked"<?php }?> onclick="chgfeedinfoNew('Y');" <?php if($TPL_VAR["socialcpuse"]){?> disabled <?php }?>> 예</label>
								&nbsp;&nbsp;
								<label><input type="radio" name="feed_status" value="N" <?php if($TPL_VAR["socialcpuse"]){?>checked="checked"<?php }?> onclick="chgfeedinfoNew('N');"> 아니오</label>
								<!-- <?php }?>  -->
							</div>
						</td>
					</tr>
					<tr>
						...
						<td colspan="3">
							<div class="resp_radio">
								<label><input type="radio" name="feed_goods_use" class="feed_goods_use" value='N' <?php if($TPL_VAR["goods"]["feed_goods_use"]!='Y'){?>checked<?php }?>>통합 설정</label>
								<label><input type="radio" name="feed_goods_use" class="feed_goods_use" value='Y' <?php if($TPL_VAR["goods"]["feed_goods_use"]=='Y'){?>checked<?php }?>>개별 설정</option>
							</div>
							<div id="feed_goods_name_view" class="<?php if($TPL_VAR["goods"]["feed_goods_use"]!='Y'){?>hide<?php }?> mt10">
								...
								<button type="button" onClick="javascript:helperMessage('replaceCode');" class="resp_btn">치환 코드</button>
								<ul class="bullet_hyphen resp_message">
									<li>상품명과 간략 설명 영문 작성 시, 반드시 대문자와 소문자 혼용하여 작성해 주세요.</li>
								</ul>
							</div>
						</td>
					</tr>
					<tr>
						<th class="left">검색어</th>
						<td colspan="3">
							<div class="resp_limit_text limitTextEvent">
								<input type="text" id="openmarket_keyword" name="openmarket_keyword" size='100' class="resp_text" maxlength="100" value="<?php echo $TPL_VAR["goods"]["openmarket_keyword"]?>" title="검색어는 ,(콤마)로 구분됩니다" />
							</div>
							<button type="button" id="keyword_btn" class="resp_btn" onclick="openmarketKeyword()">가져오기</button>
						</td>
					</tr>
					<tr>
						<th class="left">상품 상태 <span class="tooltip_btn" onClick="showTooltip(this, '/admin/tooltip/goods', '#regist_marketing_goods_status', 'sizeS')"></span></th>
						<td>
							<select id="feed_condition" name="feed_condition">
								<option value="">선택</option>
								<option value="신상품" <?php if($TPL_VAR["goods"]["feed_condition"]=='신상품'){?>selected<?php }?>>신상품</option>
								<option value="중고" <?php if($TPL_VAR["goods"]["feed_condition"]=='중고'){?>selected<?php }?>>중고</option>
								<option value="리퍼" <?php if($TPL_VAR["goods"]["feed_condition"]=='리퍼'){?>selected<?php }?>>리퍼</option>
								<option value="전시" <?php if($TPL_VAR["goods"]["feed_condition"]=='전시'){?>selected<?php }?>>전시</option>
								<option value="반품" <?php if($TPL_VAR["goods"]["feed_condition"]=='반품'){?>selected<?php }?>>반품</option>
								<option value="스크레치" <?php if($TPL_VAR["goods"]["feed_condition"]=='스크레치'){?>selected<?php }?>>스크레치</option>
							</select>
						</td>
						<th class="left">판매방식 구분</th>
						<td>
							<select id="product_flag" name="product_flag">
								<option value="">선택</option>
								<option value="도매" <?php if($TPL_VAR["goods"]["product_flag"]=='도매'){?>selected<?php }?>>도매</option>
								<option value="렌탈" <?php if($TPL_VAR["goods"]["product_flag"]=='렌탈'){?>selected<?php }?>>렌탈</option>
								<option value="대여" <?php if($TPL_VAR["goods"]["product_flag"]=='대여'){?>selected<?php }?>>대여</option>
								<option value="예약판매" <?php if($TPL_VAR["goods"]["product_flag"]=='예약판매'){?>selected<?php }?>>예약판매</option>
								<option value="구매대행" <?php if($TPL_VAR["goods"]["product_flag"]=='구매대행'){?>selected<?php }?>>구매대행</option>
							</select>
						</td>
					</tr>
					<tr>
						...
						<td>
							<select id="installation_costs" name="installation_costs">
								<option value="">선택</option>
								<option value="Y" <?php if($TPL_VAR["goods"]["installation_costs"]=='Y'){?>selected<?php }?>>설치비 추가</option>
							</select>
						</td>
						<th class="left">
							병행수입/주문 제작
						...
						<td>
							<select id="compound_state" name="compound_state">
								<option value="">선택</option>
								<option value="parallel_import" <?php if($TPL_VAR["goods"]["compound_state"]=='parallel_import'){?>selected<?php }?>>병행수입</option>
								<option value="order_made" <?php if($TPL_VAR["goods"]["compound_state"]=='order_made'){?>selected<?php }?>>주문제작</option>
							</select>
						</td>
					</tr>
					<tr>
						<th class="left">노출 이벤트
							<span class="tooltip_btn" onClick="showTooltip(this, '../tooltip/goods', '#regist_feed_evt_sdate', '500')"></span>
						</th>
						<td colspan="3">
							<label class="resp_checkbox"><input type="checkbox" name="marketing_event_set" value='y' <?php if($TPL_VAR["goods"]["feed_evt_sdate"]&&$TPL_VAR["goods"]["feed_evt_sdate"]!="0000-00-00"&&$TPL_VAR["goods"]["feed_evt_edate"]&&$TPL_VAR["goods"]["feed_evt_edate"]!="0000-00-00"){?>checked<?php }?>> 노출 이벤트 설정</label>
					...
					<tr>
						<th class="left">노출 배송비</th>
						<td colspan="3">
							<div class="resp_radio">
								<label><input type="radio" class="feed_shipp_type" name="feed_ship_type" value="G" <?php if($TPL_VAR["goods"]["feed_ship_type"]=='G'||$TPL_VAR["goods"]["feed_ship_type"]==''){?>checked<?php }?>> 상품 배송비와 동일</label>
								<label class="ml20"><input type="radio" class="feed_shipp_type" name="feed_ship_type" value="S" <?php if($TPL_VAR["goods"]["feed_ship_type"]=='S'){?>checked<?php }?>> 통합설정</label>
								<label class="ml20"><input type="radio" class="feed_shipp_type" name="feed_ship_type" value="E" <?php if($TPL_VAR["goods"]["feed_ship_type"]=='E'){?>checked<?php }?>> 개별설정</label>
							</div>
						</td>
					</tr>
					<tr class="epSel hide">
						<th class="left">배송비 개별 설정</th>
						<td colspan="3">
							<div class="resp_radio">
								<label><input type="radio" name="feed_pay_type" value="free" <?php if(!$TPL_VAR["goods"]["feed_pay_type"]||$TPL_VAR["goods"]["feed_pay_type"]=='free'){?>checked<?php }?>> 무료</label>
								<label class="ml10"><input type="radio" name="feed_pay_type" value="fixed" <?php if($TPL_VAR["goods"]["feed_pay_type"]=='fixed'){?>checked<?php }?>> 유료</label>
								...
								<label class="ml10">
									<input type="radio" name="feed_pay_type" value="postpay" <?php if($TPL_VAR["goods"]["feed_pay_type"]=='postpay'){?>checked<?php }?>> 착불
								</label>
					...
					<tr class="epSel hide">
						<th class="left">배송비 문구</th>
						<td colspan="3">
							<div class="resp_limit_text limitTextEvent">
								<input type="text" name="feed_add_txt" id="feed_add_txt" size='40' class="resp_text" maxlength="50" value="<?php echo $TPL_VAR["goods"]["feed_add_txt"]?>" title="예) 도서산간 5천원 추가"  <?php if($TPL_VAR["goods"]["feed_pay_type"]=='postpay'){?>disabled<?php }else{?><?php }?> />
							</div>
						</td>
					</tr>
				</tbody>
			...
			<ul class="bullet_hyphen resp_message">
				<li>입점 마케팅 필수 항목 <span class="underline blue hand btn_markting_required">자세히 보기 &gt;</span></li>
				<li>입점 마케팅 통합 설정은 <span class="underline blue hand" onClick="window.open('/admin/marketing/marketplace_url')">입점 마케팅</span>에서 가능합니다.</li>
			</ul>
			...
			<div class="option-message hide" style="margin:0 0 0 5px; border:1px solid #dadada; padding:10px 5px; background-color:#f1f1f1;">
				<ul class="desc">
					<li style="margin-bottom:5px">
						입점 마케팅 통합 설정은 마케팅 > <a href="../marketing/marketplace_url" target="_blank"><span class="highlight-link hand">입점마케팅 설정</span></a> 에서 가능 합니다.
					</li>
					<li style="margin-bottom:5px">
						페이스북 마케팅 상품 데이터 전달을 위해서는 반드시 간략 설명은 필수 입력 항목입니다.
					</li>
					<li style="margin-bottom:5px">
						페이스북 마케팅에 전달 할 상품 데이터의 상품명과 간략 설명을 영문으로 작성하는 경우, 반드시 대문자와 소문자 혼용하여 작성해 주세요. 영문 대문자로만 작성 시 상품 데이터가 전달되지 않습니다.
					</li>
					<li style="margin-bottom:5px">
						페이스북 페이지 '샵'에 상품 노출을 원하는 경우 반드시 상품 브랜드를 등록해 주세요.
					</li>
				</ul>
			</div>
		</div>
					...
					<td>
						<div style="max-width:1000px;">
							<span class="<?php if($TPL_VAR["cfg_goods"]["video_use"]=='Y'){?><?php }else{?>gray<?php }?> disabled">
								<button type="button" class="<?php if($TPL_VAR["cfg_goods"]["video_use"]=='Y'){?>batchVideoRegist<?php }?> resp_btn v2 " uptype="image" <?php if($TPL_VAR["cfg_goods"]["video_use"]!='Y'){?>onclick="videoSettingAlert();"<?php }?>>등록</button>
							</span>
					...
					<td>
						<div style="max-width:1000px;">
							<span class="<?php if($TPL_VAR["cfg_goods"]["video_use"]=='Y'){?><?php }else{?>disabled<?php }?> ">
								<button id="videofiles_tables" type="button" class="resp_btn v2 <?php if($TPL_VAR["cfg_goods"]["video_use"]=='Y'){?>batchVideoRegist<?php }?>"  uptype="contents" <?php if($TPL_VAR["cfg_goods"]["video_use"]!='Y'){?>onclick="videoSettingAlert();"<?php }?>>등록</button>
							</span>
