<?php /* Template_ 2.2.6 2022/03/07 12:32:25 /www/suns_firstmall_kr/admin/skin/default/_modules/layout/header.html 000050025 */ 
...
<script type="text/javascript">
	var gl_admin_skin_type = '<?php echo $TPL_VAR["admin_skin_type"]?>';
...
<?php if($TPL_VAR["admin_skin_user_type"]=='existing'){?>
					<!-- [ 관리자 전환 버튼 : 시작 ] -->
					<img src="/admin/skin/default/images/common/<?php if($TPL_VAR["admin_skin_type"]=='new'){?>org<?php }else{?>new<?php }?>_ver_change_btn.png" data-admin-skin-type='<?php echo $TPL_VAR["admin_skin_type"]?>' class='ver_change_btn' style="display: none;">					
					<!-- [ 관리자 전환 버튼 : 시작 ] -->					
<?php }?>
...
<?php }else{?>
<?php if($TPL_VAR["admin_skin_type"]=='new'||$TPL_VAR["admin_skin_type"]==''){?>								
								<li class="item hsnb-pc shortcutBtn">	
...
<?php }else{?>
								<li class="item hsnb-pc"><a href="//<?php echo $TPL_VAR["pcDomain"]?>/?setDesignMode=off&setMode=pc" target="_blank"  title="PC 쇼핑몰 바로가기">PC 바로가기</a></li>
								<li class="item hsnb-mobile"><a href="//<?php echo $TPL_VAR["mobileDomain"]?>/?setDesignMode=off&setMode=mobile" target="_blank" title="모바일 쇼핑몰 바로가기">모바일 쇼핑몰 바로가기</a></li>
<?php }?>
<?php }?>
...
<?php }?>										
										<li>
											<a href="https://ads.gabia.com/" target="_blank">가비아애즈</a>
...
<?php }else{?>
						<li class="item hsnb-link">
							<a href="javascript:;" title="더보기" class="openBtn">더보기</a>
							<div class="hsnbm-menu">
								<span class="point_c"></span>
								<ul class="tb_link">
									<li><a href="https://www.firstmall.kr/myshop/shops/<?php echo $TPL_VAR["enc_shopsno"]?>/view" target="_blank" title="새창열림">MY퍼스트몰</a></li>
									<li><a href="https://www.firstmall.kr/ec_hosting/customer/1to1.php" target="_blank" title="새창열림">1:1문의</a></li>
									<li><a href="https://www.firstmall.kr/ec_hosting/customer/" target="_blank" title="새창열림">고객센터</a></li>
									...
									<li><a href="https://design.firstmall.kr/" target="_blank" title="새창열림">디자인샵</a></li>
									<li><a href="javascript:<?php if(solutionServiceCheck( 1)){?>alert('맞춤개발 서비스는 프리미엄몰Plus와 독립몰Plus에서만 적용됩니다.'); <?php }?>void(window.open('https://www.firstmall.kr/buildservice/customize'));">맞춤개발</a></li>
<?php if($TPL_VAR["config_system"]["webmail_domain"]){?>
										<li><a href="http://webmail.<?php echo $TPL_VAR["config_system"]["webmail_domain"]?>/admin/adminhome" target="_blank" title="새창열림">웹메일/세금계산서</a></li>
<?php }else{?>
...
<?php if($TPL_VAR["managerInfo"]["manager_seq"]){?>
						<li class="item hsnb-admin">
							<a href="javascript:;" title="관리자 정보" class="openBtn">관리자 정보</a>
							<div class="hsnbm-menu">
								<span class="point_c"></span>
								<ul class="tb_admin_info">
									<li class="admin_thumb">
										<img src=<?php if($TPL_VAR["managerInfo"]["mphoto"]){?>"../../../data/icon/manager/<?php echo $TPL_VAR["managerInfo"]["mphoto"]?>"<?php }else{?>"/admin/skin/default/images/main/def_img.png"<?php }?> width="50" height="50" align="absmiddle" />
									</li>
									<li class="admin_info">
										<p><?php echo $TPL_VAR["managerInfo"]["manager_id"]?></p>
										<p>(<?php echo $TPL_VAR["managerInfo"]["mname"]?>)</p>
										<p style="color:#348ddb;"><?php if($TPL_VAR["managerInfo"]["manager_yn"]=='Y'){?>대표운영자<?php }else{?>부운영자<?php }?></p>
									</li>
									<li class="admin_link">
										<a href="../setting/manager_reg?manager_seq=<?php echo $TPL_VAR["managerInfo"]["manager_seq"]?>">정보수정</a>
...
<?php }?>				
					<li>									
						<ul class="header-gnb">
							<li class="mitem-td <?php if($TPL_VAR["adminMenuCurrent"]=='main'){?>current<?php }?>">
								<span class="mitem mitem_main"><a href="../main/index"><span>홈</span></a></span>
							</li>			
<?php if($TPL_adminMenu_1){foreach($TPL_VAR["adminMenu"] as $TPL_K1=>$TPL_V1){?>							
<?php if($TPL_K1=='design'){?><li style="float:right;"><ul><?php }?>
							<li class="mitem-td <?php if(in_array($TPL_VAR["adminMenuCurrent"],$TPL_V1["folders"])){?>current<?php }?> <?php if($TPL_K1=='setting'){?>qnb-config<?php }?>" code="<?php echo $TPL_K1?>">								
								
								<span class="mitem mitem_<?php echo $TPL_K1?>">
									<a href="<?php echo $TPL_V1["url"]?>">
										<span><?php echo $TPL_V1["name"]?></span>
<?php if($TPL_K1!='setting'){?><div class="header-gnb-issueCount-layer" code="<?php echo $TPL_K1?>"></div><?php }?>	
									</a></span>													
<?php if($TPL_V1["childs"]){?>
								<div class="submenu-wrapper">
									<div class="submenu <?php if($TPL_K1=='design'||$TPL_K1=='setting'){?>v2<?php }?>">
										<ul class="<?php if($TPL_K1=='setting'){?>setting<?php }?>">
<?php if($TPL_K1!="setting"){?>	
<?php if(is_array($TPL_R2=$TPL_V1["childs"])&&!empty($TPL_R2)){$TPL_I2=-1;foreach($TPL_R2 as $TPL_V2){$TPL_I2++;?>											
												<li>																						
<?php if($TPL_V1["folders"][$TPL_I2]=='distributor'||$TPL_V1["folders"][$TPL_I2]=='market_linkage'||$TPL_V1["folders"][$TPL_I2]=='market_qna_list'){?>
														<div style="border-top:1px solid #C2C2C2;padding-top:3px;width:95%;margin-left:5px;"></div>
<?php }?>
<?php if($TPL_V2["limit"]&&serviceLimit($TPL_V2["limit"],'return')){?>
													<a href="#" onclick="<?php echo serviceLimit($TPL_V2["limit"])?>"><?php if($TPL_K1=='setting'){?><?php echo str_pad($TPL_I2+ 1, 2,"0",$TPL_VAR["STR_PAD_LEFT"])?>.<?php }?> <?php echo $TPL_V2["name"]?></a>
<?php }else{?>
													<a href="<?php echo $TPL_V2["url"]?><?php if($TPL_I2== 0&&$TPL_K1=='design'){?><?php if(serviceLimit('S32')){?>?prefix=fammerce<?php }?><?php }?>"><?php if($TPL_K1=='setting'){?><?php echo str_pad($TPL_I2+ 1, 2,"0",$TPL_VAR["STR_PAD_LEFT"])?>.<?php }?> <?php echo $TPL_V2["name"]?> 	</a>
<?php }?>
												</li>
<?php }}?>
<?php }else{?>																			
<?php if(is_array($TPL_R2=$TPL_VAR["settingMenu"]['setting']['childs'])&&!empty($TPL_R2)){foreach($TPL_R2 as $TPL_V2){?>
												<li class="<?php echo $TPL_V2["class"]?>">	
																					
<?php if($TPL_V2["limit"]&&serviceLimit($TPL_V2["limit"],'return')){?>
													<a href="#" onclick="<?php echo serviceLimit($TPL_V2["limit"])?>"><span class="hide"><?php echo str_pad($TPL_K1+ 1, 2,"0",$TPL_VAR["STR_PAD_LEFT"])?></span> <?php echo $TPL_V2["name"]?></a>
<?php }else{?>
													<a href="<?php echo $TPL_V2["url"]?>"><span class="hide"><?php echo str_pad($TPL_K1+ 1, 2,"0",$TPL_VAR["STR_PAD_LEFT"])?>.</span> <?php echo $TPL_V2["name"]?></a>
<?php }?>
												</li>
<?php }}?>
<?php }?>

<?php if($TPL_K1=="design"){?>
<?php if($TPL_VAR["config_system"]["operation_type"]=='light'){?>
<?php if($TPL_VAR["config_system"]["skin_type"]=='responsive'){?>
											<li><a href="../design/main?setMode=mobile" target="_blank">디자인환경 (반응형)</a></li>
<?php }else{?>
											<li><a href="../design/main?setMode=pc" target="_blank">디자인환경 (반응형PC)</a></li>
											<li><a href="../design/main?setMode=mobile" target="_blank">디자인환경 (반응형Mobile)</a></li>
<?php }?>
<?php }else{?>
											<li><a href="../design/main?setMode=pc" target="_blank">디자인환경 (PC)</a></li>
											<li><a href="../design/main?setMode=mobile" target="_blank">디자인환경 (Mobile)</a></li>
											<li>
<?php if(serviceLimit('H_FR')){?>
												<a href="#" onclick="<?php echo serviceLimit('A1')?>">디자인환경 (Facebook)</a>
<?php }else{?>
...
<?php if($TPL_K1=='setting'){?></li></ul><?php }?>
<?php }}?>
						</ul>
					</li>					
				</ul>			
...
<?php if($TPL_VAR["adminMenuCurrent"]!='main'&&$TPL_VAR["adminMenuCurrent"]!='realpacking'&&$TPL_VAR["adminMenuCurrent"]!='export_download'&&$TPL_VAR["adminMenuCurrent"]!='total_download'){?>
			<div class="LNB">			
				<div id="lnbCloseBtn" class="btn_lnb_close"><span class="i_lnb_close"></span></div>	
				<div class="logo_wrap">
					<a href="/admin">			
...
<?php if(serviceLimit('H_AD')){?>
											<li class="<?php if(in_array($TPL_VAR["adminSubMenuCurrent"],$TPL_V3["file"])&&($TPL_VAR["adminMenuCurrent"]==$TPL_V3["folder"])){?>current <?php }?><?php echo $TPL_V3["class"]?>"><a href="<?php echo $TPL_V3["url"]?>"><?php echo $TPL_V3["name"]?></a></li>
<?php }?>	
<?php }else{?>	
...
<?php }?>
			...
			<div class="contentsWarp <?php if($TPL_VAR["adminMenuCurrent"]=='main'||$TPL_VAR["adminMenuCurrent"]=='realpacking'||$TPL_VAR["adminMenuCurrent"]=='export_download'||$TPL_VAR["adminMenuCurrent"]=='total_download'){?>close<?php }?>">
			<!--[ 레이아웃 바디(본문) : 시작 ]-->