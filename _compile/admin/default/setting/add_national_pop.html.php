<?php /* Template_ 2.2.6 2022/03/07 12:32:25 /www/suns_firstmall_kr/admin/skin/default/setting/add_national_pop.html 000047277 */ 
...
.content { height: calc(100% - 87px) !important; }
</style>

<script type="text/javascript" src="/app/javascript/js/admin-shipping.js?dummy=<?php echo date('Ymd')?>"></script>
<script type="text/javascript">
var unit	= '<?php echo $TPL_VAR["basic_currency_info"]['currency_symbol']?>'; // 기본 통화 단위
...
<?php if($TPL_VAR["params"]["custom_set_use"]=='Y'){?>
	...
	$("input[name='shipping_set_name']").val('<?php echo $TPL_VAR["params"]["shipping_set_name"]?>');
<?php }?>
...
<?php }?>
<?php if($TPL_VAR["params"]["reserve_use"]=='Y'){?>
	// 희망 사용여부 처리 및 데이터 채우기
	$("input[name='reserve_use']").attr('checked',true).trigger('change');
	$("input[name='delivery_reserve_type']:radio[value=<?php echo $TPL_VAR["params"]["delivery_reserve_type"]?>]").attr("checked","checked");
<?php }?>
...
<div class="content">
	<div class="price_area pd10">
		...
		<thead>
		<tr>
			<th class="its-th center">배송방법</th>
			<th class="its-th center" colspan="3">
		...
		<tbody>
		<tr class="std_tr">
			<td class="its-td left" rowspan="6">
...
<?php }}?>
			...
			<td class="its-td center" rowspan="4">
				배송비
			</td>
				...
				<div class="basic_shipping_area">
					...
					<tr>
						<th class="its-th center" colspan="2">
							<select class="shipping_opt_type std_area" name="shipping_opt_type[std]" onchange="shipping_opt_type(this, []);">
...
<?php if($_GET["nation"]=='korea'){?>대한민국<?php }else{?>전세계<?php }?>
<?php }?>
								<span class="add_zone hand blue"></span>
							</span>
					...
					<tr>
						...
						<td class="its-td">
							...
							<input type="text" name="shipping_cost[std][]" value="0" class="line right shipping-cost-input std_area onlyfloat wx50" onclick="if($(this).val()==0) $(this).val('');" disabled /> <?php echo $TPL_VAR["basic_currency_info"]['currency_symbol']?>
					...
					<tr class="last-tr-base hide">
						...
						<td class="its-td" width="105px">
							<input type="hidden" name="shipping_cost_seq[std][]" value="0" /> 
							<input type="text" name="shipping_cost[std][]" value="0" class="line right shipping-cost-input std_area onlyfloat wx50" onclick="if($(this).val()==0) $(this).val('');" /> <?php echo $TPL_VAR["basic_currency_info"]['currency_symbol']?>
		...
		<tr class="add_tr">
			...
			<td class="its-td left">
				<!-- 금액 설정부 :: START -->
				<table class="table_basic wauto price_set_area v7" id="price_add" price_type="add">
				<thead>
				<tr>
					<th class="its-th center" colspan="2">
				...
				<tr>
					<th class="its-th center">
						<div class="section_area_add_btn hide">
							<span class="btn small ctrl-add-btn add-row cyanblue add_btn_area"><button class="ctrl-btn resp_btn active size_S" type="button" onclick="add_price_section(this,'section');">추가</button></span>
						</div>
					</th>
					<td class="its-td section_area_td">
						<div class="section_area_input hide">
					...
					<td class="its-td">
						...
						<input type="text" name="shipping_cost[add][]" value="0" class="line right shipping-cost-input add_area onlyfloat wx50" onclick="if($(this).val()==0) $(this).val('');" disabled /> <?php echo $TPL_VAR["basic_currency_info"]['currency_symbol']?>
				...
				<tr class="last-tr-base hide">
					...
					<td class="its-td">
						<input type="hidden" name="shipping_cost_seq[add][]" value="0" /> 
						<input type="text" name="shipping_cost[add][]" value="0" class="line right shipping-cost-input add_area onlyfloat wx50" onclick="if($(this).val()==0) $(this).val('');" /> <?php echo $TPL_VAR["basic_currency_info"]['currency_symbol']?>
		...
		<tr class="hop_tr">
			...
			<td class="its-td left">
				...
				<div class="hopeday_setting_area">
					...
					<tr>
						<th class="left">특정요일 선택 불가</th>
						<td class="its-td">
							<div class="resp_checkbox">
								<label><input type="checkbox" class="hop_area" name="hopeday_limit_week[]" value="1" <?php if($TPL_VAR["params"]['hopeday_limit_week_arr'][ 1]=='1'){?>checked<?php }?> /> 월요일</label>								
								<label><input type="checkbox" class="hop_area" name="hopeday_limit_week[]" value="2" <?php if($TPL_VAR["params"]['hopeday_limit_week_arr'][ 2]=='1'){?>checked<?php }?> /> 화요일</label>								
								<label><input type="checkbox" class="hop_area" name="hopeday_limit_week[]" value="3" <?php if($TPL_VAR["params"]['hopeday_limit_week_arr'][ 3]=='1'){?>checked<?php }?> /> 수요일</label>								
								<label><input type="checkbox" class="hop_area" name="hopeday_limit_week[]" value="4" <?php if($TPL_VAR["params"]['hopeday_limit_week_arr'][ 4]=='1'){?>checked<?php }?> /> 목요일</label>								
								<label><input type="checkbox" class="hop_area" name="hopeday_limit_week[]" value="5" <?php if($TPL_VAR["params"]['hopeday_limit_week_arr'][ 5]=='1'){?>checked<?php }?> /> 금요일</label>								
								<label><input type="checkbox" class="hop_area" name="hopeday_limit_week[]" value="6" <?php if($TPL_VAR["params"]['hopeday_limit_week_arr'][ 6]=='1'){?>checked<?php }?> /> 토요일</label>								
								<label><input type="checkbox" class="hop_area" name="hopeday_limit_week[]" value="0" <?php if($TPL_VAR["params"]['hopeday_limit_week_arr'][ 0]=='1'){?>checked<?php }?> /> 일요일</label>								
							</div>
						</th>
					</tr>
					<tr>
						<th class="left">선택 불가일</th>
						<td class="its-td nonpd clear">
							<table class="table_basic v3 hope_day_tb info-tb-inner" style="border-left: 0 !important;">
							<col width="135px" /><col />
							<thead>
							<tr>
								<td>매년</td>
...
<?php }?>
				...
				<div class="hop_shipping_area pdt20">
					<table class="table_basic wauto price_set_area v7" id="price_hop" cellspacing="0" cellpadding="0" price_type="hop">
					...
					<tr>
						...
						<td class="its-td section_area_td">
							...
							<div class="section_area_input hide">
								<input type="text" name="section_st[hop][]" value="0" class="section_st_input hop_area section_area_td wx90" readonly /> <span class="section_st_unit"></span>
								~
						...
						<td class="its-td right pdr10">
							<input type="hidden" name="shipping_cost_seq[hop][]" value="0" /> 
							<input type="text" name="shipping_cost[hop][]" value="0" class="line right shipping-cost-input hop_area onlyfloat wx50" onclick="if($(this).val()==0) $(this).val('');" disabled /> <?php echo $TPL_VAR["basic_currency_info"]['currency_symbol']?></br>
							
							당일
							<input type="text" name="shipping_cost_today[hop][]" value="0" class="line right shipping-cost-today hop_area onlyfloat today_idx_0 wx50" onclick="if($(this).val()==0) $(this).val('');" disabled /> <?php echo $TPL_VAR["basic_currency_info"]['currency_symbol']?>
					...
					<tr class="last-tr-base hide">
						...
						<td class="its-td right pdr10">
							<input type="hidden" name="shipping_cost_seq[hop][]" value="0" /> 
							<input type="text" name="shipping_cost[hop][]" value="0" class="line right shipping-cost-input hop_area onlyfloat wx50" onclick="if($(this).val()==0) $(this).val('');" /> <?php echo $TPL_VAR["basic_currency_info"]['currency_symbol']?><br/>						
							당일
							<input type="text" name="shipping_cost_today[hop][]" value="0" class="line right shipping-cost-today hop_area onlyfloat today_idx_0 wx50" onclick="if($(this).val()==0) $(this).val('');" disabled /> <?php echo $TPL_VAR["basic_currency_info"]['currency_symbol']?>
		...
		<tr class="store_tr">
			...
			<td class="its-td left">
				...
				<div class="direct_set_area">
					<button type="button" onclick="shipping_address_pop('address');" class="resp_btn v2 store_btn_area">설정</button>
<?php if($TPL_VAR["scm_cfg"]['use']=="Y"){?>
					<span class="warehouseAdress gray pdr10" style="float:right;">
						※ 미사용 창고는 사용자화면에 노출되지 않습니다.
					</span>
<?php }?>
				</div>
				<div class="direct_store_area pdt5">
					<!-- 매장 설정부 :: START -->
					<table class="table_row_basic v4 store_tb">
					<colgroup>
						<col width="80px" />
						<col width="100px" />
						<col width="60px" />
...
<?php if($TPL_V1["store_scm_use"]=='Y'){?>
							<div class="blue">(사용 창고)</div>
<?php }elseif($TPL_V1["store_scm_use"]=='N'){?>
...
<?php }?>
						...
						<input name="store_type[]" type="hidden" value="<?php echo $TPL_V1["store_type"]?>" />
						<input name="store_scm_seq[]" type="hidden" value="<?php echo $TPL_V1["store_scm_seq"]?>" />
					</tr>
<?php }}?>
<?php }else{?>
					<tr base_tr="Y">
						<td colspan="8" class="its-td center">설정된 매장이 없습니다.</td>
					</tr>
<?php }?>
					</tbody>
					</table>
		...
		<!-- 반품배송비 설정부 :: START -->
		<tr class="refund_tr">
			<td class="its-td center" rowspan="2">반품</br/>배송비</td>
			<td class="its-td nonpd center check_td">반품</td>
			<td class="its-td left">
				편도 : <input type="text" name="refund_shiping_cost" id="refund_shiping_cost" class="line right shipping-cost-input onlyfloat wx50" value="<?php echo $TPL_VAR["params"]["refund_shiping_cost"]?>" /> <?php echo $TPL_VAR["basic_currency_info"]['currency_symbol']?>

				( <label class="resp_checkbox"><input type="checkbox" name="shiping_free_yn" value="Y" <?php if($TPL_VAR["params"]["shiping_free_yn"]=='Y'){?>checked<?php }?>/> 배송비가 무료인 경우, 반품배송비 왕복 <span class="refund_shiping_double"><?php echo ($TPL_VAR["params"]["refund_shiping_cost"]* 2)?></span> <?php echo $TPL_VAR["basic_currency_info"]['currency_symbol']?> 받음</label> )
			</td>
		</tr>
		<tr class="refund_tr">
			<td class="its-td nonpd center check_td">(맞)교환</td>
			<td class="its-td left">
				왕복 : <input type="text" name="swap_shiping_cost" class="line right shipping-cost-input onlyfloat wx50" value="<?php echo $TPL_VAR["params"]["swap_shiping_cost"]?>" /> <?php echo $TPL_VAR["basic_currency_info"]['currency_symbol']?>
