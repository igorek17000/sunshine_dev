<?php /* Template_ 2.2.6 2022/03/07 12:32:25 /www/suns_firstmall_kr/admin/skin/default/setting/shipping_group.html 000012193 */ 
...
<?php if(serviceLimit('H_AD')&&$TPL_VAR["view"]["provider_seq"]){?>
			<h2><?php echo $TPL_VAR["view"]["provider_name"]?> → 배송비</h2>
<?php }else{?>
			<h2>배송비</h2>
<?php }?>
		</div>
...
<?php if(!$TPL_VAR["view"]){?>
		...
		<div class="page-buttons-right">
			<a class="resp_btn active size_L"  <?php if(serviceLimit('H_FR')&&count($TPL_VAR["grp_list"])> 2){?>onclick="<?php echo serviceLimit('A2')?>"<?php }else{?>href="./shipping_group_regist"<?php }?>>등록</a>
		</div>
<?php }?>
...
<?php }?>
		<!-- LIST GROUP -->
		<table class="table_row_basic v2 grp-list">
			<colgroup>
				<col width="4%" /><!-- chk box -->
				<col width="4%" /><!-- 번호 -->
				<col width="15%" /><!-- 배송그룹명 -->
