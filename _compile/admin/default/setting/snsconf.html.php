<?php /* Template_ 2.2.6 2022/03/07 12:32:25 /www/suns_firstmall_kr/admin/skin/default/setting/snsconf.html 000061733 */  $this->include_("snslinkurl");
...
<?php }?>
		...
		addtr += "상품 할인가(판매가) x 수량의 <input type='text' name='fblike_sale_price[]' value='" + sale_price + "' size='3' class='line onlynumber input-box-default-text' "+disabled+"' />% 할인, </td></tr><tr><th>마일리지</th><td>";
...
<?php }?>
		...
		$(".facebookconflay").live("click",function(){
			openDialog("페이스북 전용앱 설정", "snsdiv_f", {"width":"700","show" : "fade","hide" : "fade"});
		});

		$(".twitterconflay").live("click",function(){
			openDialog("트위터 설정", "snsdiv_t", {"width":"770","show" : "fade","hide" : "fade"});
		});
		...
		$(".naverconflay").click(function() {
			...
			openDialog("네이버 아이디로 로그인 설정", "snsdiv_n", {"width":"700","height":"720","show" : "fade","hide" : "fade"});
		});

		$(".kakaoconflay").click(function() {
			openDialog("카카오 설정", "snsdiv_k", {"width":"700","show" : "fade","hide" : "fade"});
		});

		$(".instagramconflay").click(function() {
			openDialog("인스타그램 설정", "snsdiv_i", {"width":"700","show" : "fade","hide" : "fade"});
		});

		// 애플 로그인 설정
		$(".appleconflay").click(function() {
			openDialog("애플 설정", "snsdiv_a", {"width":"700","show" : "fade","hide" : "fade"});
		});
			...
			<tr class="hide"> <!--fbLikeDetail"-->
				...
				<td>
					...
					<ul class="bullet_hyphen gray mt10">
						<li>마일리지 및 포인트 지급 시점은 <a class="link_blue_01" href="/admin/setting/reserve" target="_blank">마일리지/포인트/예치금</a>에 따릅니다.</li>
...
<div class="contents_dvs">
	<div class="item-title">고객상담</div>
	
	<div class="dvs_box">
		<ul class="tab_01 tabEvent">
			<li><a href="javascript:void(0);" data-showcontent="tabCon7" class="current">채널톡</a></li>
			<li><a href="javascript:void(0);" data-showcontent="tabCon8">네이버 톡톡 </a></li>
		</ul>
		...
		<div class="tabCon7">
			<table class="table_basic thl">
				<tr>
					...
					<td>
						<div class="resp_radio">
							<label><input type="radio" name="channeltalk_use" value="Y" <?php if($TPL_VAR["channeltalk"]["use"]=='Y'){?> checked<?php }?>> 사용함</label>
							<label><input type="radio" name="channeltalk_use" value="T" <?php if($TPL_VAR["channeltalk"]["use"]=='T'){?> checked <?php }?>> 테스트</label>
							<span class="tooltip_btn" onClick="showTooltip(this, '/admin/tooltip/snsconf', '#tip15')"></span>
							<label><input type="radio" name="channeltalk_use" value="N" <?php if($TPL_VAR["channeltalk"]["use"]=='N'||!$TPL_VAR["channeltalk"]["use"]){?> checked <?php }?>> 사용 안 함</label>
							<div class="resp_message v2">
							- 서비스를 사용하시려면 채널톡 서비스를 신청 후, 가능합니다.
							</div>
						</div>
					</td>
				</tr>
				<tr id="channeltalk_display"  <?php if($TPL_VAR["channeltalk"]["use"]=='N'||!$TPL_VAR["channeltalk"]["use"]){?> class="hide"<?php }?>>
					<th>연동 설정</th>
					<td>
						<div>
							<dl class="dl_list_01 w120 mt3">
								<dt>Plugin Key</dt>
								<dd style="width: 240px;"><input type="text" name="plugin_key" value="<?php echo $TPL_VAR["channeltalk"]["plugin_key"]?>" class="resp_limit_text" style="width: 100%;" /></dd>
							</dl>
							<dl class="dl_list_01 w120 mt3">
								<dt>Access Secret</dt>
								<dd style="width: 240px;"><input type="text" name="access_secret" value="<?php echo $TPL_VAR["channeltalk"]["access_secret"]?>" class="resp_limit_text" style="width: 100%;" /></dd>
							</dl>
							<div class="resp_message v2">
							- 채널톡 서비스 신청 완료 후, 발급된 Plugin Key, Access Secret 를 복사해 입력합니다.
							</div>
						</div>
					</td>
				</tr>
				<tr>
					<th>연동 설정</th>
					<td>
						<a href="https://channel.io/ko" class="resp_btn active size_XL" <?php if($TPL_VAR["channeltalk"]["use"]=='Y'){?> style="display:none;"<?php }?> id="channeltalk_apply" target="_blank">신청</a>
						<a href="https://desk.channel.io/#/signin" class="resp_btn v2 size_XL" target="_blank">관리</a>
					</td>
				</tr>
			</table>				
		</div>

		<div class="hide" id="tabCon8">
			<table class="table_basic thl">
				<tr>
					<th>
						네이버 톡톡
...
<?php if($TPL_VAR["sns"]["ntalk_connect"]=='Y'){?>
					<td class="clear">
						...
						<table class="table_basic thl v3">
							<tr>
							<td colspan="2">
								<span class="btn large <?php if($TPL_VAR["sns"]["ntalk_use"]==''||$TPL_VAR["sns"]["ntalk_use"]=='Y'){?>green<?php }?>"><button class="resp_btn  <?php if($TPL_VAR["sns"]["ntalk_use"]==''||$TPL_VAR["sns"]["ntalk_use"]=='Y'){?>active5<?php }else{?>v2<?php }?>"type="button" <?php if($TPL_VAR["sns"]["ntalk_use"]=='N'){?>class="ntalkenable"<?php }?>>ON</button></span>
							<span class="btn large <?php if($TPL_VAR["sns"]["ntalk_use"]=='N'){?>green<?php }?>"><button class="resp_btn <?php if($TPL_VAR["sns"]["ntalk_use"]=='N'){?>active5<?php }else{?>v2<?php }?>" type="button" <?php if($TPL_VAR["sns"]["ntalk_use"]=='Y'){?>class="ntalkdisable"<?php }?>>OFF</button></span>
							...
							<tr>
								...
								<td>
									<div class="resp_checkbox">
										<label><input type="checkbox" name="ntalk_use_mobile_product" value="Y" <?php if($TPL_VAR["sns"]["ntalk_use_mobile_product"]=='Y'){?>checked="checked"<?php }?>/> 상품상세</label>
										<label><input type="checkbox" name="ntalk_use_mobile_main" value="Y" <?php if($TPL_VAR["sns"]["ntalk_use_mobile_main"]=='Y'){?>checked="checked"<?php }?>/> 메인</label>
...
<?php }else{?>
							<tr>
								...
								<td>
									<div class="resp_checkbox">
										<label><input type="checkbox" name="ntalk_use_web_product" value="Y" <?php if($TPL_VAR["sns"]["ntalk_use_web_product"]=='Y'){?>checked="checked"<?php }?>/> 상품상세</label>
										<label><input type="checkbox" name="ntalk_use_web_quick" value="Y" <?php if($TPL_VAR["sns"]["ntalk_use_web_quick"]=='Y'){?>checked="checked"<?php }?>/> 퀵메뉴</label>
...
<?php }?>
							<tr>
								...
								<td>
									<div class="resp_radio">
										<label><input type="radio" name="ntalk_use_sniffet" value="Y" <?php if($TPL_VAR["sns"]["ntalk_use_sniffet"]=='Y'){?>checked="checked"<?php }?>/> 사용함</label>
...
<div class="box_style_05 mt20 tabCon7">
	<div class="title">안내</div>
	<ul class="bullet_circle">
		<li>
			채널톡 가입 완료 시 발급되는 Plugin Key, Access Secret을 입력 후 저장해주시면 됩니다.
			<a href="https://www.firstmall.kr/customer/faq/1686" class="link_blue_01" target="_blank">자세히보기</a>
		</li>
		<li>채널톡 사용 방법 <a href="https://guide-kr.channel.io/" class="link_blue_01" target="_blank">자세히보기</a></li>
		<li>채널톡 서비스 이용 관련 <a href="https://channel.io/ko" class="link_blue_01" target="_blank">문의하기</a></li>
	</ul>
</div>

<div class="box_style_05 mt20 tabCon8 hide">
	<div class="title">안내</div>
	<ul class="bullet_circle">
		<li>
		...
		<li>SNS 를 통한 로그인 및 회원 가입은 <a href="/admin/setting/member" class="link_blue_01">로그인 및 회원가입</a>에서도 설정이 가능합니다.</li>
	</ul>
</div>
...
<?php }?>
    ...
    $(function(){
	    ...
	    // 사용 여부
	    $("input[name='channeltalk_use']").on("click", function(){
            var channeltalk_use = $("input[name='channeltalk_use']:checked").val();
            if(channeltalk_use == 'Y' || channeltalk_use == 'T'){
                $("#channeltalk_display").removeClass('hide');
				if (channeltalk_use == 'Y') {
					$('#channeltalk_apply').css('display','none');
				} else{
					$('#channeltalk_apply').css('display','inline-block');
				}
            } else {
                $('#channeltalk_display').addClass('hide');
				$("#channeltalk_apply").css('display','inline-block');
            }
	    });
    });
