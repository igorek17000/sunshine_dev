<?php /* Template_ 2.2.6 2022/03/07 12:32:25 /www/suns_firstmall_kr/admin/skin/default/setting/manager.html 000009196 */ 
$TPL_loop_1=empty($TPL_VAR["loop"])||!is_array($TPL_VAR["loop"])?0:count($TPL_VAR["loop"]);?>
<?php $this->print_("layout_header",$TPL_SCP,1);?>

<script type="text/javascript" src="../../app/javascript/js/admin-manager.js?dummy=<?php echo date('Ymd')?>"></script>
...
<?php if($TPL_VAR["service_limit"]&&$TPL_VAR["config_system"]["service"]["max_manager_cnt"]&&$TPL_VAR["use_manager_cnt"]>=$TPL_VAR["config_system"]["service"]["max_manager_cnt"]){?>		
			<button class="resp_btn active size_L" type="button"  <?php if($TPL_VAR["functionLimit"]){?>  onclick="servicedemoalert('use_f');" <?php }else{?> onclick="manager_reg(false)" <?php }?> >관리자 등록</button>
<?php }else{?>
			<button class="resp_btn active size_L" type="button"  <?php if($TPL_VAR["functionLimit"]){?>  onclick="servicedemoalert('use_f');" <?php }else{?> onclick="manager_reg(true)" <?php }?> >관리자 등록</button>
<?php }?>	
		</div>
	</div>
</div>
...
<?php if($TPL_VAR["service_limit"]&&$TPL_VAR["config_system"]["service"]["max_manager_cnt"]){?> 
	<div class="list_info_container">		
		<div class="dvs_left">			
		현재 : <?php echo number_format($TPL_VAR["use_manager_cnt"])?>명 / <?php echo number_format($TPL_VAR["config_system"]["service"]["max_manager_cnt"])?>명까지 가능			
		</div>
	</div>	
<?php }?>	
...
<?php if($TPL_VAR["managerInfo"]["manager_yn"]=='Y'){?>
				<button type="button" class="resp_btn v3"  <?php if($TPL_VAR["isdemo"]["isdemo"]){?>  <?php echo $TPL_VAR["isdemo"]["isdemojs1"]?> <?php }else{?>  onclick="auto_logout();" <?php }?> >자동로그아웃 설정</button>
