<?php /* Template_ 2.2.6 2022/03/07 12:32:25 /www/suns_firstmall_kr/admin/skin/default/marketing/marketplace_url.html 000053509 */ 
...
<script type="text/javascript">
	...
	$(function(){
		...
		$("a[href='#<?php echo $_GET["type"]?>']").trigger("click");
...
<div id="page-title-bar-area">
	<div id="page-title-bar">
		...
		<div class="page-title">
			<h2>
<?php if($TPL_VAR["admin_skin_type"]=="org"){?>
					입점 마케팅
<?php }else{?>
<?php if($_GET["type"]=="naverpay"){?>네이버 페이<?php }elseif($_GET["type"]=="navershopping"){?>네이버 쇼핑<?php }elseif($_GET["type"]=="daum"){?>다음 쇼핑하우<?php }elseif($_GET["type"]=="facebook"){?>페이스북 쇼핑<?php }elseif($_GET["type"]=="google"){?>구글 스마트 쇼핑<?php }elseif($_GET["type"]=="talkbuy"){?>카카오페이 구매<?php }?>
<?php }?>
			</h2>
		</div>
...
<?php if($TPL_VAR["functionLimit"]){?>	
				<button type="button" onclick="servicedemoalert('use_f');" class="resp_btn active size_L">저장</button>
<?php }else{?>
				<button type="submit" class="resp_btn active size_L">저장</button></li>
<?php }?>
		</ul>
<?php }?>
...
<div class="center mt5" >
	<div class="naverpay hide mt25 mb20"><?php echo $TPL_VAR["marketing_naverpay_banner"]?></div>	
	<div class="navershopping hide mt25 mb20"><?php echo $TPL_VAR["marketing_navershopping_banner"]?></div>		
	<iframe id="gabiaPageFrame" class="google hide"src="//firstmall.kr/ec_hosting/marketing/marketplace_url.php?firstmall=yes&shopSno=<?php echo $TPL_VAR["config_system"]["shopSno"]?>&domain=<?php echo $_SERVER["HTTP_HOST"]?>&type=<?php echo $TPL_VAR["config_system"]["service"]["code"]?>&renew=Y&isdemo=<?php echo $TPL_VAR["isdemo"]["isdemo"]?>" width="100%" height="160" frameborder="0"></iframe>
</div>
<!-- Firstmall 입점마케팅 배너 로드 :: 끝 -->
<ul class="tab_01 v2 tabEvent" style="display:none;">
	<li><a href="#naverpay" data-showcontent="naverpay">네이버 페이</a></li>
	<li><a href="#navershopping" data-showcontent="navershopping">네이버 쇼핑</a></li>
...
<div class="google hide">
	<div class="contents_dvs">
		<div class="title_dvs"><div class="item-title">구글 마케팅</div></div>			
...
<?php $this->print_("talkbuy_config",$TPL_SCP,1);?>

	</div>
</div>
<!-- 카카오페이 구매 : 끝 -->	

<div class="navershopping daum google facebook hide">	
	<div class="contents_dvs">	
		<div class="item-title">DB URL 공통 설정</div>		
		<table class="table_basic thl ">	
			<tr>
				<th>상품 DB URL 설정</th>
...
<div class="navershopping daum google facebook talkbuy hide">	
	<div class="box_style_05 mt15">
		<div class="title">안내</div>
		<ul class="bullet_hyphen">	
			<li>
				<span class="navershopping hide">네이버 업데이트 주기 : 01시 (일 1회)</span>
			...
			<li class="navershopping hide">DB URL 전달 시 상품 리뷰 수는 쇼핑몰의 리뷰만 전달됩니다. (네이버페이의 리뷰 수는 네이버 쇼핑에서 자체 집계)</li>
...
<!-- 네이버 페이 설정 :: 시작 -->
<div class="naverpay hide">
	<!-- 네이버 페이 설정 :: 시작 -->	
	<div class="contents_dvs">
		<div class="item-title">네이버 공통 인증 키</div>		
		<table class="table_basic thl">		
			<tr>
				<th>사용 여부</th>
				<td>					
			...
			<tr>
				...
				<td>
					<table id="checkoutWhitelist" class="table_basic v7 wauto">
						<colgroup>
							<col width="90%" />
							<col width="10%" />
						</colgroup>
...
<?php }else{?>
					...
					<a href="https://admin.checkout.naver.com/" class="resp_btn v2 size_XL" target="_blank">관리</a>		
<?php }?>	
				</td>
			</tr>
		</table>
...
<div id="shippingGroupInfo" class="hide">
	<div class="content">		
		<table class="table_row_basic v2 tdc">
			<colgroup>
<?php if(serviceLimit('H_AD')){?>
				<col width="21%" />
<?php }?>
