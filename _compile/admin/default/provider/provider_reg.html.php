<?php /* Template_ 2.2.6 2022/03/07 12:32:25 /www/suns_firstmall_kr/admin/skin/default/provider/provider_reg.html 000068845 */ 
...
<?php }?>
		...
		$(".manager_sms_send").live("click",function(event){
			...
			$.get('../member/sms_pop?certify_code='+certify_code, function(data) {
				$('#sendPopup').html(data);
				openDialog("SMS 발송 <span class='desc'></span>", "sendPopup", {"width":"750"});
			});
		});
		...
		$('.popupOpenBtn').on('click', function()
		{
			...
			switch (name) {
				...
				case "detailPageSetting" :
					title = "상세 페이지 설정";
					option = {"width":"1000","show" : "fade","hide" : "fade"};
					break;
...
<?php if($TPL_VAR["calcu_file"]){?>imgUploadEvent("#bankBtn", "", "/data/provider/", "<?php echo $TPL_VAR["calcu_file"]?>")<?php }?>	
...
<?php if($TPL_VAR["provider_id"]){?>	
			<button type="button" onclick="selladmin_login()" class="resp_btn size_L v2">입점사 로그인</button>			
<?php }?>
			<button type="button" onclick="provider_save();" class="resp_btn active size_L">저장</button>
		</div>

		<!-- 좌측 버튼 -->
		<div class="page-buttons-left">
			<button type="button" onclick="document.location.href='catalog';" class="resp_btn v3 size_L">리스트 바로가기</button>
		</div>
...
<?php if($TPL_VAR["provider_seq"]){?>
<div class="provider-statistic mt15">	
...
<?php }?>					
				...
				<dl class="change_password dl_list_01 w120">
					<dt>비밀번호</dt>
					<dd><input type="password" name="provider_passwd" class="line class_check_password_validation" /></dd>
				</dl>
...
<div class="contents_dvs">
	...
	<div class="resp_message">
		- 정산 주기 및 정산 마감은 <a href="../accountall/accountall_setting" target="_blank" class="resp_btn_txt">정산 마감일 설정</a>에서 변경할 수 있습니다.
	</div>
</div>
...
<div class="contents_dvs">
	...
	<table class="table_basic thl">		
		...
		<tr>
			...
			<td>
				<table id="cerfify_manager" class="table_basic v7 tdc wauto">
					<colgroup>
						<col width="30%" />					
						<col width="30%" />	
						<col width="13%" />	
...
<div class="contents_dvs">
	...
	<table class="table_basic">	
		...
		<tr>
			<td valign="top">
				<textarea name="admin_memo" style="width:100%; padding:10px 5px; height:120px; border:1px solid #d7d7d7; border-radius:3px;"><?php echo $TPL_VAR["admin_memo"]?></textarea>
			</td>
			<td valign="top">
				<div style="overflow:auto;height:120px;width:100%;border:0;padding: 10px 5px; border:1px solid #d7d7d7; border-radius:3px; box-sizing: border-box;"><?php echo $TPL_VAR["provider_log"]?></div>
			</td>
		</tr>	
	</table>
</div>
