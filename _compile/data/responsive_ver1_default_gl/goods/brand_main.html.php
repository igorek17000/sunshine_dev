<?php /* Template_ 2.2.6 2022/03/07 12:32:25 /www/suns_firstmall_kr/data/skin/responsive_ver1_default_gl/goods/brand_main.html 000007225 */  $this->include_("assignBrandMenuData","showDesignBanner");?>
<!-- ++++++++++++++++++++++++++++++++++++++++++++++++++++
@@ 브랜드 메인 @@
...
<?php if((preg_match('/[a-z]/',$TPL_K1)&&in_array('eng',$TPL_VAR["page_config"]["search_filter"]))||(preg_match('/[ㄱ-ㅎ]/',$TPL_K1)&&in_array('kor',$TPL_VAR["page_config"]["search_filter"]))){?>
				<li <?php if(!$TPL_V1[ 0]){?>class="hide"<?php }?>><a href="javascript:void(0)" class="alphabet_btn" data-target="wrap<?php echo str_replace("/","",strtr(base64_encode(str_replace("/","",$TPL_K1)),"+=","-_"))?>"><?php echo strtoupper($TPL_K1)?></a></li>
<?php }?>
<?php }}?>
			</ul>
		</div>
...
<?php if(is_array($TPL_R1=($TPL_VAR["ALL"]))&&!empty($TPL_R1)){foreach($TPL_R1 as $TPL_V1){?>
		<ul class="brandwrap all brand<?php echo $TPL_V1["category_code"]?>" >
			<li>
				<a class="brand_list_block" <?php if($TPL_V1["brand_image"]){?><?php }?> href="/goods/brand?code=<?php echo $TPL_V1["category_code"]?>">
					<img src="<?php echo $TPL_V1["brand_image"]?>" alt="<?php echo $TPL_V1["prn_title"]?>" onerror="this.src='/data/skin/responsive_ver1_default_gl/images/common/noimage.gif';" />
					<span class="name">
						<?php echo $TPL_V1["prn_title"]?>
...
<?php if(is_array($TPL_R2=$TPL_V1)&&!empty($TPL_R2)){foreach($TPL_R2 as $TPL_V2){?>
		<ul class="brandwrap brand<?php echo $TPL_V2["category_code"]?> wrap<?php echo str_replace("/","",strtr(base64_encode(str_replace("/","",$TPL_K1)),"+=","-_"))?>" style="display: none">
			<li>
				<a class="brand_list_block" <?php if($TPL_V2["brand_image"]){?><?php }?> href="/goods/brand?code=<?php echo $TPL_V2["category_code"]?>">
					<img src="<?php echo $TPL_V2["brand_image"]?>" alt="<?php echo $TPL_V2["prn_title"]?>" onerror="this.src='/data/skin/responsive_ver1_default_gl/images/common/noimage.gif';" />
					<span class="name">
