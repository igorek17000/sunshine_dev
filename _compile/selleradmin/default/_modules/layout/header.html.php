<?php /* Template_ 2.2.6 2022/03/07 12:32:25 /www/suns_firstmall_kr/selleradmin/skin/default/_modules/layout/header.html 000024397 */ 
...
<?php if($TPL_VAR["admin_skin_user_type"]=='existing'){?>
					<!-- [ 관리자 전환 버튼 : 시작 ] -->
					<img src="/selleradmin/skin/default/images/common/<?php if($TPL_VAR["admin_skin_type"]=='new'){?>org<?php }else{?>new<?php }?>_ver_change_btn.png" data-admin-skin-type='<?php echo $TPL_VAR["admin_skin_type"]?>' class='ver_change_btn' style="display: none;">
					<!-- [ 관리자 전환 버튼 : 시작 ] -->					
<?php }?>
...
<?php }else{?>
						<li class="item hsnb-admin">
							...
							<div class="hsnbm-menu hide">
								<span class="point_c"></span>
								<ul class="tb_admin_info">
									<li class="admin_thumb">
										<img src="/selleradmin/skin/default/images/main/sell_img.png" width="50" height="50" align="absmiddle" />
									</li>
									<li class="admin_info">
										<p><?php echo $TPL_VAR["providerInfo"]["provider_id"]?></p>
										<p>(<?php echo $TPL_VAR["providerInfo"]["provider_name"]?>)</p>
										<p style="color:#348ddb;"><?php if($TPL_VAR["providerInfo"]["pgroup_name"]){?><?php echo $TPL_VAR["providerInfo"]["pgroup_name"]?><?php }else{?>입점사<?php }?></p>
									</td>
									<li class="admin_link">
<?php if($TPL_VAR["providerInfo"]["manager_yn"]=='Y'){?>
										<a href="../setting/provider_reg?no=<?php echo $TPL_VAR["providerInfo"]["provider_seq"]?>">정보수정</a>
<?php }else{?>
										<a href="../setting/manager_reg?provider_seq=<?php echo $TPL_VAR["providerInfo"]["sub_provider_seq"]?>">정보수정</a>
<?php }?>
...
<?php }else{?>	
<?php if(!$TPL_VAR["providerInfo"]["provider_seq"]){?>
				<div id="header-gnb-cover" style="top:0px;position:absolute;width:100%;border:1px solid #000000;height:72px;z-index:100000;background-color:#ffffff;"></div>
				<script>$("#header-gnb-cover").css('opacity',0);</script>
<?php }?>
				<li class="fl">
					<!-- 좌측 GNB -->
					<table class="header-gnb" cellpadding="0" cellspacing="0">
						<tr>
							<td class="mitem-td <?php if($TPL_VAR["adminMenuCurrent"]=='main'){?>current<?php }?>">
								<span class="mitem"><a href="../main/index">홈</a><span>
							</td>
<?php if($TPL_adminMenu_1){foreach($TPL_VAR["adminMenu"] as $TPL_K1=>$TPL_V1){?>
							<td class="mitem-td <?php if(in_array($TPL_VAR["adminMenuCurrent"],$TPL_V1["folders"])){?>current<?php }?>" code="<?php echo $TPL_K1?>">								
								<span class="mitem"><a href="<?php echo $TPL_V1["url"]?>"><span><?php echo $TPL_V1["name"]?></span><div class="header-gnb-issueCount-layer" code="<?php echo $TPL_K1?>"></div></a></span>								
...
<?php if($TPL_V2["limit"]&&serviceLimit($TPL_V2["limit"],'return')){?>
													<a href="#" onclick="<?php echo serviceLimit($TPL_V2["limit"])?>"><?php echo $TPL_V2["name"]?></a>
<?php }else{?>
...
<?php if(is_array($TPL_R3=$TPL_V2["childs"])&&!empty($TPL_R3)){foreach($TPL_R3 as $TPL_V3){?>							
							<div <?php if(in_array($TPL_VAR["adminSubMenuCurrent"],$TPL_V3["file"])){?>class="current"<?php }?>>					
...
<?php }?>
		...
		<div class="contentsWarp <?php if($TPL_VAR["adminMenuCurrent"]=='main'||$TPL_VAR["adminMenuCurrent"]=='accountall'||$TPL_VAR["adminMenuCurrent"]=='setting'||$TPL_VAR["adminMenuCurrent"]=='board'){?>close<?php }?>">
		<!--[ 레이아웃 바디(본문) : 시작 ]-->