<?php /* Template_ 2.2.6 2022/03/07 12:32:25 /www/suns_firstmall_kr/selleradmin/skin/default/goods/_option_for_regist.html 000047235 */ 
...
<?php if($TPL_VAR["adminSessionType"]=='provider'||$TPL_VAR["provider_seq"]=='1'){?>
			$("input[name='commissionRate[]']").on('keydown change focusin selectstart',function(){
				$(this).blur();
				return false;
			});
<?php }?>
		...
		$("input[name='commissionRate[]']").on("change",function(){
...
<tr <?php if($TPL_VAR["socialcpuse"]){?>class="hide"<?php }?>>
	...
	<td>
		<div class="resp_radio">
			<label><input type="radio" name="optionUse" value="1" <?php if($TPL_VAR["goods"]["option_use"]=='1'||$TPL_VAR["socialcpuse"]){?>checked<?php }?> /> 사용</label>
			<label class="ml10"><input type="radio" name="optionUse" value="" <?php if($TPL_VAR["goods"]["option_use"]!='1'&&!$TPL_VAR["socialcpuse"]){?>checked<?php }?> /> 사용 안 함</label>
		</div>
				</td>
</tr>
...
<?php if($TPL_VAR["socialcpuse"]){?>
						<th style="min-width:50px" class="couponinputtitle"><span class="couponinputsubtitle"><?php if($TPL_VAR["goods"]["socialcp_input_type"]=='price'){?>금액<?php }else{?>횟수<?php }?></span></th>
<?php }else{?>
...
<?php if(is_array($TPL_R1=range( 1,$TPL_VAR["package_count"]))&&!empty($TPL_R1)){foreach($TPL_R1 as $TPL_V1){?>
					<th style="min-width:40px">상품<?php echo $TPL_V1?></th>
<?php }}?>
<?php }else{?>
...
<?php if($TPL_VAR["package_yn"]!='y'&&$TPL_VAR["isplusfreenot"]){?>
		<td class="center">
...
<?php }?>
					...
					<div class="reg_package_option reg_package_option<?php echo $TPL_V2?>"><?php echo $TPL_V1["package_option"][$TPL_V2]?></div>
...
<?php }?>
<?php }?>
			<td class="right pdr10 settlementAmount">0</td>
			<td class="right pdr10">
...
<?php }?>
		<!-- (공통) -->
		<th class="not_for_seller">정산 금액</th>
		<th class="not_for_seller">
...
<?php if(is_array($TPL_R1=range( 1,$TPL_VAR["package_count"]))&&!empty($TPL_R1)){foreach($TPL_R1 as $TPL_V1){?>
						<td class="reg_package_option_tbl">
<?php if($TPL_V1== 1){?><input type="hidden" name="stock[]" value="<?php if($TPL_VAR["options"][ 0]["stock"]){?><?php echo $TPL_VAR["options"][ 0]["stock"]?><?php }else{?>0<?php }?>" /><?php }?>
...
<?php }else{?>
			<td class="center _stock">
				<input type="text" name="stock[]" value="<?php if($TPL_VAR["options"][ 0]["stock"]){?><?php echo $TPL_VAR["options"][ 0]["stock"]?><?php }else{?>0<?php }?>" size="5" class="onlynumber right"/>
			</td>
<?php }?>
...
<?php }else{?>
			<td class="center _stock">
				<input type="text" name="badstock[]" value="<?php if($TPL_VAR["options"][ 0]["badstock"]){?><?php echo $TPL_VAR["options"][ 0]["badstock"]?><?php }else{?>0<?php }?>" size="5" class="onlynumber right" />
			</td>
<?php }?>
			<td class="right pdr10 _stock">
<?php if($TPL_VAR["cfg_order"]["ableStockStep"]== 15){?>
...
<?php }?>
			...
			<td class="center _stock">
				<input type="text" name="safe_stock[]" value="<?php if($TPL_VAR["options"][ 0]["safe_stock"]){?><?php echo $TPL_VAR["options"][ 0]["safe_stock"]?><?php }else{?>0<?php }?>" size="5" class="onlynumber right"/>
			</td>
<?php }?>
			<td class="settlementAmount right not_for_seller pdr10">0</td>
			<td class="not_for_seller right pdr10">
<?php if(!$TPL_VAR["goods"]["goods_seq"]){?>
				<?php echo $TPL_VAR["provider_charge"][ 0]["charge"]?>

				<input class="right onlyfloat input-box-default-text" name="commissionRate[]" value="<?php echo $TPL_VAR["provider_charge"][ 0]["charge"]?>" size="3" type="hidden">
<?php }else{?>
<?php if($TPL_VAR["options"][ 0]["commission_rate"]){?><?php echo $TPL_VAR["options"][ 0]["commission_rate"]?><?php }else{?>0<?php }?>
				<input class="right onlyfloat input-box-default-text bg-gray" name="commissionRate[]" value="<?php if($TPL_VAR["options"][ 0]["commission_rate"]){?><?php echo $TPL_VAR["options"][ 0]["commission_rate"]?><?php }else{?>0<?php }?>" size="3" type="hidden" readonly>
<?php }?>
<?php if($TPL_VAR["provider_charge"][ 0]["commission_type"]=='SACO'&&$TPL_VAR["provider_charge"][ 0]["commission_type"]!=''){?>
				<span class="commission_type SACO_unit">%<input type="hidden" name="commissionType[]" value="SACO" /></span>
<?php }else{?>
				<span class="commission_type SUPPLY_unit">
<?php if(($TPL_VAR["goods"]["goods_seq"]&&$TPL_VAR["options"][ 0]["commission_type"]!='SUPR')||(!$TPL_VAR["goods"]["goods_seq"]&&$TPL_VAR["provider_charge"][ 0]["commission_type"]!='SUPR')){?>
						%
<?php }else{?>
						<?php echo $TPL_VAR["basic_currency"]?>

<?php }?>
					<input type="hidden" name="commissionType[]" value="<?php echo $TPL_VAR["provider_charge"][ 0]["commission_type"]?>" />
				</span>
<?php }?>
			</td>
...
<?php if($TPL_VAR["options"][ 0]["reserve_unit"]=='percent'){?>
				<?php echo number_format($TPL_VAR["options"][ 0]["reserve_rate"])?>% (<?php echo get_currency_price($TPL_VAR["options"][ 0]["reserve"], 2)?>)
<?php }else{?>
				<?php echo get_currency_price($TPL_VAR["options"][ 0]["reserve"], 2)?>
