<?php /* Template_ 2.2.6 2022/03/07 12:32:25 /www/suns_firstmall_kr/selleradmin/skin/default/goods/_regist_popup_guide.html 000055982 */ 
...
<?php if(!$TPL_VAR["socialcpuse"]){?>
			<td class="center"><input class="line onlyfloat input-box-default-text right" name="weight[]" value="0" size="4" type="text"></td>
<?php }?>
<?php }?>
...
<?php }?>
		...
		<td class="center pricetd"><input type="text" name="consumerPrice[]" value="0" class="onlyfloat right black wp80" /></td>
		<td class="center pricetd"><input type="text" name="price[]" value="0" class="onlyfloat right wp80" /></td>
...
<?php if($TPL_VAR["sellermode"]!='SELLER'){?>
			...
			<select name="reserveUnit[]" class="wx40">
...
<div id="view_editor_div" class="hide">
	<div class="content">
		...
		<table class="table_basic" style="height:300px">
			...
			<tbody>
				...
				<tr>
					...
					<td>
						...
						<div class="resp_checkbox">
							<label><input type="checkbox" name="possible_pay[]" value="account" <?php if(!$TPL_VAR["payment"]["account"]){?>disabled<?php }?> <?php if($TPL_VAR["payment_check"]["account"]){?>checked<?php }?>> 계좌 이체</label>
							<label><input type="checkbox" name="possible_pay[]" value="escrow_account" <?php if(!$TPL_VAR["escrow"]["account"]){?>disabled<?php }?> <?php if($TPL_VAR["escrow_check"]["account"]){?>checked<?php }?>> 계좌 이체 에스크로</label><br />
						</div>
						<div class="resp_checkbox">
							<label><input type="checkbox" name="possible_pay[]" value="virtual" <?php if(!$TPL_VAR["payment"]["virtual"]){?>disabled<?php }?> <?php if($TPL_VAR["payment_check"]["virtual"]){?>checked<?php }?>> 가상 계좌</label>
							<label><input type="checkbox" name="possible_pay[]" value="escrow_virtual" <?php if(!$TPL_VAR["escrow"]["virtual"]){?>disabled<?php }?> <?php if($TPL_VAR["escrow_check"]["virtual"]){?>checked<?php }?>> 가상 계좌 에스크로</label>
						</div>
					</td>
				</tr>
				<tr>
					...
					<td>
						<div class="resp_checkbox">
							<label><input type="checkbox" name="possible_pay[]" value="kakaopay" <?php if($TPL_VAR["config_system"]["not_use_kakao"]=='y'){?>disabled<?php }?> <?php if($TPL_VAR["payment_check"]["kakaopay"]){?>checked<?php }?>> 카카오 페이</label>
...
<div id="eventDetailViewLay" class="eventView hide">
	<div class="content">
	<span class="item-title">할인 이벤트</span>
	<table class="table_basic">
		<colgroup>
			<col width="15%" />
			<col />
...
<?php }?>
	...
	<span class="item-title mt20">모바일 할인</span>
	<table class="table_basic">
		<colgroup>
			<col />
			<col width="16%" />
...
<tr>
	<td class="center">
		...
		<span>
		<input type="text" name="iconStartDate[]" value="" maxlength="10" size="10" />~
		<input type="text" name="iconEndDate[]" value="" maxlength="10" size="10" />
		</span>
	</td>
	<td class="center">
		...
		<thead>
			...
			<tr>
				<td class="left pd5">
					...
					<span class="btn small black"><button type="button" class="hscode_all_btn" target="hscode_type" target_name="류">▼</button></span>
				</td>
				<td class="left pd5">
					<input type="text" id="hscode_unit_all" value="<?php echo $TPL_VAR["goodsHscodeInfo"]["hscodeUnitCode"]?>" style="width:43px"/>
					<span class="btn small black"><button type="button" class="hscode_all_btn" target="hscode_unit" target_name="호">▼</button></span>
				</td>
				<td class="left pd5">
					<input type="text" id="hscode_num_all" class="" style="width:90px"/>
					<span class="btn small black"><button type="button" class="hscode_all_btn" target="hscode_num" target_name="번호">▼</button></span>
				</td>
				<td class="left pd5">
					<input type="text" id="hscode_tax_all" class="onlyfloat" style="width:53px"/>
					<span class="btn small black"><button type="button" class="hscode_all_btn" target="hscode_tax"  target_name="세율">▼</button></span>
				</td>
			</tr>
		</thead>
	</table>
...
<?php if($TPL_nationsInfo_1){foreach($TPL_VAR["nationsInfo"] as $TPL_K1=>$TPL_V1){?>
				<tr>
					...
					<th class="its-th pd5">
						<img src="/selleradmin/skin/default/images/common/icon/nation/<?php echo $TPL_V1["nationCode"]?>.png" style="vertical-align:middle"/>
						...
						<input type="hidden" name="nation_flag[<?php echo $TPL_K1?>]" value="/selleradmin/skin/default/images/common/icon/nation/<?php echo $TPL_V1["nationCode"]?>.png" <?php if(!$TPL_VAR["goodsHscodeInfo"]["nationList"][$TPL_K1]){?>disabled<?php }?>/>
