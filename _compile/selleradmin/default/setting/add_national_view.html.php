<?php /* Template_ 2.2.6 2022/03/07 12:32:25 /www/suns_firstmall_kr/selleradmin/skin/default/setting/add_national_view.html 000026205 */ 
...
<?php if($TPL_VAR["mode"]=='modify'){?>
	location.href="../setting_process/add_shipping_item?mode=modify&grp_seq=<?php echo $TPL_VAR["grp_seq"]?>&num=<?php echo $TPL_VAR["num"]?>";
<?php }else{?>
	parent.window.close();
<?php }?>
});
...
<div class="item_view">
	...
	<tr class="item_tr ready_item_tr">
		<td class="center nonpd under-line" width="150px">
			...
			<input type="hidden" name="talkbuy_order_possible" value="<?php echo $TPL_VAR["ship_set"]["talkbuy_order_possible"]?>" />
			<input type="hidden" name="talkbuy_order_impossible_msg" value="<?php echo $TPL_VAR["ship_set"]["talkbuy_order_impossible_msg"]?>" />
...
<?php if($TPL_VAR["ship_set"]["shipping_set_seq"]){?>
				<input type="button" value="배송안내" onclick="ship_desc_pop('<?php echo $TPL_VAR["ship_set"]["shipping_set_seq"]?>')" class="resp_btn"/>
<?php }?>
...
<?php if($TPL_shipping_type_1){$TPL_I1=-1;foreach($TPL_VAR["shipping_type"] as $TPL_K1=>$TPL_V1){$TPL_I1++;?>
					<tr>
						<td class="center nonpd " width="85px" style="border-left:none;<?php if(count($TPL_VAR["shipping_type"])==($TPL_I1+ 1)){?>border-bottom:none;<?php }?>"><?php echo $TPL_V1?></td>
						<td class="left" <?php if(count($TPL_VAR["shipping_type"])==($TPL_I1+ 1)){?>style="border-bottom:none;"<?php }?>>
...
<?php if(is_array($TPL_R3=$TPL_VAR["ship_zone"][$TPL_K1]['sel_address_street'][$TPL_K2])&&!empty($TPL_R3)){$TPL_I3=-1;foreach($TPL_R3 as $TPL_V3){$TPL_I3++;?>
											<tr>
												<td class="nonpd center"><?php echo (count($TPL_VAR["ship_zone"][$TPL_K1]['sel_address_street'][$TPL_K2])-$TPL_I3)?></th>
												<td>
...
<?php if(is_array($TPL_R3=$TPL_VAR["ship_cost"][$TPL_K1][$TPL_I2]['shipping_cost'])&&!empty($TPL_R3)){foreach($TPL_R3 as $TPL_K3=>$TPL_V3){?>
									<td class="right pdr5" width="105px">
										...
										<?php echo $TPL_V3?> <?php echo $TPL_VAR["basic_currency"]?>
...
<?php if($TPL_VAR["ship_cost"][$TPL_K1][$TPL_I2]['shipping_today_yn'][$TPL_K3]=='Y'){?>
										...
										<br/>당일 <?php echo $TPL_VAR["ship_cost"][$TPL_K1][$TPL_I2]['shipping_cost_today'][$TPL_K3]?> <?php echo $TPL_VAR["basic_currency"]?>
				...
				<td class="center nonpd clear">
					...
					<tr>
						...
						<td class="left">
							...
							편도 : <?php echo $TPL_VAR["ship_set"]["refund_shiping_cost"]?> <?php echo $TPL_VAR["basic_currency"]?>

<?php if($TPL_VAR["ship_set"]["shiping_free_yn"]=='Y'){?>
							(배송비가 무료인 경우, 왕복 <?php echo ($TPL_VAR["ship_set"]["refund_shiping_cost"]* 2)?> <?php echo $TPL_VAR["basic_currency"]?> 받음)
<?php }?>
						</td>
					</tr>
					<tr>
						...
						<td class="left">
							...
							왕복 : <?php echo $TPL_VAR["ship_set"]["swap_shiping_cost"]?> <?php echo $TPL_VAR["basic_currency"]?>
		...
		<td class="center under-line controll_td" width="80px" <?php if($TPL_VAR["default_yn"]=='Y'){?>style="background-color:#FFE3BB;"<?php }?>>
