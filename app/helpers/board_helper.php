...
function getGoodsinfo($data, $goods_seq_data ,$type = 'list' )
{
	...
	foreach($goods_seq_array as $goods_seq){
		...
		$goods['opt_consumerprice']		= ($options[0]['consumer_price']);
		$goods['opt_price']				= ($options[0]['price']);
		...
		if(isset($goods['goods_seq'])) {
			...
			if($type == 'write') {//수정
				...
				if ($goods['string_price_use']) {
					...
					$goodslistprice  = ((int)$goods['opt_consumerprice']>(int)$goods['opt_price'])?'<div class="name"><a href="'.$CI->boardurl->goodsview.$goods['goods_seq'].'" target="_blank"><span style="color: #7f7777; font-weight: normal; text-decoration: line-through;">'.get_currency_price($goods['opt_consumerprice'],2).'</span> → <span style="color: #cc0000;">'.get_currency_price($goods['opt_price'],2).'</span> </a></div>':'<div class="name"><a href="'.$CI->boardurl->goodsview.$goods['goods_seq'].'" target="_blank"><span style="color: #cc0000;">'.get_currency_price($goods['opt_price'],2).'</span> </a></div>';
				}
...
function getBulkorderGoodsinfo($data, $goods_seq_data ,$type = 'list' )
{
	...
	foreach($goods_seq_array as $goods_seq){
		$goods = $CI->goodsmodel->get_goods($goods_seq);
		$options = $CI->goodsmodel->get_goods_option($goods_seq);
		$goods['opt_consumerprice']		= get_currency_price($options[0]['consumer_price']);
		$goods['opt_price']				= get_currency_price($options[0]['price']);
		...
		if(isset($goods['goods_seq'])) {
			...
			if($type == 'write') {//수정
				...
				if(!empty($goods['string_price'])){
					...
					$goodslistprice  = ((int)$goods['opt_consumerprice']>(int)$goods['opt_price'])?'<div class="name"><a href="'.$CI->boardurl->goodsview.$goods['goods_seq'].'" target="_blank"><span style="color: #7f7777; font-weight: normal; text-decoration: line-through;">'.get_currency_price($goods['opt_consumerprice'],3).'</span> → <span style="color: #cc0000;">'.get_currency_price($goods['opt_price'],3).'</span> </a></div>':'<div class="name"><a href="'.$CI->boardurl->goodsview.$goods['goods_seq'].'" target="_blank"><span style="color: #cc0000;">'.get_currency_price($goods['opt_price'],3).'</span> </a></div>';
				}
...
function boardCaptchValidation($board='board') {
	$CI =& get_instance();
