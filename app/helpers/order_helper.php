...
function get_goods_coupon_view($export_code){
	...
	if( $socialcp_cancel_refund_day[0] == 'payoption' ){
		...
		if( $socialcp_cancel_refund_day[4] == 'social_date' ) {//유효기간내 취소시
			$params['socialcp_cancel_refund_day_status'] = "유효기간 ".$item['social_start_date']."~".$item['social_end_date']." ".$socialcp_cancel_refund_day[3]."%";
		}else{
			$params['socialcp_cancel_refund_day_status'] = "유효기간 시작".$socialcp_cancel_refund_day[4]."일 전( ".date("Y년 m월 d일 ", strtotime($socialcp_cancel_refund_day[2]))."까지) ".$socialcp_cancel_refund_day[3]."%";
		}
