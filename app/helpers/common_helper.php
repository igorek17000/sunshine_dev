...
function firstmall_typereceipt($order_seq, $seq = null){
	...
	###
	if($seq){
		$sql = "SELECT * FROM fm_sales WHERE seq = '{$seq}' AND tstep = 1 ";
	}else{
		// 2016.05.19 세금계산서 자동연동 기능 추가 pjw
		$sql = "SELECT * FROM fm_sales WHERE order_seq = '{$order_seq}' AND tstep = 1";
	}
	$query			= $CI->db->query($sql);
...
function get_currency_info($currency=''){
	...
	function get_base_config_system(){
		...
		# 기본통화설정 @2016-05-31 pjm
		$get_params = array('shopSno'=>$CI->config_system['shopSno']	);
		$CI->load->model('adminenvmodel');
		$query		= $CI->adminenvmodel->get($get_params,0,1);
		$row		= $query->row_array();
        ...
        $CI->config_currency_all = $currency_list_tmp;  // 모든 통화 정보 정의
		...
		foreach($env_data as $v){
			...
			foreach($admin_currency as $admin_list => $c_list) {
...
function getAlert($code = null,$args = null){
	...
	$CI		=& get_instance();
	$lang	= 'KR';
	if($CI->config_system['language']) $lang	= $CI->config_system['language'];
	...
	if(!$CI->aAertMsg[$code]) {
		...
		$sql					= "select SQL_CACHE * from fm_alert where code = ? limit 1";
		$query					= $CI->db->query($sql,$code);
	...
	if	(strpos($ret,'%s') >  -1) {
		...
		if	($str_len > $arr_len) {
			...
			$len			= $arr_len-$str_len;
			for	( $i=0; $i<$len; $i++ ) unset($args[($arr_len--)-1]);
		}
...
function getAlert_for_text($text_lang = null,$text = null,$args = null){
	...
	$sql					= "select SQL_CACHE * from fm_alert where ".$text_lang." = ? limit 1";
	$query					= $CI->db->query($sql,$text);
	$rows					= $query->row_array();
...
/** 
 * 관리자UI 2차 개선 - 기존/신규설치 구분
 */
function getAdminSkinUserType() 
{
	$CI =& get_instance();
	$adminSkinPatchDate = config_load('adminSkinType', "defaultDate");
	// 신규설치 사용자 :: 솔루션 설치일과 기능 배포일이 같으면 신규설치
	if ($CI->config_system['service']['setting_date'] >= substr($adminSkinPatchDate['defaultDate'], 0, 10) )
	{
		return 'newSetting';
	} else {
	// 기존사용자 :: 설정값대로 노출. 설정값 없으면 기존 타입 노출
		return 'existing';
	}
}
