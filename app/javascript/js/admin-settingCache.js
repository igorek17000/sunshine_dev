...
function chgMainCacheErr(){    
    openDialogAlert("<a href='../setting/snsconf' class='highlight-link'>SNS/외부 연동</a> 메뉴에서 먼저 페이스북 쇼핑몰 설정을 하십시오.", 450, 150);
}
...
function allGoodsDisplayCache(){
    ...
    $.ajax({
        ...
        'success' : function(result){
            ...
            openDialogAlert(msg, 400, 280, function(){}, callback);
        }
    });
}
    ...
    $.ajax({
        ...
        success    : function(result){
            if(result.code == '10' && chgStatus == 'y'){
                openDialogAlert('<div class=\'left\'>해당 상품디스플레이의 캐시파일이 사용으로 설정되었습니다.<br>앞으로 해당 상품디스플레이의 노출되는 상품 및 정보가 변경되었을 경우<br>최신의 캐시파일로 생성(수동/자동)해 주십시오.</div>', 450, 280);
