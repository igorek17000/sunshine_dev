...
var label_item_radio = 0;
var goodsreviewicon = '/data/icon/goods_review/';
$(function() {
	var layer_seq = 0;
	...
	if(!$('body').data('listJoinBtnEventBinded')){
		$("body").on("click",".listJoinBtn",function(){
			...
			var labelIcon	=	$('input[name="labelItem[user]['+seq+'][icon]"]').val().split("|");
			...
			$("input[name=windowLabelIcon]").val(labelIcon);
			...
			html = labelAddItem(labelType, labelValue, labelsubValue, labelIcon);
			...
			case "radio" :
				$("input[name='windowLabelValue[]']").each(function(i){
					if(!$("input[name='windowLabelValue[]']").eq(i).val()){
						...
						labelIcon += $("input[name='labelIcon[]']").eq(i).val()+"|";
					}
				});
			break;
			...
			return $('<tr>')
				...
				.append($('<td>')
					...
					.append($('<button>')
						...
						.prop('id', Firstmall.Config.Environment.serviceLimit.H_FR?null:'listJoinBtn')
...
function resetLabel()
{
	...
	if(operation_type == 'light'){
		$("[name=windowLabelType]").val('radio');
		$("[name=windowLabelType]").closest('tr').hide();
	}
...
{
	openDialog("평가정보 추가","joinDiv",{"width":450,"height":300});
}
	...
	switch(type)
	{
		...
		case "radio" :
			...
			if (right) {
				html += '<tr class="labelAdd"><th align="left" style="width:65px;"></th>';
				html += '<td>';
				html += '<input type="hidden" name="labelIcon[]" value="'+labelIcon[i]+'" /><img src="'+goodsreviewicon+'/emotion_happy.png" border="0" class="goodsReviewIcon hand" align="absmiddle" iconindex="'+label_item_radio+'" ><span class="btn small"><input type="button" value="선택" class="goodsReviewIcon"  iconindex="'+label_item_radio+'" /></span><input type="text" name="windowLabelRight[]" value=""  size="30"> <button type="button" value="-" class="btn_minus" onclick="deleteRow(this)"></button>';
				html += '</td>';
				html += '</tr>';
			}
			else {
				html += '<input type="hidden" name="labelIcon[]" value="" /><img src="'+goodsreviewicon+'/emotion_happy.png" border="0" class="goodsReviewIcon hand" align="absmiddle"  iconindex="'+label_item_radio+'" ><span class="btn small"><input type="button" value="선택" class="goodsReviewIcon"  iconindex="'+label_item_radio+'" /></span> <label class="resp_radio"><input type="radio" name="labelKey[]" class="null" disabled></label> <input type="text" name="windowLabelValue[]" size="30"> <button type="button" value="-" class="btn_minus" onclick="deleteRow(this)"></button>';
			}
			html += '</td>';
			html += '</tr>';
			label_item_radio++;
...
function labelSubAdd(parent_label_item_seq){
	...
	var html = '<tr class="labelAdd"><th></th><td>';
	html += '→ <input type="text" name="windowSubLabelValue['+parent_label_item_seq+'][]"> <button type="button" value="-" class="pointer" onclick="deleteRow(this)"><img src="/admin/skin/default/images/common/icon_minus.gif" height="18px" align="middle"></button>';
	html += '</td></tr>';
...
function labelAddItem(labelType, labelValue, labelsubValue)
{
	...
	switch(labelType)
	{
		...
		case "radio" :
			if (labelValue)
			{
				if(!labelIcon[0]) labelIcon[0] = 'emotion_happy.png';
				if(!labelIcon[1]) labelIcon[1] = 'emotion_happy.png';
				html = '<input type="hidden" name="labelIcon[]" value="'+labelIcon[0]+'" /><img src="'+goodsreviewicon+labelIcon[0]+'" border="0" class="goodsReviewIcon hand" align="absmiddle" iconindex="0" ><span class="btn small"><input type="button" value="선택" class="goodsReviewIcon" iconindex="0" /></span> <label class="resp_radio"><input type="radio" name="labelKey[]" class="null" disabled></label> <input type="text" name="windowLabelValue[]" value="'+ labelValue[0] +'" size="30"><br/><input type="hidden" name="labelIcon[]" value="'+labelIcon[1]+'" /><img src="'+goodsreviewicon+labelIcon[1]+'" border="0" class="goodsReviewIcon hand" align="absmiddle" iconindex="1" ><span class="btn small"><input type="button" value="선택" class="goodsReviewIcon" iconindex="1" /></span> <label class="resp_radio"><input type="radio" name="labelKey[]" class="null" disabled></label> <input type="text" name="windowLabelValue[]" value="'+ labelValue[1] +'" size="30"> <button type="button" class="btn_plus" onclick="labelAdd(\'radio\')"></button>';
				label_item_radio=2;
				...
				if (count > 2)
				{
					for (i=2; i<count; i++)
					{
						if(!labelIcon[i]) labelIcon[i] = 'emotion_happy.png';
						labelTableHtml += '<tr class="labelAdd"><td>&nbsp;</td>';
						labelTableHtml += '<td>';
						labelTableHtml += '<input type="hidden" name="labelIcon[]" value="'+labelIcon[i]+'" /><img src="'+goodsreviewicon+''+labelIcon[i]+'" border="0" class="goodsReviewIcon hand" align="absmiddle" iconindex="'+i+'" ><span class="btn small"><input type="button" value="선택" class="goodsReviewIcon"  iconindex="'+i+'" /></span> <label class="resp_radio"><input type="radio" name="labelKey[]" class="null" disabled></label> <input type="text" name="windowLabelValue[]" value="'+ labelValue[i] +'" size="30"> <button type="button" class="btn_minus" onclick="deleteRow(this)"></button>';
						labelTableHtml += '</td>';
						labelTableHtml += '</tr>';
					}
				}
				...
				html = '<input type="hidden" name="labelIcon[]" value="" /><img src="'+goodsreviewicon+'/emotion_happy.png" border="0" class="goodsReviewIcon hand" align="absmiddle" iconindex="0" ><span class="btn small"><input type="button" value="선택" class="goodsReviewIcon" iconindex="0" /></span> <label class="resp_radio"><input type="radio" name="labelKey[]" class="null" disabled></label> <input type="text" name="windowLabelValue[]" value="" size="30"><br/><input type="hidden" name="labelIcon[]" value="" /><img src="'+goodsreviewicon+'/emotion_happy.png" border="0" class="goodsReviewIcon hand" align="absmiddle" iconindex="1" ><span class="btn small"><input type="button" value="선택" class="goodsReviewIcon" iconindex="1" /></span> <label class="resp_radio"><input type="radio" name="labelKey[]" class="null" disabled></label> <input type="text" name="windowLabelValue[]" value="" size="30"> <button type="button" class="btn_plus" onclick="labelAdd(\'radio\')"></button>';
				label_item_radio=2;
			}
