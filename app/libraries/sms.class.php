...
DOC_XML;
	...
	function sms_send($phone, $callback, $msg, $refkey="", $reserve = "0", $ordno="", $sendType=''){
		...
		if(is_array($msg)){
			...
			$checkmsg	= $msg;
			$msg		= preg_replace($patterns, $replacements, urldecode($msg));
			$sendType = $this->sms_msg_type($msg, $phone);

			if($sendType == "lms"){
				$smsData['lms']["phone"][0] = $phone;
				$smsData['lms']["msg"][0] = $msg;
			}elseif($sendType == "gms"){
				$smsData['gms']["phone"][0] = $phone;
				$smsData['gms']["msg"][0] = $msg;
			}else{
				$smsData['sms']["phone"][0] = $phone;
				$smsData['sms']["msg"][0] = $msg;
			}
						...
						if ($talkYN != 'Y'){
							...
							if(is_array($to_sms)){
								...
								for($i=0; $i<$to_sms_count; $i++){
									...
									if( in_array($case,array('coupon_released','coupon_released2')) ) $sms_flag = true;
