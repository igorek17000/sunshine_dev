...
class AssetManager {
    ...
    protected static function get_cache_key(int $cache_key_type) {
        if(empty(static::$__cache_key_cache[$cache_key_type])) {
            if(ENVIRONMENT === 'development') static::$__cache_key_cache[$cache_key_type] = static::get_cache_key_random();
            else
            {
                if($cache_key_type & CACHE_KEY_USE_COMMIT_ID) static::$__cache_key_cache[$cache_key_type] = static::get_cache_key_commit_id();
                elseif($cache_key_type & CACHE_KEY_USE_RANDOM) static::$__cache_key_cache[$cache_key_type] = static::get_cache_key_random();
            }
        }
