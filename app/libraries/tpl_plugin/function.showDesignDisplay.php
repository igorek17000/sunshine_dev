...
function _designDisplayTab($aParams)
{
    ...
    if( !$iscach && $display_tab['cache_use'] == 'y' && !$CI->userInfo['member_seq'] && !$CI->designMode && !$CI->managerInfo ){
        $sFileName   = $CI->goodsdisplay->checkDesignDisplayCach($display_tab['display_seq'], $iTabIndex, $perpage, $display['kind']);
        if( $sFileName && !$iscach){
            $tabList[$iTabIndex]    = unserialize(file_get_contents($sFileName));
            /*
             * 반응형 스킨-캐시를 적용하면 상품디스플레이가 노출되지 않는 오류 수정 
             */
            $list = array(
                'record' => $tabList[$iTabIndex]['record'],
                'page' => array('querystring' => get_args_list()),
            );
        }
    }
