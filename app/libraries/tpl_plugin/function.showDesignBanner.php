...
function showDesignBanner($banner_seq,$return=false)
{
	...
	if($banner['platform'] == 'responsive'){
		...
		$cssSlider = 'sliderA';
		if($banner['style'] == 'light_style_2')		$cssSlider = 'sliderB';
		if($return) $html .= '<div class="custom_slider ' . $cssSlider . '">';
		...
		if($return){
			...
			/*
			$imgHTML	= '<div style="width:100%;position:relative;opacity: 0;"><img src="/data/skin/' . $skin . '/' . $banner_item[0]['image'] . '" width="' . $banner['image_width'] . '" height="' . $banner['image_height'] . '" style="position:absolute;top:0;left:0;" /></div>';
			$html .= "<div class='designBanner' designElement='banner' templatePath='{$template_path}' bannerSeq='{$banner_seq}' style='height:{$banner['image_height']}px;'>" . $imgHTML . "</div>";
			*/
		}
