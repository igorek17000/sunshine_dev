...
/* 상품디스플레이 출력*/
function showCategoryRecommendDisplay($category_code)
{
	$CI =& get_instance();
	...
	if($operation_type == 'light'){
		_lightShowCategoryRecommendDisplay($category_code);
	}else{
		_heavyShowCategoryRecommendDisplay($category_code);
	}

	return;
}

function _lightShowCategoryRecommendDisplay($category_code){
	$CI =& get_instance();
	...
	$display_seq_arr	= $CI->categorymodel->get_category_recommend_display_seq($category_code);
	...
	if($display_seq){
		...
		if($display_data['style'] == 'sizeswipe') $display_tabs = array($display_tabs[0]);

		############ 상품 평가정보 ############
		// 제일 첫 행의 제일 첫번째 키를 기준으로 가져온다.
		$sql				= "select * from fm_boardform where used = 'Y' and label_type = 'radio' and boardid = 'goods_review' order by sort_seq asc limit 1";
		...
		$CI->template->assign('displayClass','designCategoryRecommendDisplay');
		$CI->template->assign('displayElement', 'categoryRecommendDisplay');
		...
		echo '상품 디스플레이 정보가 없습니다.';
	}
}

function _heavyShowCategoryRecommendDisplay($category_code){
	$CI =& get_instance();
	...
	// 디스플레이 설정 데이터
	$query  = $CI->db->query("select d.* from fm_category as c, fm_design_display as d where c.recommend_display_seq=d.display_seq and c.category_code = ?",$category_code);

	$display = $query->row_array();

	if($display){

		// 카테고리 pc 추천상품에 간혹 mobile 디스플레이가 연결되어있음. 강제로 PC 스타일로 변경해줌.
		if($display['platform'] == 'mobile') {
			$query = "UPDATE fm_design_display SET style='lattice_a',platform='pc',kind='category' WHERE display_seq=?";
			$CI->db->query($query, $display['display_seq']);
		...
		if($CI->realMobileSkinVersion >  2 && $CI->mobileMode && $display['m_list_use'] == 'y'){
			$query  = $CI->db->query("select d.* from fm_category as c, fm_design_display as d where c.m_recommend_display_seq=d.display_seq and c.category_code = ?",$category_code);
			$display = $query->row_array();
		...
		**/
		...
		if(!$CI->designDisplayTabAjaxIdx) echo "<div id='{$display_key}' class='designCategoryRecommendDisplay' designElement='categoryRecommendDisplay' templatePath='{$template_path}' displaySeq='{$display_seq}' perpage='{$perpage}' category='{$category_code}' displayStyle='{$display['style']}'>";
