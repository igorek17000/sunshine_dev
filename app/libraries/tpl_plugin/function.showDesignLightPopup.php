	...
	if($flag){
		...
		if($data['style']=='layer'){
			...
			if($data['contents_type']=='image'){
				...
				$popupHtml .= '	<div id="popup_slider_'.$popup_seq.'" class="popup_slider_view"';
				$popupHtml .= '>';
				
				$popupHtml .= $slideHtml;
				$popupHtml .= '	</div>';
				$popupHtml .= '	<div class="popup_slider_tab">';
				$popupHtml .= '		<div id="pop_tab_'.$popup_seq.'" class="pop_tab_list">';
				foreach($banner_item as $k=>$item){
				...
				$popupHtml .= '<script>';
				$popupHtml .= '$(function(){';
				$popupHtml .= " $('#popup_slider_{$popup_seq}').slick({
					slidesToShow: 1,
					slidesToScroll: 1,
					fade: true,
					arrows: false,
					asNavFor: '#pop_tab_{$popup_seq}'
				});
				$('#pop_tab_{$popup_seq}').slick({
					slidesToShow: 3,
					responsive: [{
						breakpoint: 480,
						settings: {
					...
					slidesToScroll: 1,
					asNavFor: '#popup_slider_{$popup_seq}',
					dots: false,
					speed: 600,
					centerMode: true,
					centerPadding: '10px',
