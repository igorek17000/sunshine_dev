...
function assignBrandMenuData(){
	...
	$arr = array('a','b','c','d','e','f','g','h','i','j','k','l','m','n','o','p','q','r','s','t','u','v','w','x','y','z','ㄱ','ㄴ','ㄷ','ㄹ','ㅁ','ㅂ','ㅅ','ㅇ','ㅈ','ㅊ','ㅋ','ㅌ','ㅍ','ㅎ','기타');
	$arr2 = array(
		'ㄱ' => array('가','나'),
		'ㄴ' => array('나','다'),
		'ㄷ' => array('다','라'),
	...
	$query = $CI->db->query("select title,title_eng,category_code,best,brand_image from fm_brand where category_code!='' and level=2 and (hide_in_gnb='0' or hide_in_gnb is null) order by title,title_eng");
	...
	foreach($query->result_array() as $row){
		...
		if($row['title_eng']) $row['prn_title'] = $row['title']."(".$row['title_eng'].")";
		else $row['prn_title'] = $row['title'];
		$all[] = $row;	
	}
