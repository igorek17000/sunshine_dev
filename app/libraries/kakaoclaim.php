...
class Kakaoclaim implements TalkbuyInterface
{
	...
	// 텔레그램 알람 정보
	private $telegrambot = '1720438513:AAESI6BxJd8b6rdyoAeVDfAeMGsBUJ43O6Q';
	private $telegramchatid = '-1001475619918';
	...
	private function _sendTelegramNoticeError(array $errData) {
		$this->CI->load->helper("readurl");

		$msg = "Type : KakaoPay 클레임 오류\n";
		$msg .= "Function : ".$errData["function"]."\n";
		$msg .= "Message : ".$errData["message"]."\n";

        $url = 'https://api.telegram.org/bot' . $this->$telegrambot . '/sendMessage';
		
		$result = readurl($url, array('chat_id' => $this->$telegramchatid, "text" => $msg), false, 7, ["Content-Type" => "application/x-www-form-urlencoded"]);
