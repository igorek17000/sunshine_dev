...
trait BroadcastTrait
{
	...
	/**
	 * 방송 대표이미지 업로드
	 * admin 은 이미 업로드 해서 post 데이터
	 * app 에선 form 태그로 전송받기 때문에 files
	 * 파일 경로는 /data/broadcast/업로드연도/업로드월 (한 폴더에 대량 파일 방지)
	 */
	private function uploadImage($bsSeq, $imageName)
	{
		// admin > post['image] , app > files
		$this->load->library('upload');

		$newpath = ROOTPATH . 'data/broadcast/' . date('Y') . '/' .date('m');
		$this->makePath($newpath);
		...
		if ($this->upload->do_upload('inputsValue')) {
			return $newpath . '/' . $this->upload->file_name;
		}
	}
	...
	* 파일 업로드 했는지 체크
	*/
	function uploadCheckImage($params, $grant) {
		$imageName = false;
	...
	*/
    ...
    private function catalog($param=array())
    {
		...
		if($param['select_status']) {
			...
			$totalPage			= ceil($page['totalcount']/$params['perpage']);
    ...
    private function createBroadcast($params ,$grant='admin')
    {
		...
		// 대표이미지 업로드 / db update
		$imageName = $this->uploadCheckImage($params, $grant);
		$image = $this->uploadImage($bsSeq, $imageName);
		$this->imageBroadcast($bsSeq, $image);
     ...
     * @param string $image
	 */
	private function imageBroadcast($bsSeq, $image) {
		// 단순 이미지 path 수정
    ...
    private function modifyBroadcast($bsSeq, $params, $grant = "admin")
    {
		...
		// 이미지 업로드 체크 (수정 시에는 이미지 업로드 안하면 false);
		$imageName = $this->uploadCheckImage($params, $grant);
		if($imageName !== false) {
