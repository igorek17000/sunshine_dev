...
class Boardlibrary
{
	...
	var $boardManager 	= array();
	var $boardid 		= '';
	
	function Boardlibrary($parameters=array()) {
	 ...
	 * 게시글 등록
	 */
	function data_write($params, $config=array()) {
		...
		if($result) {
			if(BOARDID == 'goods_review') {
				goods_review_count(array("goods_seq"=>$orderProduct['ProductID']), $result);
			}
	 ...
	 * 게시글 수정
	 */
	function data_modify($params, $config=array()) {
	 ...
	 * 게시글 등록 후 처리
	 */
	function board_insert_after_proc($gid) {
		$idxparams = array();
		...
		$upmanagerparams = array('totalnum' => $this->boardManager['totalnum'] );
	 ...
	 * 상품 정보 추출
	 */
	function get_goods_info($goods_seq) {
		if(!$goods_seq)	return;
	 ...
	 * 첨부파일 본문에 삽입
	 */
	function content_insert_file($files, $contents, $mode) {
