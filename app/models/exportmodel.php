...
class exportmodel extends CI_Model {
	...
	public function get_export_item($export_code)
	{
		...
		foreach($query->result_array() as $data){
			...
			// 반품 왕복 배송비 여부 :: 2018-05-17 lwh
			if($data['shipping_cost'] <= 0 && $data['shiping_free_yn'] == 'Y'){
				$data['swap_refund_shiping_cost'] = get_currency_price($data['refund_shiping_cost'] * 2, 1);
			}
	...
	**/
	...
	public function get_exports($arr_export_code)
	{
		...
		if(count($bundle_code_list) > 0)	$where_arr[]	= "AND bundle_export_code in ('".implode("','",$bundle_code_list)."')";
		if(count($export_code_list) > 0)	$where_arr[]	= "AND export_code in ('".implode("','",$export_code_list)."')";
