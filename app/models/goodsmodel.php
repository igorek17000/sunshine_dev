...
class Goodsmodel extends CI_Model {
	...
	public function check_param_regist(){
		...
		if($_POST['goods_type'] != 'gift' && $_POST['goods_kind'] != 'coupon' && !$_POST['shipping_group_seq']){
			openDialogAlert('배송방법을 선택해 주세요.',400,140,'parent','top.moveBookmark("07");');
			exit;
		}
	...
	*/
	...
	public function get_goods_list($arrNo,$imageType='list1', $limit=null){
		...
		$query = "SELECT
				...
				FROM
					fm_goods AS g
					LEFT JOIN fm_provider AS p ON p.provider_seq=g.provider_seq
					LEFT JOIN fm_goods_option AS o ON (g.goods_seq=o.goods_seq AND o.default_option='y')
				WHERE
					g.goods_seq IN (".implode(',',$arrNo).")
				GROUP BY
					g.goods_seq ORDER BY field(g.goods_seq, ".implode(',',$arrNo).")
		";
	...
	public function goods_list($sc){
		...
		if(!empty($sc) && $sc['auto_use']=='y'){
			...
			if(!empty($sc['selectEvent']) ){
				...
				$query = "select provider_list from fm_event_benefits where event_seq = ? LIMIT 1";
				$query = $this->db->query($query, array($sc['selectEvent']));
				$event_benefit_data = $query->row_array();

				$in_provider_list			= array();
				if($event_benefit_data){
					$in_provider_list = array_values(array_filter(explode('|', $event_benefit_data['provider_list'])));
				}

				if(!empty($in_provider_list)){
				    $sqlWhereClause .= " and g.provider_seq in ('".implode("','",$in_provider_list)."')";
				} else {
				    $sqlWhereClause .= " and g.provider_seq = '1' ";
				}
			}
			...
			$sqlSelectClause .= ", fm_design_display_tab_item.display_tab_item_seq";
		...
		switch($sc['sort']){
			case "popular":
				if(!empty($sc['category'])){
					if($sc['m_list_use'] == 'y'){
						$sqlSelectClause	.=  ",cl.mobile_sort";
						$sqlOrderbyClause	= " order by cl.mobile_sort asc, g.goods_seq desc ";
					}else{
						$sqlSelectClause	.=  ",cl.sort";
						$sqlOrderbyClause	= " order by cl.sort asc, g.goods_seq desc ";
					}
				}elseif(!empty($sc['brand'])){
					if($sc['m_list_use'] == 'y') {
						$sqlOrderbyClause	= " order by bl_mobile_sort asc, g.goods_seq desc ";
					}else{
						$sqlOrderbyClause	= " order by bl_sort, g.goods_seq desc ";
					}
				}elseif(!empty($sc['location'])){
					if($sc['m_list_use'] == 'y'){
						$sqlOrderbyClause	= " order by ll_mobile_sort asc, g.goods_seq desc ";
					}else{
						$sqlOrderbyClause	= " order by ll_sort asc, g.goods_seq desc ";
					}
				}else{
					$sqlOrderbyClause	= " order by g.page_view desc ";
				}
		...
		if(!empty($sc['color'])){
			$sqlWhereClause .= " and exists ( select 1 from fm_goods_option oc where oc.goods_seq=g.goods_seq and oc.color != '' and ifnull(oc.color,'') = '".$sc['color']."' )";
		}

		if(!empty($sc['brands'])){
			$sqlSelectClause .= ", bl.sort bl_sort, bl.mobile_sort bl_mobile_sort";
		...
		{
			$sqlSelectClause .= ",ll.location_link_seq,ll.sort as ll_sort,ll.mobile_sort ll_mobile_sort,ll.location_code ";
		...
		if(!empty($sc['relation'])){
			$sqlSelectClause .= ",r.relation_seq";
		...
		if(!empty($sc['relation_seller'])){
			$sqlSelectClause .= ",rs.relation_seq";
		...
		if(!empty($sc['provider_relation'])){
			$sqlSelectClause .= ",r.relation_seq";
			$sqlFromClause		.= " inner join fm_provider_relation r on g.goods_seq=r.relation_goods_seq";
			$sqlWhereClause		.= " and r.provider_seq = '{$sc['provider_relation']}'";
			$sqlOrderbyClause	=" order by r.relation_seq asc";
		}
		...
		$sqlOrderbyClause =  str_replace(array('g.','o.','cl.','rs.','r.','ll.','bl.','gls.','fm_design_display_tab_item.'),'',$sqlOrderbyClause);
		$sql = "
			select * from (
				{$sqlSelectClause}
				{$sqlFromClause}
				{$sqlWhereClause}
			) tbl {$sqlOrderbyClause} {$sqlLimitClause}
		";
		...
		if($sqlLimitClause){
			...
			if($sc['m_code'] == 'all_item') {
				...
				for($i=1; $i<=$sc['page']; $i++){
					$result = select_page($sc['perpage'], $i, 10, $sql, array());
					foreach($result['record'] as $dataRecord) $aRecord[]	= $dataRecord;
				}
			...
			";
			$totalcount = $this->db->query($totalCountsql);
			$totalcount = $totalcount->result_array();
			$result['page']['totalcount'] = $totalcount[0]['cnt'];
		}
		...
		";
		$query = $this->db->query($sql);
		$totalCnt = $query->result_array();
		$result['page']['totalcount'] = $totalCnt[0]['cnt'];
		...
		if($result['record']){
			...
			foreach($result['record'] as $k => $data){
				...
				// 배송사명이 본사일 경우
				if($result_provider[$data['goods_seq']]['provider_name']=="본사"){$result_provider[$data['goods_seq']]['provider_name'] = getAlert("sy009");} // "본사";
			...
			foreach($result['record'] as $k => $data){
				...
				}else if(is_infinite($data['review_divide'])){
				...
				if(isset($sc['selectEvent'])) {
					...
					if(!(in_array($sc['selectEvent'], $event_list))) unset($result['record'][$k]);
				}
	...
	public function get_stat_light($params, $main_condition){
		...
		switch($params['act']){
			case 'order_ea':
				$sqlColOrderBy = ' sum(fds.daily_ea) as total_order ';
				break;
			case 'review_sum':
				$sqlColOrderBy = ' sum(fds.daily_score_5)+sum(fds.daily_score_4)+sum(fds.daily_score_3)+sum(fds.daily_score_2)+sum(fds.daily_score_1) as total_order ';
				break;
			default :
				$sqlColOrderBy = ' sum(fds.daily_cnt) as total_order ';
		}
		...
		// 기준이 되는 코드 설정
		$standard_field = 'category';
		if($params['standard'] == 'location') $standard_field = 'location';

		// 조건에 맞는 테이블 가져오나, 기본 조건이 없으면 category 에서 가져오도록 수정
		$standard_table = in_array($params['standard'],array('brand','category','location')) ? $params['standard'] : 'category';

		$query = "
		SELECT
		fds.daily_goods_seq, [:ACT_ORDERBY:]
		FROM (select * from fm_".$standard_table."_link
				where ".$standard_field."_code = '".$params['category_code']."' )as fcl
			left join (
				select rfdsvr.cnt, fdsvr.* from
				(
			...
			ON fcl.goods_seq = fds.daily_goods_seq
			[:ACT_CONDITION:]
	...
	public function auto_condition_goods_list($sc){
		...
		$data = array();
		$platform	= 'P';
		if( $this->mobileMode || $this->_is_mobile_agent ) $platform	= 'M';
		...
		foreach($sc as $key => $params){
			$is_sort = false;
			$sqlSelectClause	 = "select * from ( select ";
			if($params['sort']){
				$is_sort = true;
			...
			$sqlGroupbyClause		= array();
			$sqlHavingClause		= array();
			$sqlHavingClauseOrderby	= '';

			if	(!empty($params['bigdata'])){
				if	($key == 'admin' || !$params['goods_seq_exclude']) continue;
				if	(empty($params['same_category'])) $params['same_category'] = 1;
			}
			...
			if	($params['act'] != 'recently'){
				if($key != 'none'){
					...
					$big_ip_arr		= $result_bigdata['big_ip_arr'];
					$result_stat	= $this->get_stat($params, $main_condition);

					foreach($result_stat as $goods_seq => $data_stat) $stat_goods_seqs[] = $goods_seq;
					if( $stat_goods_seqs ) $sqlWhereClause[] = "g.goods_seq in (".implode(',' ,$stat_goods_seqs).")";
					...
					foreach($result_stat as $goods_seq => $data_stat) {
						if($goods_seq)	$stat_goods_seqs[] = $goods_seq;
					}
				...
				if($params['act'] != 'review_sum'){
					...
					$sqlWhereClause[] = "  g.regist_date >= '{$act_month}' ";
				}
			}
			...
			$sqlFromClause[]		= " from
											fm_goods g
													inner join fm_goods_option o
														on o.default_option = 'y'
															and g.goods_seq = o.goods_seq
													left join fm_goods_list_summary as gls on g.goods_seq = gls.goods_seq and gls.platform = '".$platform."' ";
			...
			if	($act_goods_seq == 'login_no') continue;
			...
			if( serviceLimit('H_AD') ) {
			    $sqlWhereClause[]		= "g.provider_status = '1'";
			}
			$sqlWhereClause[]		= "g.goods_type = 'goods'";
			$sqlWhereClause[]		= "g.goods_view = 'look'";
			$sqlWhereClause[]		= "g.goods_status = 'normal'";

			############# 상품기준 동일한 카테고리,브랜드,지역 ###############
			if	(!empty($params['same_category'])){
				$sqlFromClause[]		= " inner join fm_category_link l on l.goods_seq=g.goods_seq ";

				// none 인경우 해당 항목 자체가 기준이 됨
				if($is_none){
					$sqlWhereClause[]		= " l.category_code = '". $params['category_code'] ."' ";
				}else{
					$sqlWhereClause[]		= " l.category_code = (select category_code from fm_category_link where goods_seq = '".$act_goods_seq."' and link = 1 limit 1) ";
				}
			}
			...
			if	(!empty($params['same_brand'])){
				...
				// none 인경우 해당 항목 자체가 기준이 됨
				if($is_none){
					$sqlWhereClause[]		= " bl.category_code = '" . $params['category_code'] . "'";
				}else{
					$sqlWhereClause[]		= " bl.category_code = (select category_code from fm_brand_link where goods_seq = '".$act_goods_seq."' and link = 1 limit 1)";
				}
			}
			...
			if	(!empty($params['same_location'])){
				...
				// none 인경우 해당 항목 자체가 기준이 됨
				if($is_none){
					$sqlWhereClause[]	= " ll.location_code = '" . $params['category_code'] . "' ";
				}else{
					$sqlWhereClause[]	= " ll.location_code = (select location_code from fm_location_link where goods_seq = '".$act_goods_seq."' and link = 1 limit 1) ";
				}

			}

			if	(!empty($params['goods_seq_exclude']) && !$is_none){
				$sqlWhereClause[]		= " g.goods_seq != '".$params['goods_seq_exclude']."' ";
			}

			################# 관리자지정 카테고리,브랜드,지역 #################
			if	(!empty($params['category'])){
				$sqlFromClause[]		= " inner join fm_category_link l on l.goods_seq=g.goods_seq and l.link=1 ";
				$sqlWhereClause[]		= " l.category_code = '".$params['category']."' ";
			}

			if	(!empty($params['brand'])){
				$sqlFromClause[]		= " left join fm_brand_link bl on bl.goods_seq=g.goods_seq ";
				$sqlWhereClause[]		= " bl.category_code = '".$params['brand']."' and bl.link = 1 ";
			}

			if	(!empty($params['location']))
			{
				$sqlFromClause[]		= " left join fm_location_link ll on ll.goods_seq=g.goods_seq ";
				$sqlWhereClause[]		= " ll.location_code = '".$params['location']."' ";
			}

			############################# 입점사 ###############################
			if	(!empty($params['same_seller'])){
				$sqlWhereClause[]		= " g.provider_seq = (select provider_seq from fm_goods where goods_seq = '".$act_goods_seq."' limit 1) ";
			}
			if	($params['provider'] == 1)
				$sqlWhereClause[]		= " g.provider_seq = 1 ";

			if	( $params['provider_seq'] )
				$sqlWhereClause[]		= " g.provider_seq='{$params['provider_seq']}' ";

			if	(!empty($params['limit']))
				$sqlLimitClause = " limit {$params['limit']} ";

			$sqlOrderbyClause		= " order by goods_seq desc ";//total_order desc,

			if	($params['act'] != 'recently' && $stat_goods_seqs)
				$sqlOrderbyClause		= " order by field(g.goods_seq, ".implode(',' ,$stat_goods_seqs).")";//total_order desc,
			...
			if($is_sort){
				switch($params['sort']){
					...
					case "popular":
						if(!empty($params['category']))
							$sqlOrderbyClause	= " order by l.sort asc";
						elseif(!empty($params['brand']))
							$sqlOrderbyClause	= " order by bl.sort asc";
						else
							$sqlOrderbyClause	= " order by g.page_view desc ";
					break;
					case "newly":
			...
			if	(!$is_sort)
				$sqlOrderbyClause = ' '.$sqlOrderbyClause . ' '.$sqlLimitClause.') as tab ';

			$sqlWhereClause = $sqlWhereClause ? " where ".implode(" and ",$sqlWhereClause): '';

			$sqlFromClause		= implode(' ',$sqlFromClause);

			$sqlGroupbyClause	= implode(' ',$sqlGroupbyClause);

			$sqlHavingClause	= $sqlHavingClause ? " having ".implode(' and ',$sqlHavingClause)." ".$sqlHavingClauseOrderby : '';
			...
			if($sqlLimitClause){
				...
				if($params['m_code'] == 'all_item') {
					$result = select_page($params['perpage'],$params['page'],10,$sql,array(),null,$params['m_code']);
				} else {
					$result = select_page($params['perpage'],$params['page'],10,$sql,array());
				}
			...
			if($goods_seq_array) $result_image		= $this->get_images($goods_seq_array, $params['image_size']);
			if($goods_seq_array) $result_option		= $this->get_colors($goods_seq_array);
			if($goods_seq_array) $result_provider	= $this->get_provider_names($goods_seq_array);
			if($goods_seq_array) $result_category_code	= $this->get_category_codes($goods_seq_array);
			if($goods_seq_array) $result_category_codes	= $this->get_goods_category_codes($goods_seq_array);
			...
			foreach($result['record'] as $k => $data){
				...
				if( $result_stat[$data['goods_seq']] ) foreach( $result_stat[$data['goods_seq']] as  $field_stat => $data_stat){
		...
		if($result['record']){
			...
			foreach($result['record'] as $k => $data){
				...
				if($base_image_alt){
					$image_alt				= $base_image_alt;
					$data['shop_name']	= $this->config_basic['shopName'];
					foreach($replace as $key => $code) $image_alt	= str_replace($code, $data[$key], $image_alt);
					$image_alt				= htmlspecialchars(strip_tags($image_alt));
				}
	...
	public function admin_goods_list_new($sc) {
		...
		if( $sc['search_feed_status'] ){
			...
			if( count($sc['search_feed_status']) == 1){
				if( in_array('Y', $sc['search_feed_status']) ){
					$whereStr['goods'] .= " AND C.feed_status != 'N'";
		...
		if (count($sc['color_pick']) > 0) {
			$colorKeys	= implode(' + ', $sc['color_pick']);
			$whereStr['goods'] .= " AND MATCH (C.color_pick) AGAINST ('{$colorKeys}') ";
		}
		...
		$tables = "(
					...
					FROM
						...
						FROM
							...
							INNER JOIN fm_goods_restock_option H ON A.restock_notify_seq = H.restock_notify_seq
							INNER JOIN fm_goods_option B ON A.goods_seq = B.goods_seq AND IFNULL(H.option1,'') = IFNULL(B.option1,'') AND IFNULL(H.option2,'') = IFNULL(B.option2,'') AND IFNULL(H.option3,'') = IFNULL(B.option3,'') AND IFNULL(H.option4,'') = IFNULL(B.option4,'') AND IFNULL(H.option5,'') = IFNULL(B.option5,'')
							INNER JOIN fm_goods_supply C ON A.goods_seq	= C.goods_seq AND B.option_seq = C.option_seq
							INNER JOIN fm_goods F ON A.goods_seq = F.goods_seq
						WHERE
							1 ".$where_link_sql.$where_brand_link_sql."
						) AS K
					) AS Z
					";
	...
	public function runout_check($goods_seq,$mode='auto')
	{
		...
		if( $cfg['runout'] != 'unlimited' ){
			...
			$query = "select 
							...
							sum(if(IFNULL(if(s.stock < s.badstock, 0, s.stock-s.badstock),0) < IFNULL(s.reservation15,0),0,IFNULL(if(s.stock < s.badstock, 0, s.stock-s.badstock),0) - IFNULL(s.reservation15,0))) ablestock 
					from
	...
	**/
	...
	public function get_goods_view($no, $no_related = false, $no_bigdata = false){
		...
		if	($goods['goods_type'] == 'gift'){
			//해당상품이 존재하지 않습니다.
			return array('status'=>'error', 'errType'=>'echo', 'msg'=>'<script>alert("'.getAlert('gv022').'");top.location.href="/main";</script>');
		}

		if	(!isset($goods['goods_seq'])) {
			//해당상품이 존재하지 않습니다.
			return array('status'=>'error', 'errType'=>'back', 'msg'=>getAlert('gv023'));
		}
		...
		if($images[1]['view']['image']){
			...
			if(!$imageWidth)	$imageWidth		= 415;
			if(!$imageHeight)	$imageHeight	= 554;
		}
		...
		## 옵션 순번 정렬
		if($options) foreach($options as $data_option){
			$sort_options[] = $data_option;
		}
		if($sort_options) $options = $sort_options;
		...
		if(!$goods['shipping_group_seq']){
			...
			$query	= $this->db->query($base_sql,$goods['provider_seq']);
			$res	= $query->row_array();
		...
		foreach($ship_set_arr as $k => $val){
			...
			if($val['hop_use'] == 'Y'){
				...
				if(date('Y-m-d') == $val['hop_date'])	$today_possible = '(오늘)';
				else									$today_possible = '';

				// 필수여부 문구 추출
				if($val['hopeday_required'] == 'Y')		$required_txt	= '(필수)';
				else									$required_txt	= '(선택)';
			...
			if($method_key == 'direct_store'){
				...
				foreach($store_info as $s => $store){
					...
					if($store['store_supply_set'] == 'Y'){
						...
						$sql			= "select stock from fm_goods_supply where goods_seq = '".$goods['goods_seq']."'";
						$query			= $this->db->query($sql);
			...
			if($shipping_policy['shipping_method_key'][$method_key]) continue;
		...
		$goods['gift']	= $this->giftmodel->get_gift($today, $goods['price'],
													$gift_goods, $gift_categorys, $goods['provider_seq'],$goods['shipping_group_seq']);
		...
		if(!$no_related){
			...
			$sc['limit']			= $this->operation_type == 'light' ? $display['count_r'] : $display['count_w']*$display['count_h'];
			...
			if( !$goods['relation_count_w'] )			$goods['relation_count_w']			= $display['count_w'];
			if( !$goods['relation_count_h'] )			$goods['relation_count_h']			= $display['count_h'];
			if( !$goods['relation_count_r'] )			$goods['relation_count_r']			= $display['count_r'];
			if( !$goods['relation_image_size'] )		$goods['relation_image_size']		= $display['image_size'];
	...
	*/
	public function get_review_option_standard(){
		$sql	= "SELECT label_value FROM fm_boardform WHERE boardid = 'goods_review' ORDER BY bulkorderform_seq ASC LIMIT 1";
		$query	= $this->db->query($sql);
		$data	= $query->row_array();

		// 가공하여 첫번째 값을 반환 (첫번째 항목이 기준임)
		$data	= explode('|', $data['label_value']);
