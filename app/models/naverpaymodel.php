...
class naverpaymodel extends CI_Model {
	...
	public function order_export($export){
		...
		if($export['domestic_shipping_method'] == "direct_delivery"){
	...
	public function set_reverse_return($return_code,$npay_data=array()){
		...
		if($this->cfg_order['save_type'] == 'exist') $reserve_mode = "destroy";
		...
		foreach($data_return_item as $return_item){
			...
			if($reject_use){
				...
				##----------------------------------------------------------------------------------
				//구매확정기간 종료 후 반품삭제시 : 자동구매확정처리를 위한 데이터 정리 pjm
				if($this->cfg_order['buy_confirm_use']){
					...
					$auto_buyconfirms				= true;								// Npay 주문 > 설정 구매확정일시랑 관련없음.
		...
		##----------------------------------------------------------------------------------
		## 구매확정기간 종료 후 반품삭제시 : 지급예정반품수량 저장. 마일리지예정수량-반품수량 2015-03-27 pjm
		...
		foreach($exports as $export_code=>$data_export){
			...
			$reserve_mode		= "";	//자동구매확정 로그>마일리지 실 지급 여부
			$tot_reserve		= 0;
			$tot_point			= 0;
			$tot_confirm_ea		= 0;
			...
			foreach($data_export['items'] as $export_items){
				...
				## 구매확정기간이 종료된 출고건
				if($export_items['auto_buyconfirms'] && $this->cfg_order['buy_confirm_use']){

					$auto_buyconfirms	= true;
					$reserve_update		= true;

					//지급예정수량 : 지급예정수량 + 반품취소수량
					$confirm_ea		= $export_items['reserve_ea'] + $return_ea;
					$tot_confirm_ea += $confirm_ea;

					if($this->cfg_order['save_type'] == 'give'){

						$reserve_mode				= "buyconfirm";
						$tmp['reserve_ea']			= 0;
						//지급예정 반품수량 : 기존 지급예정반품수량-반품취소수량
						$tmp['reserve_return_ea']	= $export_items['reserve_return_ea'] - $return_ea;
						//구매확정수량 : 기존구매확정수량+지급예정수량+반품취소수량
						$tmp['reserve_buyconfirm_ea'] = $export_items['reserve_ea'] + $return_ea + $export_items['reserve_buyconfirm_ea'];

						$reserve		= 0;
						$point			= 0;
						if($export_items['opt_type'] == "opt"){
							$reserve	= $this->ordermodel->get_option_reserve($export_items['option_seq']);
							$point		= $this->ordermodel->get_option_reserve($export_items['option_seq'],'point');
						}else if($export_items['opt_type']=='sub'){
							$reserve	= $this->ordermodel->get_suboption_reserve($export_items['suboption_seq']);
							$point		= $this->ordermodel->get_suboption_reserve($export_items['suboption_seq'],'point');
						}
						$tot_reserve	+= $reserve * $confirm_ea;
						$tot_point		+= $point * $confirm_ea;

					## 지급예정 수량 소멸 처리
					}elseif($this->cfg_order['save_type'] == 'exist'){

						$reserve_mode					= "destroy";
						#지급예정수량 : 0, 소멸수량 : 기존수량+지급예정수량+반품수량
						$tmp['reserve_ea']				= 0;
						$tmp['reserve_destroy_ea']		= $export_items['reserve_ea']+$export_items['reserve_destroy_ea']+$return_ea;
						$tmp['reserve_return_ea']		= $export_items['reserve_return_ea'] - $return_ea;
					}
			...
			//자동구매확정시, 마일리지/포인트 지급 일때
			if( $data_order['member_seq'] && $reserve_mode == "buyconfirm" && $auto_buyconfirms){
				if($tot_reserve){
		...
		// 실제 삭제 처리가 끝난 후에 정산 데이터 조정 및 구매확정 처리 by hed
		foreach($exports as $export_code=>$data_export){
			$auto_buyconfirms	= false;
			foreach($data_export['items'] as $export_items){
				## 구매확정기간이 종료된 출고건
				if($export_items['auto_buyconfirms'] && $cfg_order['buy_confirm_use']){
					$auto_buyconfirms	= true;
				}
			}
			...
			$params_check_buyconfirm['order_seq']		= $data_order['order_seq'];
			$params_check_buyconfirm['export_code']		= $export_code;

			$this->load->model('buyconfirmmodel');
			$check_buyconfirm = $this->buyconfirmmodel->check_ing_return_for_buyconfirm($params_check_buyconfirm);

			if($auto_buyconfirms && $check_buyconfirm){
				...
				$data_buy_confirm['doer']			= $this->managerInfo['mname'];
				$data_buy_confirm['ea']				= $tot_confirm_ea;
				$data_buy_confirm['emoney_status']	= ($reserve_mode == "buyconfirm")? "pay" : $reserve_mode;
				$data_buy_confirm['actor_id']		= $this->managerInfo['manager_id'];
				...
				$this->ordermodel->set_log($data_export['order_seq'],'buyconfirm',$this->managerInfo['mname'],$order_log_title,'','','','Npay');
				...
				**/
				...
				//정산확정 처리
				$this->accountallmodel->insert_calculate_sales_buyconfirm($order_seq, $export_code);
				//debug_var($this->db->queries);
				//debug_var($this->db->query_times);
				/**
