...
class order2exportmodel extends CI_Model {
	...
	public function export_for_coupon($export,$cfg){
		...
		foreach($export as $export_data){
			...
			if ($export_data['export_code']){
				$result_export_code = $export_data['export_code'];

				/** 
				- 티켓번호 때문에 실시간 발송되도록 개선
				- @ 2017-08-14
				// 티켓상품 출고완료 후처리
				$arr_params = array(
					'result_export_code' => array($result_export_code),
					'sendType' => 'all',
					'email' => $export_data['coupon_mail'],
					'sms' => $export_data['coupon_sms']
				);
				$this->batchmodel->insert('complete_ticket',serialize($arr_params),'none');
				**/
			}else{
				for($i=0;$i<count($export_data['items']['item_seq']);$i++){
					...
					if($result){
						...
						$result_export_code = $result[1];
						$result_export[$param_coupon['export_item_seq']] = $result[2];
						/** 
						- 티켓번호 때문에 실시간 발송되도록 개선
						- @ 2017-08-14
						// 티켓상품 출고완료 후처리
						$arr_params = array(
							'result_export_code' => $result_export_code,
							'sendType' => 'all',
							'email' => $export_data['coupon_mail'],
							'sms' => $export_data['coupon_sms']
						);
	...
	public function check_error_export($export,$cfg,$mode='',$param_shipping_for_order='')
	{
		...
		foreach($export as $export_key => $export_data){
			...
			// 출고준비 처리일때만 오픈마켓 주문 여부 확인 r23843 에 의해 변경 불가 기능 제거 by hed
			/*
			if($cfg['step']=="45"){
				$data_order = $this->ordermodel->get_order($export_data['order_seq']);
				if($data_order['linkage_id'] == "connector"){
					$error_arr[] = array(
							'msg'=>'오픈마켓 주문건은 출고준비 상태로 변경 불가능합니다.출고완료로 변경해주세요.',
							'order_seq'=>$export_data['order_seq'],
							'step'=>$cfg['step']
					);
					$export_data['export_result'] = "오픈마켓 주문 출고준비 상태 변경 불가";
					continue;
				}
			}
			 */
			...
			if( $export_data['OPT'] ) foreach( $export_data['OPT'] as $option_seq => $data_option ){
				...
				foreach($order_data as $shipping_data){
					...
					$_stork_check_use		= true;																		//재고체크 사용유무
					...
					*/
					if(($this->scm_cfg['use'] != 'Y' && $cfg['stockable'] != 'limit') 
						|| (defined('__SELLERADMIN__') === true && $cfg['stockable'] != 'limit')
						|| ($this->scm_cfg['use'] == 'Y' && $provider_seq == 1 && ($scm_auto_warehousing || $data_option['not_match_goods_order'] == 'n' || $cfg['not_connect_scm'] == 'Y'))){
						$_stork_check_use = false;
					}
					...
					if($data_option['package_yn'] == 'y'){
						...
						if( $goods_matching && $goods_stock < $data_option['request_ea'] && $cfg['step'] >= 50 && $_stork_check_use){
							$error_arr[] = array(
									'msg'=>'재고 부족',
									'order_seq'=>$export_data['order_seq'],
									'shipping_seq'=>$shipping_seq,
						...
						foreach($shipping_data['options'] as $option_data){
							...
							if( array_key_exists($suboption_seq,$option_data['suboptions']) ){
								...
								if($data_suboption['package_yn'] == 'y'){
									...
									//출고되는 실물상품은 '부분출고완료'이상에서만 재고체크 @2016-05-09
									if( $goods_matching && $goods_stock && $goods_stock < $data_suboption['request_ea'] && $cfg['step'] >= 50 && $_stork_check_use ){
										$error_arr[] = array(
												'msg'=>'재고 부족',
												'order_seq'=>$export_data['order_seq'],
												'shipping_seq'=>$export_data['shipping_seq'],
			...
			if( $export_data['COU'] ) foreach( $export_data['COU'] as $option_seq => $data_option ){
				...
				foreach($order_data as $shipping_data){
					...
					//출고되는 티켓상품은 '부분출고완료'이상에서만 재고체크 @2016-05-09
					if( $goods_matching && $goods_stock < $data_option['request_ea'] && $cfg['ticket_step'] >= 50 && $cfg['ticket_stockable'] == 'limit' ){
						$error_arr[] = array(
								'msg'=>'재고 부족',
								'order_seq'=>$export_data['order_seq'],
								'shipping_seq'=>$shipping_seq,
