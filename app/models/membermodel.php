...
class Membermodel extends CI_Model {
	...
	public function admin_member_list($sc) {
		...
		$sqlOrderClause		=" order by A.member_seq ";
	...
	public function admin_member_list_spout($sc) {
		...
		if( !empty($sc['anniversary_edate']))
			$sc['anniversary_edate']	= date('md', strtotime(date('Y-') . $sc['anniversary_edate']));
		if( !empty($sc['anniversary_sdate']) && !empty($sc['anniversary_edate'])){
			$specialDayWhereClause .= " AND REPLACE(A.anniversary, '-', '') between ? and ? ";
			$bind[] = $sc['anniversary_sdate'];
			$bind[] = $sc['anniversary_edate'];
		}else if( !empty($sc['anniversary_sdate']) && empty($sc['anniversary_edate']) ){
			$specialDayWhereClause .= " AND LENGTH(A.anniversary) > 0 AND REPLACE(A.anniversary, '-', '') >= ? ";
			$bind[] = $sc['anniversary_sdate'];
		}else if( empty($sc['anniversary_sdate']) && !empty($sc['anniversary_edate']) ){
			$specialDayWhereClause .= " AND LENGTH(A.anniversary) > 0 AND REPLACE(A.anniversary, '-', '') <= ? ";
			$bind[] = $sc['anniversary_edate'];
...
SQL;
	...
	public function get_group_except_category($group_seq,$sale_seq,$category,$type)
	{
		...
		if(is_array($category))
			$where		= " and category_code in('".implode("','",$category)."') ";
		else
			$where		= " and category_code = '".$category."' ";

		$query = "select count(*) cnt from fm_member_group_issuecategory where type=? and sale_seq = ? " . $where;
	...
	public function get_sms_restriction(){
		...
		### 발송시간 제한
		$restriction_title = array("comm"				=> "주문 메시지"
									,"goods"			=> "배송 메시지"
									,"coupon"			=> "티켓<br />상품"
									,"join"				=> "회원가입"
									,"findid"			=> "아이디 찾기"
	...
	public function update_private_encrypt($memberseq, $params = []){
		...
		if(!empty($memberseq)){
			...
			$sql = "select email,cellphone,phone from fm_member where member_seq=?";
	...
	public function getMemberBySns($snsType, $snsId) {
		...
		return $this->db->select('m.*, mg.group_name')
				->from('fm_member AS m')
