...
class ordermodel extends CI_Model {
	...
	public function get_order_list($sc=array()){
		...
		if(!empty($sc['member_seq'])){
			$sqlWhereClause = " WHERE ord.member_seq = '{$sc['member_seq']}'";
		}
		...
		if( $sc['keyword'] ){
			...
			$bind[] = "%" .$this->db->escape_str($sc['keyword'])."%";
			$bind[] = "%" .$this->db->escape_str($sc['keyword'])."%";
			...
			(
				(
					...
					order_seq IN
					(
						SELECT order_seq FROM fm_order_item WHERE goods_name LIKE ?
					)
				)
			)
			";
		...
		if( $sc['step_type'] ){
			switch($sc['step_type']){
				...
				case 'deposit_only': $where[] = "step = '25'"; break;
				case 'ready': $where[] = "step in ('35','40','45')"; break;
				case 'ready_only': $where[] = "step = '35'"; break;
				case 'export': $where[] = "step in ('40','45','50','55','60','65','70')"; break;
				case 'export_and_complete': $where[] = "step in ('40','45','50','55','60','65','70','75')"; break;
				case 'delivery_ing': $where[] = "step between '50' and '70'"; break;
				case 'delivery_complete': $where[] = "step = '75'"; break;
				case 'non_attempt': $where[] = "step <> '0'"; break;
			}
		}
		...
		$sql = "SELECT * FROM (
			...
			(
				SELECT b.provider_name FROM fm_order_item a
				LEFT JOIN fm_provider b on a.provider_seq = b.provider_seq
				WHERE order_seq=ord.order_seq ORDER BY item_seq LIMIT 1
			) provider_name,
			(
		...
		foreach($result['record'] as $k => $data)
		{
			...
			// 배송사명이 본사일 경우
			if($result['record'][$k]['provider_name']=="본사"){$result['record'][$k]['provider_name'] = getAlert("sy009");} // "본사";
		}
		...
		if( count($result_order_seq) > 0 ){
			$in_string = str_replace(' ', ',', trim(str_repeat("? ", count($result_order_seq))));
			$where = "order_seq in(".$in_string.")";

			// 상품번호, 상품명, 이미지를 구하기 위해 sub query 최적화
			$itemsql	= "select order_seq,goods_seq, goods_name, image from fm_order_item
						where ".$where." order by item_seq asc";
			$itemsql = $this->db->query($itemsql, $result_order_seq);

			foreach($itemsql->result_array() as $item_data){
			...
			$optsql	= "SELECT A.order_seq,sum(reserve*ea) as tot_reserve, sum(point*ea) as tot_point
						FROM fm_order_item_option A
							left join fm_order_item B ON A.item_seq = B.item_seq
						WHERE B.".$where." group by A.order_seq";
			$optsql = $this->db->query($optsql, $result_order_seq);
			foreach($itemsql->result_array() as $opt_data){
			...
			// 상품번호, 상품명, 이미지를 구하기 위해 sub query 최적화
			$subsql		= "SELECT A.order_seq,sum(reserve*ea) as tot_reserve, sum(point*ea) as tot_point
						FROM fm_order_item_suboption A
							left join fm_order_item B ON A.item_seq = B.item_seq
						WHERE B.".$where." group by A.order_seq";
			$subsql = $this->db->query($subsql, $result_order_seq);
			foreach($itemsql->result_array() as $sub_data){
	...
	public function get_item($order_seq){
		...
		if( defined('__SELLERADMIN__') === true ){
			...
			SELECT
				...
				b.provider_name,
				b.provider_id,
				(select cancel_type from fm_goods where goods_seq=a.goods_seq) cancel_type,
				g.master_goods_seq
			FROM fm_order_item as a
			LEFT JOIN fm_goods as g on a.goods_seq = g.goods_seq
			LEFT JOIN fm_provider as b on g.provider_seq = b.provider_seq
			WHERE a.order_seq=? and  a.provider_seq=?
			ORDER BY CASE WHEN a.goods_type =  'goods' THEN 0 ELSE 99 END ASC, a.provider_seq asc,a.shipping_seq asc
			";
			...
			WHERE a.order_seq=?
			ORDER BY CASE WHEN a.goods_type =  'goods' THEN 0 ELSE 99 END ASC, a.provider_seq asc,a.shipping_seq asc, a.item_seq
			";
	...
	**/
	 ...
	 * 관리자 주문 조회 전용
	 * */
	public function get_order_catalog_query_spout( $_PARAM = array('list') ){
	    $groupBy = "";
	    ...
	    //query where 시작
	    $str_where_order = " WHERE ord.step > 0 AND ord.hidden =  'N'";

	    //날짜
	    if($_PARAM['regist_date'][0] && $_PARAM['regist_date'][1]){
	        if ($_PARAM['date_field']) {
	            $str_where_order .= " AND ord.{$_PARAM['date_field']} >= '{$_PARAM['regist_date'][0]} 00:00:00' AND ord.{$_PARAM['date_field']} <= '{$_PARAM['regist_date'][1]} 23:59:59'";
	        } else {
	            $str_where_order .= " AND ord.regist_date >= '{$_PARAM['regist_date'][0]} 00:00:00' AND ord.regist_date <= '{$_PARAM['regist_date'][1]} 23:59:59'";
	        }
	    }

	    // 검색시간 검색 후 들어온 데이터를 무시 :: 2019-08-09 hyem
	    if( $_PARAM['searchTime'] ){
	        $str_where_order .= "AND ord.regist_date <= '" . $_PARAM['searchTime'] . "'";
	    }
	    ...
	    if($_PARAM['chk_step']){
	        ...
	        $str_where_order .= " AND ord.step IN ('" . join("', '", $aSteps) . "')";
	    }

	    //합포장 여부
	    if($_PARAM['chk_bundle_yn']){
	        $str_where_order .= " AND ord.bundle_yn = 'y'";
	    }

	    //중요주문
	    if($_PARAM['important']){
	        $str_where_order .= " AND ord.important IN ('" . join("', '", $_PARAM['important']) . "')";
	    }
		...
		*/
		...
		//결제수단
		if($_arr_payment) $_where_payment[] = " ord.payment IN ('" . join("', '", $_arr_payment) . "')";

		//결제사
		if($_arr_pg) $_where_payment[] = " ord.pg IN ('" . join("', '", $_arr_pg) . "')";

		if($_is_payment_bank){
			//무통장 포함 검색 시
			$str_where_order .= " AND ( ord.payment = 'bank' ";
			if($_where_payment){
				$str_where_order .= " or ( ".implode(" AND ",$_where_payment) .")";
			}
			$str_where_order .= ")";
		}else{
			//무통장 미포함 검색 시
			if($_where_payment) $str_where_order .= " AND ".implode(" AND ",$_where_payment);
		}

		if (!$_is_payment_bank && ($_PARAM['pg'] || $_PARAM['payment'])) { //#28245 무통장입금 시 기본 pg 정보도 DB에 입력 되어 부득이 추가 kmj
			$str_where_order .= " AND ord.payment != 'bank'";
		}
		//-------------------------------------------------------------------------------------------------------
	    ...
	    //주문환경
	    if($_PARAM['sitetype']){
	        $str_where_order .= " AND ord.sitetype IN ('" . join("', '", $_PARAM['sitetype']) . "')";
	    }
	    ...
	    if($_PARAM['ordertype']){
	        ...
	        foreach($_PARAM['ordertype'] as $k => $v){
	            ...
	            if($v == 'change'){
	                $str_where_order .= "IFNULL(ord.orign_order_seq, '') != ''";
	            }else if($v == 'admin'){
	                $str_where_order .= "IFNULL(ord.admin_order, '') != ''";
	            }else if($v == 'personal'){
	                $str_where_order .= "IFNULL(ord.person_seq, 0) > 0";
	            }
	        }
	        $str_where_order .= ")";
	    }
	    ...
	    if(!empty($_PARAM['selectMarkets']) && count($_PARAM['selectMarkets']) >0 ) {
	        ...
	        if (($key = array_search('NOT', $_PARAM['selectMarkets'])) !== false) {
	            ...
	            $selectMarkets[] = "ord.linkage_id IS NULL";
	            $selectMarkets[] = "ord.linkage_id = ''";
	        }

	        if(count($_PARAM['selectMarkets']) > 0 ) {
	            $selectMarkets[] = "ord.linkage_mall_code IN ('" . implode("', '", $_PARAM['selectMarkets']) . "') ";
	        }
	        ...
	        $str_where_order .= " AND {$selectMarkets}";
	    }
	    ...
	    //주문유입
	    if($_PARAM['referer']){
	        $goodsviewjoin .= " LEFT JOIN fm_referer_group rg ON (ord.referer_domain != '' and ord.referer_domain = rg.referer_group_url)";

	        $str_where_order .= " AND (
				IF( rg.referer_group_no > 0, rg.referer_group_cd, IF( LENGTH( ord.referer ) > 0,  'etc',  'direct' ) )
				IN ('". join("','", $_PARAM['referer']) . "')";
	        $str_where_order .= ")";
	    }

		// 배송책임 - inner join 에서 subquery 로 변경 2021-05-18
		if($_PARAM['shipping_provider_seq']){
			$str_where_order .= " AND (SELECT order_seq FROM fm_order_shipping WHERE order_seq=ord.order_seq AND provider_seq = '{$_PARAM['shipping_provider_seq']}' LIMIT 1) > 0";
		}

	    if( $_PARAM['shipping_hop_use'] && $_PARAM['shipping_hope_sdate'] && $_PARAM['shipping_hope_edate'] ){
	        if (strpos($goodsviewjoin, 'fm_order_shipping') === false) {
	            $goodsviewjoin .= " INNER JOIN fm_order_shipping as os on os.order_seq=ord.order_seq";
	        }
	        if($_PARAM['shipping_hop_use']){
	            $str_where_order .= " AND os.shipping_hop_date >= '{$_PARAM['shipping_hope_sdate']}'
						AND os.shipping_hop_date <= '{$_PARAM['shipping_hope_edate']}'";
	        }
	    }
	    ...
	    if( $_PARAM['shipping_reserve_use'] && $_PARAM['shipping_reserve_sdate'] && $_PARAM['shipping_reserve_edate'] ){
	        ...
	        if($_PARAM['shipping_reserve_use']){
	            $str_where_order .= " AND os.reserve_sdate BETWEEN '{$_PARAM['shipping_reserve_sdate']}'
						AND '{$_PARAM['shipping_reserve_sdate']}'";
	        }
	    }

	    if( $_PARAM['nation'] ){
			// 중복으로 보이는 현상이 있어 subquery 로 수정함 
			$domestic_shipping_query = "(ord.international = 'domestic' AND shipping_method IN ('".join("','", $_PARAM['shipping_set_code']['domestic'])."'))";
			$international_shipping_query = "(ord.international = 'international' AND shipping_method IN ('".join("','", $_PARAM['shipping_set_code']['international'])."'))";
	        ...
	        if( array_search('domestic', $_PARAM['nation']) !== false && array_search('international', $_PARAM['nation']) !== false ) {
	            if($_PARAM['shipping_set_code']['domestic'] && $_PARAM['shipping_set_code']['international']){
					$str_where_order .= " AND (SELECT order_seq FROM fm_order_shipping WHERE order_seq=ord.order_seq AND ( $domestic_shipping_query OR $international_shipping_query) LIMIT 1 ) > 0";
	            } else {
	                $str_where_order .= " AND ord.international IN ('domestic', 'international') ";
	            }
	        }else if( array_search('domestic', $_PARAM['nation']) !== false ){
	            if($_PARAM['shipping_set_code']['domestic']){
	                $str_where_order .= " AND (SELECT order_seq FROM fm_order_shipping WHERE order_seq=ord.order_seq AND $domestic_shipping_query LIMIT 1 ) > 0";
	            } else {
	                $str_where_order .= " AND ord.international =  'domestic'";
	            }
	        }else if( array_search('international', $_PARAM['nation']) !== false ){
	            if($_PARAM['shipping_set_code']['international']){
	                $str_where_order .= " AND (SELECT order_seq FROM fm_order_shipping WHERE order_seq=ord.order_seq AND $international_shipping_query LIMIT 1 ) > 0";
	            } else {
	                $str_where_order .= " AND ord.international =  'international'";
	            }
	        }
	    }
	    ...
	    if( $_PARAM['shipping_set_code']['ticket'] ){
	        if (strpos($goodsviewjoin, 'fm_order_item') === false) {
	            $goodsviewjoin .= " INNER join fm_order_item oi on ord.order_seq = oi.order_seq";
	            $groupBy = "GROUP BY ord.order_seq";
	        }
	        $str_where_order .= " AND oi.goods_kind = 'coupon'";
	    }
	    ...
	    if( $_PARAM['goodstype'] ){
	        ...
	        if(array_search('withdraw', $_PARAM['goodstype']) !== false){
	            $goodsviewjoin .= " INNER join fm_goods g on g.goods_seq = oi.goods_seq";
	        }

	        if(array_search('package', $_PARAM['goodstype']) !== false){
	            $goodsviewjoin .= " INNER join fm_order_item_suboption ois on ois.order_seq = ord.order_seq";
	            $groupBy = "GROUP BY ord.order_seq";
	        }
	        ...
	        foreach($_PARAM['goodstype'] as $k => $v){
	            ...
	            if($v == 'adult'){
	                $str_where_order .= "oi.adult_goods = 'Y'";
	            } else if($v == 'withdraw'){
	                $str_where_order .= " g.cancel_type = '1'";
	            } else if($v == 'international_shipping'){
	                $str_where_order .= " oi.option_international_shipping_status = 'y'";
	            } else if($v == 'reserve'){
	                $str_where_order .= " oi.reservation_ship = 'y'";
	            } else if($v == 'package'){
	                $str_where_order .= " ois.package_yn = 'y'";
	            } else if($v == 'gift'){
	                $str_where_order .= " oi.goods_type = 'gift'";
	            } else if($v == 'ticket'){
	                $str_where_order .= " oi.goods_kind = 'coupon'";
	            }
	        }
	        $str_where_order .= ")";
	    }
	    ...
	    if($_PARAM['keyword']){
	        ...
	        if($_PARAM['search_type'] && $_PARAM['search_type'] != "all"){
	            switch($_PARAM['search_type']){
	                case "order_seq":
	                    $str_where_order .= " AND IFNULL( ord.order_seq, '' ) = '{$_PARAM['keyword']}'";
	                    break;

	                case "order_user_name":
	                    $goodsviewjoin .= " LEFT JOIN fm_member mem ON mem.member_seq = ord.member_seq";
	                    if($_PARAM['search_partial'] == true){
	                        $str_where_order .= " AND ( IFNULL( mem.user_name, '' ) like '%{$_PARAM['keyword']}%'
													OR IFNULL( ord.order_user_name,  '' ) like '%{$_PARAM['keyword']}%' )";
	                    } else {
	                        $str_where_order .= " AND ( IFNULL( mem.user_name, '' ) = '{$_PARAM['keyword']}'
													OR IFNULL( ord.order_user_name,  '' ) = '{$_PARAM['keyword']}' )";
	                    }
	                    break;

	                case "depositor":
	                    if($_PARAM['search_partial'] == true){
	                        $str_where_order .= " AND IFNULL( ord.depositor, '' ) like '%{$_PARAM['keyword']}%'";
	                    } else {
	                        $str_where_order .= " AND IFNULL( ord.depositor, '' ) = '{$_PARAM['keyword']}'";
	                    }
	                    break;

	                case "userid":
	                    $goodsviewjoin .= " LEFT JOIN fm_member mem ON mem.member_seq = ord.member_seq";
	                    $str_where_order .= " AND ( IFNULL(mem.userid, '') LIKE  '{$_PARAM['keyword']}%'
													OR IFNULL(ord.npay_order_id, '') LIKE '{$_PARAM['keyword']}%'
													OR IFNULL(ord.linkage_mall_order_id, '') LIKE '{$_PARAM['keyword']}%'
	                ...
	                case "recipient_phone":
	                    $str_where_order .= " AND ( INSTR(ord.recipient_cellphone, '{$_PARAM['keyword']}')
													OR INSTR(ord.order_cellphone, '{$_PARAM['keyword']}')
													OR INSTR(ord.recipient_phone, '{$_PARAM['keyword']}')
													OR INSTR(ord.order_phone, '{$_PARAM['keyword']}') )";
	                    break;

	                case "order_email":
	                    $str_where_order .= " AND IFNULL(ord.order_email, '') LIKE '%{$_PARAM['keyword']}%'";
	                    break;

	                case "recipient_user_name":
	                    if($_PARAM['search_partial'] == true){
	                        $str_where_order .= " AND IFNULL(ord.recipient_user_name,  '') like '%{$_PARAM['keyword']}%'";
	                    } else {
	                        $str_where_order .= " AND IFNULL(ord.recipient_user_name,  '') = '{$_PARAM['keyword']}'";
	                    }
	                    break;
	                ...
	                case "goods_name":
	                    ...
	                    $str_where_order .= " AND IFNULL( oi.goods_name,  '' ) LIKE  '%{$_PARAM['keyword']}%'";
	                    break;
	                ...
	                case "goods_seq":
	                    ...
	                    $str_where_order .= " AND IFNULL( oi.goods_seq,  '' ) = '{$_PARAM['keyword']}'";
	                    break;
					case "bar_code":
						if (strpos($goodsviewjoin, 'fm_order_item') === false) {
							$goodsviewjoin .= " INNER join fm_order_item oi on ord.order_seq = oi.order_seq";
						...
						$str_where_order .= " AND IFNULL( oi.goods_code,  '' ) = '{$_PARAM['keyword']}'";
						break;						
	            }
	        } else {
	            if(preg_match("/^[A-Za-z]*$/", $_PARAM['keyword'])){ //영어만
	                ...
	                $str_where_order .= " AND ( IFNULL(mem.userid, '') LIKE  '{$_PARAM['keyword']}%'
											...
											OR IFNULL(mem.sns_n, '') LIKE '{$_PARAM['keyword']}%'
											OR IFNULL( ord.order_user_name, '' ) like '%{$_PARAM['keyword']}%'
                                            OR IFNULL( ord.recipient_user_name, '' ) like '%{$_PARAM['keyword']}%'
                                            OR IFNULL( oi.goods_name,  '' ) LIKE  '%{$_PARAM['keyword']}%')";
	                ...
	                if (strlen($_PARAM['keyword']) == 19) { //주문번호
	                    ...
	                    $str_where_order .= " AND (IFNULL( oi.goods_code,  '' ) = '{$_PARAM['keyword']}'
                                                OR oi.goods_seq = '{$_PARAM['keyword']}' )";
	                }
	                ...
	                if($_PARAM['search_partial'] == true){
	                    $str_where_order .= " AND ( IFNULL( mem.user_name, '' ) like '%{$_PARAM['keyword']}%'
                                            OR IFNULL( ord.order_user_name, '' ) like '%{$_PARAM['keyword']}%'
                                            OR IFNULL( ord.recipient_user_name, '' ) like '%{$_PARAM['keyword']}%'
											OR IFNULL( ord.depositor, '' ) like '%{$_PARAM['keyword']}%'
                                            OR IFNULL( oi.goods_name, '' ) LIKE  '%{$_PARAM['keyword']}%' )";
	                } else {
	                    $str_where_order .= " AND ( IFNULL( mem.user_name, '' ) = '{$_PARAM['keyword']}'
                                            OR IFNULL( ord.order_user_name, '' ) = '{$_PARAM['keyword']}'
                                            OR IFNULL( ord.recipient_user_name, '' ) = '{$_PARAM['keyword']}'
											OR IFNULL( ord.depositor, '' ) = '{$_PARAM['keyword']}'
                                            OR IFNULL( oi.goods_name, '' ) LIKE  '%{$_PARAM['keyword']}%' )";
	                }
	                ...
	                if($_PARAM['search_partial'] == true){
	                    $str_where_order .= " AND ( IFNULL( ord.npay_order_id,  '' ) like '%{$_PARAM['keyword']}%'
	                    ...
	                    $str_where_order .= " AND ( IFNULL( ord.npay_order_id,  '' ) = '{$_PARAM['keyword']}'
	    ...
	    if( $str_where_order ) {
	        if($_PARAM['query_type'] == 'total_record'){
	            if($groupBy){
	                $query = "SELECT count(A.cnt) as cnt FROM (SELECT
    								count(ord.order_seq) as cnt
    							FROM
    								fm_order ord ".$goodsviewjoin." {$str_where_order} {$groupBy}) as A";
	            } else {
	                $query = "SELECT
    								count(ord.order_seq) as cnt
    							FROM
    								fm_order ord ".$goodsviewjoin." {$str_where_order}";
	            }
	        }else{
	            $query = "SELECT
								ord.order_seq
							FROM
								fm_order ord ".$goodsviewjoin." {$str_where_order} {$groupBy}
							ORDER BY ord.step ASC, ord.regist_date DESC, order_seq
							{$addLimit}";
	        }
	...
	**/
	...
	public function set_order($set_params,$where_params)
	{
		...
		$this->db->update('fm_order', $set_params);
	}
