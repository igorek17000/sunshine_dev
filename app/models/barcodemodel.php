...
class barcodemodel extends CI_Model {
	...
	public function set_barcode_data($param){
		...
		try{
			//상품코드를 UPDATE
			$query		= "SELECT goods_code FROM fm_goods WHERE goods_seq = '".$param['goods_seq']."' ";
			$result_set = $this->db->query($query);
			...
			if($param['goods_code'] && $refer_code != $param['goods_code']){
				$query = "UPDATE fm_goods SET goods_code = '".$this->security->xss_clean($param['goods_code'])."' WHERE goods_seq = '".$param['goods_seq']."' ";
				$this->db->query($query);
			}
			...
			//옵션코드 배열을 쿼리로 변환
			for($i=0; $i<5; $i++){
				$sub_query .= " optioncode".($i+1)." = '".$param['option_code'][$i]."' ";
				if( $i < 4 ) $sub_query .= ",";
			}
			
			$query = "UPDATE fm_goods_option SET ".$this->security->xss_clean($sub_query)." WHERE goods_seq = '".$param['goods_seq']."' AND option_seq = '".$param['option_seq']."'";
			$this->db->query($query);
