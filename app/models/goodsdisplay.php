...
class Goodsdisplay extends CI_Model {
	...
	function checkDesignDisplayCach($display_seq, $tab_idx=0, $perpage = null, $kind = null, $return_data='path'){
	    ...
	    if	(file_exists($file_name)){
	        if( $return_data == 'date' ) return date("Y.m.d H:i:s", filemtime($file_name));
	...
	function rmDesignDisplayCach($display_seq, $tab_idx=0, $perpage = null, $kind = null){
	    $file_name = $this->checkDesignDisplayCach($display_seq, $tab_idx, $perpage, $kind);
	    if( file_exists($file_name) ){
	        unlink($file_name);
	    }
	}
	...
	function getDesignDisplayParams($display_seq, $tab_idx=0, $perpage = null, $kind = null, $user = null){		
		$params = '?display_seq='.$display_seq.'&tab_idx='.$tab_idx;
	...
	function getDesignDisplayCachFileName($display_seq, $tab_idx=0, $params = ''){
		...
		// 접속모드
		$viewMode			= 'P';
		$result	= $display_seq.'_'.$tab_idx.'_'.$member_group.$platform.$viewMode.'_'.md5($params).'.html';        
		return $result;
	}
