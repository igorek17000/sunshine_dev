...
class zipcodemodel extends CI_Model {
	...
	public function zipcode($perpage)
	{
		...
		unset($_GET['zipcode_type']);
		//unset($_GET['old_zipcode']);
		...
		if($wheres){
			...
			if($_GET['SIDO'] || $_GET['SIGUNGU']){
				$wheres = array();

				if($_GET['SIDO']){
					$wheres[] = "SIDO = '".$_GET['SIDO']."'";
				}

				if($_GET['SIGUNGU']){
					$wheres[] = "SIGUNGU = '".$_GET['SIGUNGU']."'";
				}
		...
		if($sidoWheres){
			$arrSigungu = $ZIP_DB->query("select SIGUNGU from ".$this->zipcodeTable." WHERE SIDO = '".$_GET['SIDO']."' AND ".implode(" AND ", $sidoWheres)." GROUP BY SIGUNGU");
		}else{
			$arrSigungu = $ZIP_DB->query("select SIGUNGU from ".$this->zipcodeTable." WHERE SIDO = '".$_GET['SIDO']."' GROUP BY SIGUNGU");
		}
