...
class Board_process extends selleradmin_base {
	...
	public function index()
	{
			...
			if($result) {
				...
				if( defined('__SELLERADMIN__') === true ) {
					if( BOARDID == 'gs_seller_qna' ) {//입점사문의게시판
						...
						$this->load->model('membermodel');
						//sendSMS($_POST['tel1'], $sendsmscase, $this->providerInfo['provider_id'] , '');
						...
						push_for_admin(array(
							'kind'			=> 'gs_seller_qna',
							'unique'		=> $newseq,
							'provider_name'	=> $this->providerInfo['provider_name']
						));
					}
				}
				if($_POST['backtype'] == 'list') {
					...
					$callback = ($_POST['returnurl'])?"parent.boardaddFormDialog('".$_POST['returnurl']."&seq=".$newseq."', '80%', '800', '".$manager['name']." 게시글 보기','false');":"parent.boardaddFormDialog('".$this->Boardmanager->realboardviewurl.BOARDID."&seq=".$newseq."', '80%', '800', '".$manager['name']." 게시글 보기','false');";
				} else {
		...
		elseif($mode == 'board_modify') {
            ...
            // 비밀번호 유효성 체크
            $pre_enc_password = $_POST['oldpw'];
            $enc_password = '';

            if ($pre_enc_password != $_POST['pw']) {
                $check_password = $_POST['pw'];
                $password_params = array(
                    'birthday'                => '',
                    'phone'                   => '',
			...
			### 본인상품 검색 보안추가 :: 2017-08-17 lwh
			if( BOARDID == 'goods_qna'  ||  BOARDID == 'goods_review' ) {
				$this->load->model('goodsmodel');
				$goods = $this->goodsmodel->get_board_for_provider(BOARDID, $_POST['seq']);
			...
			if($parentdata['mseq'] == '-1'  && !($_POST['reply'] == 'y' && BOARDID == 'goods_qna') ){
				pageReload('입점사관리자는 권한이 없습니다.', 'parent');
				exit;
			}
			...
			if( defined('__SELLERADMIN__') === true ) {
				if( $_POST['reply'] == 'y' && ( BOARDID == 'mbqna' || BOARDID == 'goods_qna'  || BOARDID == 'bulkorder'  || BOARDID == 'gs_seller_qna' ) ) {
				unset($params['contents'], $params['subject']);//답변시 원본글  제목,내용 변경안되도록
				}
			}else{
				if( $_POST['reply'] == 'y' && ( BOARDID == 'mbqna' || BOARDID == 'goods_qna'  || BOARDID == 'bulkorder' ) ) {
					unset($params['contents'], $params['subject']);//답변시 원본글  제목,내용 변경안되도록
				}
			}
			...
			if($result) {
				...
				if( ( BOARDID == 'gs_seller_qna' ) &&  !empty($_POST['board_email']) && !empty($_POST['board_sms_email']) ) {//입점문의 게시판
				...
				if($_POST['mainview']) {
					...
					if($_POST['backtype'] == 'list') {
						...
						$callback = ($_POST['returnurl'])?"parent.boardaddFormDialog('".$_POST['returnurl']."', '80%', '800', '".$manager['name']." 게시글 보기','false');":"parent.boardaddFormDialog('".$this->Boardmanager->realboardviewurl.BOARDID."&seq=".$_POST['seq']."', '80%', '800', '".$manager['name']." 게시글 보기','false');";
					}else {
						$callback = '';
					}
				}
		...
		elseif($mode == 'board_delete') {
			...
			if($parentdata['mseq'] == '-1'){
				//$callback =  "parent.document.location.reload();";
				//openDialogAlert('입점사관리자는 권한이 없습니다.',400,140,'parent',$callback);
				echo 'auth';
				exit;
			}
