...
class goods extends board {
	...
	protected function _catalog_light()
	{
		...
		/* 접근 제한 */
		$this->check_category_member($sCategory, $categoryData);
		...
		if( $aSearch['category'] ){ // 검색조건의 상품번호 추출
	...
	protected function _brand_light()
	{
		...
		if( $aSearch['brand'] ){ // 검색조건의 상품번호 추출
		...
		if( $aFilterConfig['brand'] ){
			$aBrands	= $this->goodslistmodel->brandsForFilter($sGoodsQuery, 'brand');
		}
		if($aFilterConfig['seller']){
			$aProviders	= $this->goodslistmodel->providersFilter($sGoodsQuery);
		}
	...
	public function view(){
		...
		if	($result['status'] == 'error'){
			...
			// 공용정보 이미지 분할처리
			$mobile_contentscnt = preg_match_all("/<IMG([^>]*)src[\s]*=[\s]*([\"']?)([^>\"']+)[\"']?([^>]*)>/i",$result['assign']['goods']['common_contents'], $matches);
			if(count($matches[0])>0){
				$result['assign']['goods']['common_contents_images_true'] = true;
				$result['assign']['goods']['common_contents_images'] = array();
				foreach($matches[0] as $k=>$v){
					$v = preg_replace("/([\s]*)src([\s]*=)/i"," data-original=",$v);
					$result['assign']['goods']['common_contents_images'][] = $v;
				}
			}
			...
			if ($this->config_system['facebook_pixel_use'] == 'Y') {
			    ...
			    if ($goods['feed_ship_type'] != "E") { //그룹배송비 설정을 따를 경우
			        ...
			        } else if ($goods['feed_pay_type'] == 'free'){
			            $shipping = 0;
			        }
			    }
	...
	public function view_common_contents(){
		...
		$commonpreload = false;
		$skin_configuration = skin_configuration($this->skin);
		if	(!empty($skin_configuration['patch_version'])) $commonpreload = true;

		$this->template->assign(array('commonpreload'=>$commonpreload));
		$this->template->assign(array('goods'=>$goods));

		$commonpreload = false;
		if	($_GET['view_preload']) $commonpreload = true;
		$this->template->assign(array('commonpreload'=>$commonpreload));
	...
	protected function _search_light()
	{
		...
		if(!$aParams['sorting'])	$aParams['sorting']	= $aFilterConfig['orderby'];
		if(!in_array($aParams['per'], $aDefaultPerpages))	$aParams['per']	= 40;
		if($aParams['search_text'])	$sSearchText	= $aParams['search_text'];
		if($aParams['ship_grp_seq'])	$sShipGrpSeq	= $aParams['ship_grp_seq'];
	...
	public function search_list()
	{
		...
		if( $aParams['searchMode'] == 'event_view' && $aParams['event'] ){
		...
		// 검색 조건 파라미터
		$aSearch['keyword']		= $sSearchText;
		$aSearch['shop_group']	= $sShipGrpSeq;
		$aSearch['category']	= $aParams['category'];
		$aSearch['event']		= $aParams['event'];
		$aSearch['gift']		= $aParams['gift'];
		$aSearch['brand']		= $aParams['brand'];
		$aSearch['location']	= $aParams['location'];
		$aSearch['color']		= $aParams['color'];
		$aSearch['delivery']	= $aParams['delivery'];
		$aSearch['provider']	= $aParams['provider'];
		$aSearch['re_search']	= $aParams['re_search'];
		$aSearch['min_price']	= $aParams['min_price'];
		$aSearch['max_price']	= $aParams['max_price'];
		$aSearch['searchMode']	= $aParams['searchMode'];
		$aSearch['searchLimit']	= $aParams['searchLimit'];
		if( $aParams['member_seq'] ){
			$aSearch['member_seq']	= $aParams['member_seq'];
		}
		$aSearch['platform']	= 'P';
		if( $this->mobileMode || $this->_is_mobile_agent ){
			$aSearch['platform']	= 'M';
		...
		switch($aParams['searchMode']){
			...
			case "search":
				if(!$sSearchText) $requireErr = true;
				break;
			case "catalog" :
				if(!$aSearch['category']) $requireErr = true;
				break;
			case "brand" :
				if(!$aSearch['brand']) $requireErr = true;
				break;
			case "location" :
				if(!$aSearch['location']) $requireErr = true;
				break;
		}

		if( !$requireErr ){ // 필수 검색 조건이 있을 경우 상품 조회
...
//			$this->goodsdisplay->set('title',$display['title']);
	...
	protected function _location_light()
	{
		...
		if(!$aParams['page'])		$aParams['page']	= 1;
		if(!$aParams['sorting'])	$aParams['sorting']	= 'ranking';
		if(!in_array($aParams['per'], $aDefaultPerpages))	$aParams['per']	= 40;
		if($aParams['code']){
	...
	{
		$aParams = $this->input->get();
		if ($aParams['searchMode'] == 'catalog' || $aParams['searchMode'] == 'brand' || $aParams['searchMode'] == 'location') {
