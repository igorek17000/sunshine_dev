...
class prints extends front_base {
	...
	function print_cart(){
		...
		$tax_price	= 0;	//과세금액
		$free_price = 0;	//비과세금액
		$shipping_shop_cnt			= 0;	//기본배송 상품 건수
		...
		foreach($cart['list'] as $key => $data){
			...
			if($data['tax'] == "exempt"){
				...
				if($data['shipping_policy'] == "goods"){	//비과세 배송비
					$free_price += $this->cart['data_goods'][$data['goods_seq']]['goods_shipping'];
				}
			}else{
				if($data['shipping_policy'] == "shop") $shipping_shop_cnt++;
			}
		...
		if($shipping_shop_cnt == 0){
			$free_price += $this->cart['shop_shipping_policy']['price'];
		}
