...
class order extends front_base {
	...
	public function modify_shipping_changes(){
		...
		$ship_grp_seq	= $_POST['ship_grp_seq'];
		$ship_set_seq	= $_POST['ship_set_seq'];
		$ship_set_code	= $_POST['ship_set_code'];
		$prepay_info	= $_POST['prepay_info'];
		$hop_date		= $_POST['hop_select_date'];
		$store_seq		= $_POST['store_seq'];
		$cart_table		= $_POST['cart_table'];
		$admin_mode		= $_POST['admin_mode'];
...
*/
	...
	public function pay(){
		...
		**/
		...
		/* skin-order-settle(-mobile).js 파일에서 체크함 // 2018-01-29
		if($_POST['order_version'] >= 2){
			// 개인정보 제3자 제공 동의 체크
			if(serviceLimit('H_AD') && $_POST['agree3'] != 'Y'){
				//개인정보 제3자 제공에 동의하셔야 합니다.
				openDialogAlert(getAlert('os147'),400,140,$popupKind,$pg_cancel_script);
				if($popupKind == 'opener') {
					echo "<script>window.close();</script>";
				}
				exit;
			...
			// 개인정보 제3자 제공 동의 체크
			if(serviceLimit('H_AD') && $_POST['agree2'] != 'Y'){
				//개인정보 제3자 제공에 동의하셔야 합니다.
				openDialogAlert(getAlert('os147'),400,140,$popupKind,$pg_cancel_script);
				if($popupKind == 'opener') {
					echo "<script>window.close();</script>";
		...
		if($is_goods){
			...
			if($_POST['international'] == 0){
				...
				if($_POST['recipient_new_zipcode']){
					//우편번호
					$this->validation->set_rules('recipient_new_zipcode', getAlert('os052'),'trim|required|max_length[7]|xss_clean');
				}else{
					//우편번호
					$this->validation->set_rules('recipient_zipcode[]', getAlert('os052'),'trim|required|max_length[7]|xss_clean');
				}
				//주소
				$this->validation->set_rules('recipient_address', getAlert('os053'),'trim|max_length[255]|required|xss_clean');
				//나머지주소
				...
				//지역
				//$this->validation->set_rules('region', getAlert('os058'),'trim|numeric|required|xss_clean');
				//주소
