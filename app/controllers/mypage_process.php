	...
	public function order_return(){
		...
		foreach($aParams['chk_ea'] as $k => $return_apply_ea){
			...
			if($cfg_order['buy_confirm_use']){
				...
				## 반품수량
				if(!$suboption_seq) $return_item = $this->returnmodel->get_return_item_ea($item_seq,$option_seq);
					else $return_item = $this->returnmodel->get_return_subitem_ea($item_seq,$suboption_seq);
...
//					$title .= "<br/> ".number_format($socialcp_return_use)."건의 티켓상품은 제외되었습니다.";
	...
	public function recipient(){
		...
		if(!$this->shippingmodel->shipping_group_exists($shipping_group_seq)) { // 배송그룹 존재 유무
			if($mode == 'json') {
					echo json_encode(array("state"=>"999", "error_message"=>getAlert('mo147')));
				} else {
		...
		if(!$this->shippingmodel->shipping_group_set_exists($shipping_group_seq, $shipping_set_seq, $shipping_set_code)) { // 배송그룹셋(이하 배송방법)의 존재 유무
			if($mode == 'json') {
					echo json_encode(array("state"=>"999", "error_message"=>getAlert('mo148')));
				} else {
		...
		// 배송지변경시 국내->해외, 해외->국내 변경 처리불가 메세지 표출(2017-06-28 오후 4:09  가비아씨엔에스 채우형)
		if($international != $_POST['international']) {
			if($mode == 'json') {
				echo json_encode(array("state"=>"999", "error_message"=>getAlert('mo146')));
			} else {
		...
		if( !in_array($orders['step'],$this->ordermodel->able_step_action['shipping_region']) ){
			//$this->arr_step[$orders['step']]."에서는 배송정보 변경을 하실 수 없습니다."
			if($mode == 'json') {
				echo json_encode(array("state"=>"999", "error_message" => getAlert('mo008',$this->arr_step[$orders['step']])));
			} else {
				openDialogAlert(getAlert('mo008',$this->arr_step[$orders['step']]),400,140,'parent',"");
			}
			exit;
		}

		//받는이
		$this->validation->set_rules('recipient_user_name',getAlert('mo009'),'trim|required|xss_clean');
		//전화
		//$this->validation->set_rules('recipient_phone[]',getAlert('mo010'),'trim|numeric|required|xss_clean');
		//휴대폰
		if(is_array($_POST['recipient_cellphone'])) {
			$this->validation->set_rules('recipient_cellphone[]',getAlert('mo011'),'trim|numeric|required|xss_clean');
		} else {
			$this->validation->set_rules('recipient_cellphone',getAlert('mo011'),'trim|required|xss_clean');
		}
		//요청사항
		$this->validation->set_rules('memo',getAlert('mo012'),'trim|xss_clean');

		if($shipping_set_code != 'coupon') { // 쿠폰상품 제외한 상품만 주소 체크
			if($international == 'domestic'){
				if($_POST['recipient_new_zipcode']){
					//우편번호
					$this->validation->set_rules('recipient_new_zipcode',getAlert('mo017'),'trim|required|xss_clean');
				}else{
					$recipient_zipcode1			= $_POST['recipient_zipcode'][0];
					$recipient_zipcode2			= $_POST['recipient_zipcode'][1];
					$recipient_address_street	= $_POST['recipient_address_street'];
					...
					if( $recipient_zipcode1 && $recipient_zipcode2 && strlen($recipient_zipcode1) != 5 ) {
						//우편번호
						if(is_array($_POST['recipient_zipcode'])) {
							$this->validation->set_rules('recipient_zipcode[]',getAlert('mo017'),'trim|required|xss_clean');
						} else {
							$this->validation->set_rules('recipient_zipcode',getAlert('mo017'),'trim|required|xss_clean');
						}
					}
				...
				//주소
				$this->validation->set_rules('recipient_address',getAlert('mo013'),'trim|required|xss_clean');
				//주소
				$this->validation->set_rules('recipient_address_detail',getAlert('mo013'),'trim|required|xss_clean');
			}

			if($international == 'international'){
				//지역
				#$this->validation->set_rules('region',getAlert('mo014'),'trim|required|xss_clean');
				//주소
				$this->validation->set_rules('international_address',getAlert('mo013'),'trim|required|xss_clean');
				//시도
				$this->validation->set_rules('international_town_city',getAlert('mo015'),'trim|required|xss_clean');
				//주
				$this->validation->set_rules('international_county',getAlert('mo016'),'trim|required|xss_clean');
				//우편번호
				$this->validation->set_rules('international_postcode',getAlert('mo017'),'trim|required|xss_clean');
				//국가
				$this->validation->set_rules('international_country',getAlert('mo018'),'trim|required|xss_clean');
			}

			// 국가, 주소 배송가능여부 체크
			switch($international) {
				case 'domestic': $ship_possible = $this->shipping->check_shipping_possible($order_seq, $international, $orders['international_country'], $_POST['recipient_address_street'], $_POST['recipient_address']); break;
				case 'international': $ship_possible = $this->shipping->check_shipping_possible($order_seq, $international, $_POST['international_country'], '', ''); break;
			}
		...
		// 개인통관고유부호 체크
		if($_POST['clearance_unique_personal_code']){
		...
		is_array($_POST['recipient_phone'])      && $_POST['recipient_phone'] = implode('-',$_POST['recipient_phone']);
		is_array($_POST['recipient_cellphone']) && $_POST['recipient_cellphone'] = implode('-',$_POST['recipient_cellphone']);
		...
		$_POST['memo'] && ($data['memo'] = $_POST['memo']);
		...
		if($international == 'domestic'){
			...
			if($change){
				if($_POST['recipient_new_zipcode']){
					$data['recipient_zipcode'] 				= $_POST['recipient_new_zipcode'];
				}else{
					$data['recipient_zipcode'] 				= $_POST['recipient_zipcode'][0].$_POST['recipient_zipcode'][1];
				}
		...
		if($international == 'international'){
			$data['nation_key'] = $this->shippingmodel->get_gl_nation_key(preg_replace('/^[가-힣 ]+?\(([a-z.]+)\)/Ui', '$1', $_POST['international_country']));

			foreach($_POST as $k => $row) if($orders[$k]!=$data) $change = 1;
			if($change){
				# 현재 사용하지 않는 것으로 확인되어 주석처리함(2017-01-19 채우형)
		...
		$_POST['recipient_email'] && ($data['recipient_email'] = $_POST['recipient_email']);
		$_POST['clearance_unique_personal_code'] && ($data['clearance_unique_personal_code'] = $_POST['clearance_unique_personal_code']);
