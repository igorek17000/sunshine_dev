...
## 자동로그인 s #############################################################################
	...
	function memberInfoSave() {
		...
		if(!$this->userInfo['member_seq']) {
			$query = "select A.*,B.business_seq,B.bname,C.group_name from fm_member A LEFT JOIN fm_member_business B ON A.member_seq = B.member_seq left join fm_member_group C on C.group_seq=A.group_seq where A.member_seq = '".$_POST['member_seq']."' ";
			$query = $this->db->query($query);
			$data = $query->result_array();
			if(!$_POST['member_seq'] || !$data[0]['member_seq'] ) { //로컬에도 값이 없거나 디비상에 없는 경우
				$this->autoLogOut();					
			}else{
				if($_POST['key'] != $data[0]['api_key'] || $data[0]['status']!='done' ) {
...
//			echo "<script>alert('".$msg."');</script>";
	...
	*/
	function api_sns_login($memberseq, $api_key){
		...
		//$sql = "select A.*, C.group_name from fm_member A LEFT JOIN fm_member_group C on C.group_seq=A.group_seq where (A.status = 'done' or A.status = 'dormancy') and A.".$snstype." = '".$params[$snstype]."'";	    
	    $sql = "select A.*, C.group_name from fm_member A LEFT JOIN fm_member_group C on C.group_seq=A.group_seq where (A.status = 'done' or A.status = 'dormancy') and A.member_seq = '".$memberseq."' ";
...
// 일반 s
	function api_normal_login() {
		...
		*/
		...
		$query = "select A.*,B.business_seq,B.bname,C.group_name from fm_member A LEFT JOIN fm_member_business B ON A.member_seq = B.member_seq left join fm_member_group C on C.group_seq=A.group_seq where A.member_seq = '".$_POST['member_seq']."' and status!='withdrawal' ";		// 탈퇴회원인 경우 로그인 못하도록 체크
		$query = $this->db->query($query);
		$data = $query->result_array();
				
		$db_api_key = $data[0]['api_key'];
		$app_api_key = isset($_POST['key']) ? $_POST['key'] : "";

		if( is_null($db_api_key) || $db_api_key == "" ) {
		    # api_key 생성 및 db update
		    $db_api_key = $this->appmembermodel->create_api_key($data[0]['userid'], $data[0]['member_seq'], true);	
		}else if( $app_api_key != $db_api_key && $app_api_key != "") {
			//일치하는 회원정보가 없습니다.
			$return['result'] = '0';
			$return['msg'] = getAlert('mb203');
