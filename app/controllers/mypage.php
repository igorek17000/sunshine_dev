...
class mypage extends board {
	...
	public function index()
	{
		...
		$orders = $this->ordermodel->get_order_list(array('perpage'=>'5','member_seq'=>$this->userInfo['member_seq'],'hidden'=>'N','step_type'=>'non_attempt', 'regist_date'=>array(date('Y-m-d', strtotime('-1 month'))) ));
		...
		foreach($orders['record'] as $k => $data_order){
			...
			$reserve			= 0;
			$point				= 0;
			foreach($items as $k1 => $data_order_item){
				$items[$k1]['options']		= $this->ordermodel->get_option_for_item($data_order_item['item_seq']);
				$items[$k1]['suboptions'] = $this->ordermodel->get_suboption_for_item($data_order_item['item_seq']);

				foreach($items[$k1]['options'] as $k2 => $data_order_item_option){
					$items[$k1]['options'][$k2]['inputs']	= $this->ordermodel->get_input_for_option($data_order_item_option['item_seq'], $data_order_item_option['item_option_seq']);

					$items[$k1]['options'][$k2]['suboptions'] = $this->ordermodel->get_suboption_for_option($data_order_item_option['item_seq'], $data_order_item_option['item_option_seq']);
				...
				foreach($items[$k1]['suboptions'] as $k2 => $data_order_item_suboption){
			...
			if	(!serviceLimit('H_AD'))	unset($data_order['provider_name']);
		...
		$cfg_goodsImageSize	= ($this->goodsImageSize) ? $this->goodsImageSize : config_load('goodsImageSize');
		...
		if	($eainfo) foreach($eainfo as $order_seq => $data){
		...
		$returns = $this->returnmodel->get_return_list($sc);
		$counts['return'] = $returns;
		$counts['refund'] = $refunds;

		$this->template->assign(array('counts'=>$counts));
	...
	public function order_catalog()
	{
		...
		if($this->realMobileSkinVersion > 2 && $this->mobileMode && $aGetParams['sc_date'] != 'direct'){
		    if	($aGetParams['sc_date'] == '')	$aGetParams['sc_date']	= 3;	// 기본검색 3주
		...
		**/
		$sc=array();
		$sc['page']				= (!empty($_GET['page'])) ?		(int) $_GET['page']:'1';
		$sc['perpage']			= $perpage ? $perpage : 10;
		$sc['member_seq']		= $this->userInfo['member_seq'];
		$sc['keyword']			= $aGetParams['keyword'];
		$sc['regist_date'][0]	= $aGetParams['sc_sdate'] ? $aGetParams['sc_sdate'] : $aGetParams['regist_date'][0];
		$sc['regist_date'][1]	= $aGetParams['sc_edate'] ? $aGetParams['sc_edate'] : $aGetParams['regist_date'][1];
		$sc['hidden']			= 'N';
		$sc['step_type']		= $aGetParams['step_type']=='export' ? 'export_and_complete' : $aGetParams['step_type'];

		if (!$sc['step_type'])  $sc['step_type'] = 'non_attempt';
...
*/
			if($data_order['step'] > 40){
				...
				foreach( $data_order['exports'] as $exk => $data_export ){
					...
					$data_export['item'] =  $this->exportmodel->get_export_item($data_export['export_code']);
					if($data_export['item']['buy_confirm_use'] !== true) {
					    $data_order['buy_confirm_use'] = ($data_export['buy_confirm_use'] ? true : false);
					}
...
//			$orders['tax_receipt_possible_txt'] = '(' . date('Y년 m월 d일', strtotime($tax_limit_date)) . '까지 신청가능)';
		...
		foreach($items as $key=>$item){
			...
			foreach($data_shipping as $group_key => $row_shipping){
				...
				if($row_shipping['shipping_method'] == 'coupon'){
					...
					$ship_info = $this->shippingmodel->get_ship_info($row_shipping['shipping_group'],$row_shipping['shipping_method']);
			...
			if($options) foreach($options as $k => $data){
				...
				if($orders['each_msg_yn'] == 'Y'){
					...
					*/
					...
					$ship_message['goods_option'][$item_option_seq]	= $this->goodsmodel->optionToStr($data);
				}
		...
		foreach($shipping_group_items as $shipping_seq=>$row){
			...
			if($shipping['shipping_hop_date'] != '' || $shipping['reserve_sdate'] != '') {
				$this->template->assign('hop_reserve_date','예약'); //view페이지에서는 언어별로 수동 입력
			}
		...
		//구매확정 버튼 노출 : 출고별 마일리지 지급예정수량이 있을때만 노출(buy_confirm 사용안함)
		$exports_tot	= array();
		$export_cnt		= $buy_confirm_cnt = $able_return_ea_ck = 0;
		foreach( $exports as $k => $data_export ){
			...
			foreach( $data_export['item'] as $i=>$data){
				...
				if($data['goods_type'] == "gift"){
					$giftlog = $this->giftmodel->get_gift_title($order_seq,$data['item_seq']);
			...
			if($this->cfg_order['buy_confirm_use']){
				...
				$order_return_edate = date('Ymd',strtotime('+'.$this->cfg_order['save_term'].' day',strtotime($data_export['complete_date'])));
				...
				$order_return_sdate = date('Ymd',strtotime('+'.$this->cfg_order['save_term'].' day',strtotime($data_export['shipping_date'])));
			...
			$buyconfirmInfo = $this->buyconfirmlib->check_buyconfirm($data_export, $data_export['item']);
	...
	public function order_return(){
		...
		foreach( $exports as $k => $data_export ){
			...
			foreach($data_export['item'] as $i=>$data){
				...
				// 교환배송비 설정 :: 2018-07-03 lwh
				if($type == 'exchange'){
					$data['refund_shiping_cost'] = $data['swap_shiping_cost'];
					unset($data['swap_refund_shiping_cost']);
				}

				// 무료배송여부가 잘못 노출되어 배송비가 0 이상인 경우 N으로 처리 :: 2018-07-25 pjw
				$data['shiping_free_yn'] = $data['shipping_cost'] == 0 ? 'Y' : 'N';
				if($data['shiping_free_yn'] == 'Y'){
					$data['refund_shiping_cost'] = $data['refund_shiping_cost'] * 2;
				}
