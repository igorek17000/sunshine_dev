...
class common extends admin_base {
	...
	protected function refine_sort($data = array(), $type = ''){
		...
		if($same_sort_cnt>1){
			...
			$make_step_data = array();
			$make_step_sort = $check_same_sort;
			foreach($same_sort as $same_sort_idx=>$row){
				$make_step_data = array($sort_var => $make_step_sort);
				$this->db->where(array('goods_seq'=>$row['goods_seq'],'category_code'=>$code));
	...
	관리자UI/UX 스킨 타입 저장
	*/
	public function setSkinSwipeToggle() {
		$this->load->model('admin_menu');
		$return  = $this->admin_menu->setAdminSkinType($this->input->post("skinType"));
