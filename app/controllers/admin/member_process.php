...
class member_process extends admin_base {
	...
	public function joinform_sns_update(){
		...
		// 페이스북 값 체크
		if( $_POST['snsmode'] == 'facebook' && $_POST['use_f'] && (!$_POST['key_f'] || !$_POST['secret_f']  || !$_POST['name_f']) ){
			$msg	= '페이스북의 설정값을 정확히 입력해 주세요.';
			$result = false;
		}

		// 트위터 값 체크
		if( $_POST['snsmode'] == 'twitter' && $_POST['use_t'] && (!$_POST['key_t'] || !$_POST['secret_t']) ){
			$msg = '트위터의 설정값을 정확히 입력해 주세요.';
			$result = false;
		}

		// 카카오 값 체크
		if( $_POST['snsmode'] == 'kakao' && $_POST['use_k'] && !$_POST['key_k'] ){
			if (($_POST['type_k'] == 'rest' && !$_POST['rest_key_k']) || ($_POST['type_k'] == 'js' && !$_POST['key_k'])) {
		...
		// 애플 값 체크
		if( $_POST['snsmode'] == 'apple' && $_POST['use_a'] == '1' && (!trim($_POST['key_a']) || !trim($_POST['team_a']) || !trim($_POST['clientid_a']) || !trim($_POST['keyfile_a'])) ){
		...
		if($result){
			...
			$snssocialar['secret_f']	= $_POST['secret_f'];
			$snssocialar['name_f']		= $_POST['name_f'];
			$snssocialar['type_f']		= $_POST['type_f'];

			// 페이스북 전용앱 여부에 따라 분기처리
			if( $_POST['key_f'] != '455616624457601' ) {
				if($_POST['sns_req_type'])	$snssocialar['sns_req_type']	= $_POST['sns_req_type'];
				if($_POST['domain_f'])		$snssocialar['domain_f']		= $_POST['domain_f'];
			}
			...
			// 카카오 설정
			$snssocialar['use_k']							= $_POST['use_k'];
			$snssocialar['type_k']							= $_POST['type_k'];
			$snssocialar['key_k']							= $_POST['key_k'];
			$snssocialar['rest_key_k']						= $_POST['rest_key_k'];
			$snssocialar['kakaotalk_app_javascript_key']	= $_POST['key_k'];
			$joinformar['use_k']							= $_POST['use_k'];
	...
	public function sms_pop(){
		...
		if($aParams['send_sms']=='Y'){
			...
			if($aParams['board_id']) {
				$cellphone = explode(",",$aParams['cellphone']);
				$idx = 0;
				...
				$dataTo[0]["phone"] = $aParams['cellphone'];
			}

			if($aParams['board_id']) {
				$phone = explode(",",$aParams['cellphone']);
			}else{
