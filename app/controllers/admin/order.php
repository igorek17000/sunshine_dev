	...
	public function catalog_ajax(){
		...
		if ($connector->isConnectorUse()){
			...
			/*if($MarketLinkage["shopCode"] == "shoplinker"){
				$this->load->model('connectormodel');
				$marketList = array();
				$useMarketList = $this->connectormodel->getLinkageMarketGroup();
				foreach($useMarketList as $marketInfo){
					$marketList[$marketInfo['marketCode']] = array('name'=>$marketInfo['marketName']."(샵링커)",'productLink'=>'');
				}
