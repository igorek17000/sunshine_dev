...
class refund_process extends admin_base {
	...
	public function refund_new_save(){
		...
		if($npay_use && $data_order['pg'] == "npay"){
			...
			$cash_refund_shipping_price	= 0;	// 환급해줄 이머니 계산
			...
			if($refund_pg_price > 0 || $refund_pg_delivery > 0){
				...
				if($refund_pg_price_sum < 0) {
					$cash_refund_shipping_price = abs($refund_pg_price_sum);
					$refund_pg_price_sum		= 0;
				}
		...
		if(!$npay_use && $data_refund['status']!='complete' && $aPostParams['status']=='complete')
		{
			...
			if($data_refund['refund_emoney']) $return_emoney_pay = true; else $return_emoney_pay = false;
			if($data_refund['refund_cash'] > 0 || $aPostParams['refund_method'] == "cash") $return_cash_pay = true; else $return_cash_pay = false;

			// 실제 환불되는 예치금은 총액에서 조정금액을 제외한 금액 by hed
			if($return_cash_pay) $tmp_refund_cash = $data_refund['refund_cash'] - $cash_refund_shipping_price; else $tmp_refund_cash = 0;
			...
			*/
			...
			if($return_cash_pay){
				/*
				/*
				if($aPostParams['refund_method'] == "cash"){
					$data_refund['refund_cash'] += array_sum($aPostParams['refund_goods_price']) + $refund_delivery;
				}
