...
class export extends admin_base {
	...
	public function catalog()
	{
		...
		if( count($_GET) == 0 ){
			...
			$data_search_default_str = $this->searchdefaultconfigmodel->get_search_default_config('admin/export/catalog');
	...
	public function catalog_ajax()
	{
		...
		if	($query){
			...
			foreach($query->result_array() as $k => $data){
				...
				$delivery_company_array = array_merge($data['delivery_company_array']);
				...
				if($data['international'] == 'domestic'){
					if($data['domestic_shipping_method'] == 'delivery'||$data['domestic_shipping_method'] == 'postpaid'){
						...
						if($data['delivery_number']) {
							if($data['mdelivery']=='대신택배'){ // 대신택배일 경우 예외 추가 2016-11-03 nsg
				...
				if($data['is_bundle_export'] == 'Y'){
					$bundle_export	= $this->exportmodel->get_export_bundle($data['bundle_export_code']);
					$data['export_code']		= $data['bundle_export_code'];
					$data['bundle_order_list']	= array_values(array_unique($bundle_export['bundle_order_info']));
				}
	...
	public function view()
	{
		...
		foreach($data_export_item as $k => $data){
			...
			if( $data['opt_type'] == 'opt' ){
				...
				// 물류관리 창고 정보 추출
				if	($this->scm_cfg['use'] == 'Y' && $data['provider_seq'] == 1 && $data_export['wh_seq'] > 0){
					...
					$optionData					= $this->goodsmodel->get_goods_option($data['goods_seq'], $sc);
					if	($optionData[0]['option_seq'] > 0){
						$optionStr				= $data['goods_seq'] . 'option' . $optionData[0]['option_seq'];
						$whinfo					= $this->scmmodel->get_warehouse_stock($data_export['wh_seq'], 'optioninfo', '', array($optionStr));
						$data['whinfo']			= $whinfo[$optionStr];

						$data_export['wh_name']	= $data['whinfo']['wh_name'];
					}
					$data['whinfo']['wh_seq']	= $data_export['wh_seq'];
				}
				...
				if	($this->scm_cfg['use'] == 'Y' && $data['provider_seq'] == 1 && $data_export['wh_seq'] > 0){
					unset($sc);
					if	($data['title'])	$sc['suboption_title']	= $data['title'];
					if	($data['option1'])	$sc['suboption']		= $data['option1'];
					$optionData					= $this->goodsmodel->get_goods_suboption($data['goods_seq'], $sc);
					if	($optionData[0]['suboption_seq'] > 0){
						$optionStr				= $data['goods_seq'] . 'suboption' . $optionData[0]['suboption_seq'];
						$whinfo					= $this->scmmodel->get_warehouse_stock($data_export['wh_seq'], 'optioninfo', '', array($optionStr));
						$data['whinfo']			= $whinfo[$optionStr];
						$data_export['wh_name']	= $data['whinfo']['wh_name'];
					}
	...
	public function batch_status(){
		...
		$delivery_company_array			= config_load('delivery_url');
		$international_company_array	= get_international_company();
		$this->template->assign( array('delivery_company_array' => $delivery_company_array, 'international_company_array' => $international_company_array) );
		...
		// 기본 배송책임 지정
		if(!$_GET['provider_seq'] && !$_GET['seq']){
			if(($_GET['start_search_date'] == '') || ($_GET['end_search_date'] == '')) {
				$_GET['start_search_date'] = $start_date;
				$_GET['end_search_date'] = $end_date;
			}
		}
		...
		## 출고코드 검색일때는 다른 조건없이 출코코드로만 조회. @2015-12-15 pjm
		if($_GET['search_type'] == "export_code" && $_GET['keyword']){
			unset($_GET['start_search_date']);
			unset($_GET['end_search_date']);
			//unset($_GET['status']); // 상태값 존재 시 출고상태 변경이 가능 :: 2015-12-23 lwh
			unset($_GET['step']);
		}
		...
		if($params){
			...
			foreach($result_page['record'] as $data_export_code){
				...
				foreach($query->result_array() as $data){
					...
					if($data['option_seq']){
						$data_option_stock = $this->goodsmodel->get_goods_option_stock($data['goods_seq'],$data['option1'],$data['option2'],$data['option3'],$data['option4'],$data['option5'],2);
					...
					if( $data['package_yn'] == 'y' ){
						...
						$data['packages'] = $this->orderpackagemodel->get_suboption($data['suboption_seq']);
					}
		...
		$this->template->assign('openmarket_return_order',$openmarketReturnOrder);
		...
		*/
		...
		for($i=0;$i<count($ordarr);$i++){
			...
			foreach((array)$export_order as $order_seq){
				...
				if(is_array($order_info) === false)	$order_info	= $orders;
			...
			unset($data_arr['order_list']);
			$data_arr['order_list']				= $order_list;
			$data_arr['order']					= $order_info;
			...
			if	($this->scm_cfg['use'] == 'Y'){
				if	($data_export_item) foreach($data_export_item as $k => $exitem){
					if	($exitem['provider_seq'] == 1){
						if	($exitem['opt_type'] == 'opt'){
							unset($sc);
							if	($exitem['option1'])	$sc['option1']		= $exitem['option1'];
							if	($exitem['option2'])	$sc['option2']		= $exitem['option2'];
							if	($exitem['option3'])	$sc['option3']		= $exitem['option3'];
							if	($exitem['option4'])	$sc['option4']		= $exitem['option4'];
							if	($exitem['option5'])	$sc['option5']		= $exitem['option5'];
							$optionData			= $this->goodsmodel->get_goods_option($exitem['goods_seq'], $sc);
							if	($optionData[0]['option_seq'] > 0){
								$optionStr		= $exitem['goods_seq'] . 'option' . $optionData[0]['option_seq'];
								$whinfo			= $this->scmmodel->get_warehouse_stock($data_export['wh_seq'], 'optioninfo', '', array($optionStr));
								$exitem['whinfo']	= $whinfo[$optionStr];
							}
							$exitem['whinfo']['wh_seq']	= $data_export['wh_seq'];
						}else{
							unset($sc);
							if	($exitem['title1'])		$sc['suboption_title']	= $exitem['title1'];
							if	($exitem['option1'])	$sc['suboption']		= $exitem['option1'];
							$optionData			= $this->goodsmodel->get_goods_suboption($exitem['goods_seq'], $sc);
							if	($optionData[0][0]['suboption_seq'] > 0){
								$optionStr		= $exitem['goods_seq'] . 'suboption' . $optionData[0][0]['suboption_seq'];
								$whinfo			= $this->scmmodel->get_warehouse_stock($data_export['wh_seq'], 'optioninfo', '', array($optionStr));
								$exitem['whinfo']	= $whinfo[$optionStr];
							}
							$exitem['whinfo']['wh_seq']	= $data_export['wh_seq'];
	...
	public function download_write(){
		...
		$data = get_data("fm_exceldownload",array("gb"=>'EXPORT',"provider_seq"=>'1'));
	...
	public function invoice_prints(){
		...
		foreach($arr_export_code as $export_code){
			...
			$export_shipping[0]['recipient_address']		= ($orders['recipient_address_type'] == 'street')? $orders['recipient_address_street']:$orders['recipient_address'];
...
*/
	...
	public function gf_export_call(){
		...
		$this->load->model('ssl');
		// https 보안서버 사용하면 무조건 https 통신하도록 개선 2019-06-03 by hyem
		if( $this->ssl_use && $this->ssl_pay ) {
			$domain = 'https://'.$this->config_system['ssl_domain'];
		} else {
			$domain = 'http://'.$_SERVER['HTTP_HOST'];
		}

		$this->template->assign(array('domain'=>$domain));
		$file_path = $this->template_path();
		$this->template->define(array('tpl'=>$file_path));
		$this->template->print_("tpl");
	}
