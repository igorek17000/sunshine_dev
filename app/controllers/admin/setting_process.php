...
class setting_process extends admin_base {
	...
	public function snsconf()
	{
		...
		*/
		...
		/**
		* @2015-04-28 facebook version 2.* 활동공유제거
	...
	public function iconUpload($del_file=null){
		...
		//$directory = ROOTPATH."/data/icon/manager/";
		//if($del_file) @unlink($directory.$del_file);
		...
		if (is_uploaded_file($_FILES['manager_icon']['tmp_name'])){
			$config['upload_path']		= $path = $directory;
	...
	public function multi_basic()
	{
		...
		// $this->validation->set_rules('basic_currency', '기본통화','trim|required|max_length[5]|xss_clean');
	...
	public function insert_alert(){
		...
		$this->validation->set_rules('location', '위치','trim|required|xss_clean');
		...
		$sql		= "select max(code) as code from fm_alert where code like '".$params['location']."%'";
		$result		= $this->db->query($sql);
    ...
    public function setMainCachePrn(){
	    $this->load->library('cachemain');
	    $sPlatform                  = $this->input->post('platform');
	    $sMainCachePrn              = $this->input->post('mainCachePrn');

	    if( $sPlatform == 'responsive'  ) $sPlatform = 'pc';

	    $aConfigTitle = array(
	        'pc'        => 'skinCache',
	        'mobile'    => 'mobileSkinCache',
	        'fammerce'  => 'fammerceSkinCache'
	    );
	    $aConfigSkin = array(
	        'pc'        => $this->config_system['skin'],
	        'mobile'    => $this->config_system['mobileSkin'],
	        'fammerce'  => $this->config_system['fammerceSkin']
	    );
	    $result['code']             = '10';
	    $result['sMainCachePrn']    = $sMainCachePrn;
	    if( $aConfigTitle[$sPlatform] && $sMainCachePrn){
	        $setSystemConfig[$aConfigTitle[$sPlatform]] = $sMainCachePrn;
	        config_save('system', $setSystemConfig);
	        $result['code'] = '20';
	        if( $sMainCachePrn != 'y' ){
	            $this->cachemain->cache_file_path  = APPPATH."../data/skin/".$aConfigSkin[$sPlatform]."/";
	            $this->cachemain->set_cache_file($this->cachemain->sCachedPath);
    ...
    public function createMainCache(){
        $this->load->helper('readurl');
	    ...
	    $sRequestUrl   = '/main/index?setDesignMode=off&setMode={setMode}&createCached=1';
	    $aConfigSkinTilte = array(
	        'pc'        =>'skin',
	        'mobile'    =>'mobileSkin',
	        'fammerce'  =>'fammerceSkin'
	    );
	    if( $this->config_system[$aConfigSkinTilte[$sPlatform]] ){
	        $result = substr(readurl(get_connet_protocol().$_SERVER['HTTP_HOST'].str_replace('{setMode}', $sPlatform, $sRequestUrl)), 0, 20);
	    }else{
	        $result = "9999";
	    }

	    if($result == '1010' ) $sResultMsg   = '캐시파일 생성이 정상적으로 완료되었습니다.';
	    else $sResultMsg   = '캐시파일 생성 중 오류가 발생하였습니다.';

	    echo json_encode(array('msg' => $sResultMsg, 'code' => $result));
	}

    public function allGoodsDisplayCache(){
	    $result = "99";
	    ...
	    if( $keyword )          $aSearchParams['dd.admin_comment']  = $keyword;
	    $aParams['ddt.auto_use']     = 'n';
	    $sQuery    = $this->goodsdisplay->get_display_tab_list_sql($aParams, $aSearchParams);
	    $res            = mysqli_query($this->db->conn_id, $sQuery);
	    while ($aData = mysqli_fetch_array($res)){
	        ...
	        $sFileName   = $this->goodsdisplay->createDesignDisplayCach($aData['display_seq'], $aData['display_tab_index'], $sPerPage, $aData['kind']);
	        $iReq++;
	        if( !$sFileName ){
	            $iErr++;
	        }else{
	    ...
	    echo json_encode(array('iReq' => $iReq, 'iErr' => $iErr, 'iSuc' => $iSuc));
	}

    public function setAutoGeneration(){
	    $this->load->model('goodsdisplay');
	    $aPostParam = $this->input->post();
	    $cnt        = 0;
	    $result     = 100;
	    $aSetParams = array('auto_generation'   => 'n');
	    if($aPostParam['auto_generation']!='y'){
	        $aSetParams['auto_generation'] = 'y';
	    }
	    $aWhereParams = array('display_seq'=>$aPostParam['display_seq'], 'display_tab_index'=>$aPostParam['display_tab_index']);
	    if($aSetParams['auto_generation']=='y'){
	        $cnt = $this->goodsdisplay->get_display_tab_count(array('auto_generation'=>'y'), 50);
	    }

	    if($cnt < 50){
	        $this->goodsdisplay->update_display_tab($aSetParams, $aWhereParams);
	        $result = 200;
	    }else{
	        $result = 300;
	    }
    ...
    public function setFavorite(){
	    ...
	    $cnt        = 0;
	    $result     = 100;
	    $aSetParams = array('favorite'   => 'n');
	    if($aPostParam['favorite']!='y'){
	        $aSetParams['favorite'] = 'y';
	    }
	    $aWhereParams = array('display_seq'=>$aPostParam['display_seq'], 'display_tab_index'=>$aPostParam['display_tab_index']);
	    $this->goodsdisplay->update_display_tab($aSetParams, $aWhereParams);
	    echo json_encode(array('code' => $result));
    ...
    public function setAllFavorite(){
	    $this->load->model('goodsdisplay');
	    $aPostParam = $this->input->post();
	    $aTmp   = explode(',', $aPostParam['seqs']);
	    foreach($aTmp as $sTmp){
	        if($sTmp){
	            $aTmp2   = explode('_', $sTmp);
	            $aTab['display_seq'][]        = $aTmp2[0];
	            $aTab['display_tab_index'][]  = $aTmp2[1];
	        }
	    }
	    $bind[] = $aPostParam['favorite'];
	    foreach($aTab['display_seq'] as $key => $sdisplaySeq){
	        $aWhere[]   = "(display_seq = ? and display_tab_index = ?)";
	        $bind[]     = $sdisplaySeq;
	        $bind[]     = $aTab['display_tab_index'][$key];
	    }
	    if( $aWhere ){
	        $this->goodsdisplay->update_display_tab_faverites($aWhere,$bind);
	    }
	}

    public function createGoodsDisplayCache(){
	    $result = "99";
    ...
    public function setGoodsDisplayCache(){
	    /*
	     alter table fm_design_display_tab add cache_use enum('y', 'n') null comment '파일 캐시 사용 여부';
	     alter table fm_design_display_tab add favorite enum('y') null comment '중요표시';
	     alter table fm_design_display_tab add auto_generation enum('y', 'n') null comment '캐시파일 자동생성 여부';
	     */
	    $aPostParam = $this->input->post();
	    $this->load->model('goodsdisplay');
	    $aSetParams['cache_use']            = $aPostParam['status'];
	    $aWhereParams['display_seq']        = $aPostParam['display_seq'];
	    $aWhereParams['display_tab_index']  = $aPostParam['display_tab_index'];
	    if( $aPostParam['status'] && $aPostParam['display_seq'] && $aPostParam['display_tab_index']!='' ){
	        if( $aPostParam['status'] == 'n' ){
	            $aSetParams['auto_generation'] = 'n';
	            $this->goodsdisplay->rmDesignDisplayCach($aPostParam['display_seq'], $aPostParam['display_tab_index'], $aPostParam['perpage'], $aPostParam['kind']);
	        }
	        $this->goodsdisplay->update_display_tab($aSetParams, $aWhereParams);
