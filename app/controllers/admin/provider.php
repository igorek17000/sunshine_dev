...
class provider extends admin_base {
	...
	public function catalog(){
		...
		$_default 	= array('orderby'=>'A.regdate','sort'=>'DESC','page'=>0,'perpage'=>10);
		...
		if(!isset($sc['calcu_count'])) $sc['calcu_count'][] = 'all';
		if(!isset($sc['info_type'])) $sc['info_type'][] = 'all';
		if(!isset($sc['provider_status'])) $sc['provider_status'][] = 'all';
		...
		foreach($result as $id=>$val1){
			...
			if(in_array($id,array("chg_text","chk_text","keep_text"))){
				...
				foreach($val1 as $k=>$cont){
					if($k%2==1){ $sty = " style='background-color:#FFF;'"; }else{ $sty = ""; }
	...
	public function gl_select_provider(){
		...
		$this->template->assign(array('shippingtype'=>$shippingtype));
		$this->template->assign(array('provider_gb'=>$provider_gb));
		$this->template->assign(array('calltype'=>$calltype,'sc'=>$sc));
