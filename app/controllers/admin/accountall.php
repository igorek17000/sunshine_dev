...
class accountall extends admin_base {
	...
	{
		redirect("/admin/accountall/accountallviewerall".($_SERVER['QUERY_STRING']?"?".$_SERVER['QUERY_STRING']:''));
	}
	...
	public function accountAllExcelHeader(){
		...
		$order_referer 	= $this->input->get('order_referer');
		$s_year 		= $this->input->get('s_year');
		$s_month 		= $this->input->get('s_month');
		$filename 		= (in_array('all', $order_referer))? 'all':implode($order_referer,'_');
		$filename 		.= "_".$s_year.$s_month;
	...
	public function accountallviewerall() {
		// 정산툴 활성화 여부
		$checker_tool = false;
		if($this->input->get('checker_tool')){
			$checker_tool = true;
		}
		$this->template->assign(array('checker_tool'=>$checker_tool));
		
		$_PARAM = $this->input->get();
		...
		if( $_PARAM['accountall_excel'] ) {
			...
			if( $_PARAM['provider_seq_selector'] == 'all' ) unset($_PARAM['provider_seq_selector']);
			$providerar			= $this->providermodel->provider_goods_list_sort();
			$provideras			= array();
			foreach($providerar as $provider){
				//value="{.provider_seq}" pay_period="{.calcu_count}">{.provider_name}({.provider_id})
				$providernew['provider_seq'] = $provider['provider_seq'];
				$providernew['calcu_count'] = $provider['calcu_count'];
				$providernew['provider_name'] = $provider['provider_name'];
				$providernew['provider_id'] = $provider['provider_id'];
				
				//입점사 정산주기
				$nowPeriodArr	= $this->accountallmodel->get_account_provider_period('pre',$provider['provider_seq']);
				...
				$provideras[$provider['provider_seq']] = $providernew;
				if($provider['provider_seq'] == $_PARAM['provider_seq'] || $provider['provider_seq'] == $_PARAM['provider_seq_selector'] ){
					$provider_viewer['provider_seq']	= $provider['provider_seq'];
					$provider_viewer['viewer']			= $provider['provider_seq']."(".$provider['provider_id'].")";
					$provider_viewer['provider_id'] 	= $provider['provider_id'];
					$provider_viewer['provider_name']	= $provider['provider_name'];
				}
			}
			$this->data_provider = $provideras;
			$this->template->assign(array('provider'=>$provideras,'provider_viewer'=>$provider_viewer));
		}
		$order_referer		= ($_PARAM['order_referer'])?trim($_PARAM['order_referer']):'all';
		$_PARAM['date_field'] = ($_PARAM['date_field'])?trim($_PARAM['date_field']):'deposit_date';
		$order_referer_pg	= account_order_referer_title($order_referer);
		$search_seq			= trim($_PARAM['search_seq']);
		
		if(!$_PARAM['s_year']) $_PARAM['s_year']		= date("Y");
		if(!$_PARAM['s_month']) $_PARAM['s_month']		= date("m");
		...
		$_PARAM['acc_table'] = trim($_PARAM['s_year'].$_PARAM['s_month']);
		/**
		* 년/월 추출 끝
		**/
		...
		* 마이그레이션 날짜 가져오기 시작
		*/
		$checkDate 					= $_PARAM['s_year']."-".$_PARAM['s_month'];
		...
		*/
		...
		$accountConfirm = $this->accountallmodel->get_account_confirm(trim($checkDate));
		...
		//정산 마감전
		if(!$accountConfirm){
			$settingArr['year']				= ($sc['s_year'])?$sc['s_year']:$_PARAM['s_year'];
			$settingArr['month']			= ($sc['s_month'])?$sc['s_month']:$_PARAM['s_month'];
		...
		$_PARAM['listview'] = $listview;
		$loop = $tot = $total3 = $carryoverloop = $carryovertot = $overdrawloop = $overdrawtot = $accountAllCount =array();

		if( ($accountAllMigrationDate == "0000-00-00" || ( $checkDate >= $migrationCheckDate && $_PARAM['s_year'] >= $migrationYear) ) && !$_PARAM['old'] ){
		...
		foreach($query_result as $k => $query_list){
		...
		if($alltot['ac_out_profit_price'] && $alltot['ac_out_pg_default_price']){
			...
			$alltot['ac_out_profit_rate']			= sprintf("%.1f",$alltot['ac_out_profit_rate']);
		}
		...
		//$overdrawloopview = ( $_PARAM['acc_table'] < date('Ym') )?true:false;//차월영역은 현재달이 아닐때만 노출되도록
		...
		//타이틀 기본/매출/정산 숨김
		$totalcolspan 	= 30;
		if($_PARAM['account_hidden_name'])		$totalcolspan -= 3;
		if($_PARAM['account_hidden_sales'])		$totalcolspan -= 8;
		if($_PARAM['account_hidden_cal'])		$totalcolspan -= 11;
		...
		$file_pat_change = ( $_PARAM['accountall_excel'] )?"accountall_excel.html":"accountallviewer.html";
	...
	**/
	public function accountgroup(){
		...
		$file_path			= $this->template_path();
		$provider			= $this->accountallmodel->get_provider_calcu_list();
		...
		$this->template->assign(array('year'=>$year,'month'=>$month));
		$sc['acc_table'] = trim($sc['s_year'].$sc['s_month']);

		if(!$sc['pay_period']) $sc['pay_period'] = "all";
		$_PARAM						= $sc;
		...
		* 마이그레이션 날짜 가져오기 시작
		*/
		$checkDate 					= trim($sc['s_year']."-".$sc['s_month']);
		$_accountSettings			= getAccountSetting();
		$accountallPatchV1			= $_accountSettings['accountall_patch_v1'];		// account patch v1 : 신정산 수정 v1 패치일자 20190430 개선 pjm
		$accountAllViewV1			= $_accountSettings['accountall_view_v1'];		// account view v1 : 신정사 수정 v1 view 기준일자 20190430 개선 pjm
		$migrationYear				= $_accountSettings['migrationYear'];
		...
		* 정산 실행 내역 시작
		*/
		$accountConfirm = array();
		$accountallConfirmSetting = array();
		$accountConfirm = $this->accountallmodel->get_account_confirm($checkDate);
		...
		//정산 마감전
		if(!$accountConfirm){
			$settingArr['year'] 		= $sc['s_year'];
			$settingArr['month'] 		= $sc['s_month'];
			$accountallConfirmSetting		= $this->accountallmodel->get_account_setting("month",$settingArr);
			$yearmonth						= $settingArr['year']."-".$settingArr['month'];
			$accountallConfirmSetting['confirm_date']	= date("Y-m", strtotime($yearmonth."+1 month"))."-".sprintf("%02d",$accountallConfirmSetting['accountall_confirm']);
			$accountallConfirmSetting['confirm_name']	= getConfirmDay($accountallConfirmSetting['accountall_confirm']);
			$confirm_date				= $accountallConfirmSetting['confirm_date'];
			if($accountallPatchV1 && $accountAllViewV1){
				if(date("Ymd",$accountallPatchV1) <= date("Ymd") && date("Y-m",$accountAllViewV1) <= $checkDate){
					$listview = "list2";
				}
		...
		//정산 마감후
			...
			$accountConfirm['confirm_name'] = getConfirmDay($accountConfirm['confirm_day']);
			$confirm_date						= $accountConfirm['confirm_end_date'];
			if($accountallPatchV1 && $accountAllViewV1){
				if(date("Ymd",$accountallPatchV1) <= date("Ymd") && date("Y-m",$accountAllViewV1) <= $checkDate && date("Y-m-d",$accountAllViewV1) < substr($accountConfirm['confirm_end_date'],0,10)){
					$listview = "list2";
				}
		...
		$_PARAM['listview'] = $listview;
		$loop = $tot = $total3 = $carryoverloop = $carryovertot = $overdrawloop = $overdrawtot = $accountAllCount =array();
		if( ($accountAllMigrationDate == "0000-00-00" || ( $checkDate >= $migrationCheckDate && $_PARAM['s_year'] >= $migrationYear) ) && !$sc['old'] ){
			$query_result	= $this->accountallmodel->get_account_all_catalog_query($_PARAM);
		}
		...
		foreach($query_result as $k => $query_list){
			while ($acinsdata = mysqli_fetch_array($query_list)){
				...
				accountalllist($listview,$acinsdata, $loop, $tot, $carryoverloop, $carryovertot, $overdrawloop, $overdrawtot, $extend);
				if( $sc['account_hidden_sales'] ) unset($overdrawloop,$overdrawtot);
				if( $sc['account_hidden_cal'] ) unset($carryoverloop,$carryovertot);
			}
		}
		/*
		* 정산 실행 내역 끝
		*/
		foreach($provider as $tmp_provider){
			// 정산주기 조건절
			if($_PARAM['pay_period'] && $_PARAM['pay_period']!='all'){
				if($_PARAM['pay_period'] != $tmp_provider['calcu_count']){
					continue;
				}
			}
			$accountAllCount = $extend['accountAllCount'][$tmp_provider['provider_seq']];
			
			if($accountAllCount){
				// 입점사별 데이터 합산
				$row_accountgroup = array();
				$row_accountgroup['period_type']				= $tmp_provider['calcu_count'];
				$row_accountgroup['loop_index']					= $loop_index++;
				$row_accountgroup['provider_seq']				= $tmp_provider['provider_seq'];
				$row_accountgroup['provider_name']				= $tmp_provider['provider_name'];
				$row_accountgroup['provider_id']				= $tmp_provider['provider_id'];
				
				if($accountAllCount['account1']){
					foreach($accountAllCount['account1'] as $k=>$tmp_account){
						$row_accountgroup['period_count']				= $k;
						$row_accountgroup['sum_price']					= $accountAllCount['account1_sum_total_ac_price'][$k];
						$row_accountgroup['sum_feeprice']				= $accountAllCount['account1_sum_feeprice'][$k];
						$row_accountgroup['sum_commission_price']		= $tmp_account;
						$loop_accountgroup[$row_accountgroup['provider_seq']][$k] = $row_accountgroup;
					}
				}
				if($accountAllCount['account2']){
					if(count($accountAllCount['account2']) < $row_accountgroup['period_type']){
						// 더미 데이터 초기화
						for($dumy_i=0;$dumy_i<$row_accountgroup['period_type'];$dumy_i++){
							$row_accountgroup['period_count']				= $dumy_i;
							$row_accountgroup['sum_price']					= 0;
							$row_accountgroup['sum_feeprice']				= 0;
							$row_accountgroup['sum_commission_price']		= 0;
							$loop_accountgroup[$row_accountgroup['provider_seq']][$dumy_i] = $row_accountgroup;
						}
					}
					foreach($accountAllCount['account2'] as $k=>$tmp_account){
						$row_accountgroup['period_count']				= $k;
						$row_accountgroup['sum_price']					= $accountAllCount['account2_sum_total_ac_price'][$k];
						$row_accountgroup['sum_feeprice']				= $accountAllCount['account2_sum_feeprice'][$k];
						$row_accountgroup['sum_commission_price']		= $tmp_account;
						$loop_accountgroup[$row_accountgroup['provider_seq']][$k] = $row_accountgroup;
					}
				}
				if($accountAllCount['account4']){
					if(count($accountAllCount['account4']) < $row_accountgroup['period_type']){
						// 더미 데이터 초기화
						for($dumy_i=0;$dumy_i<$row_accountgroup['period_type'];$dumy_i++){
							$row_accountgroup['period_count']				= $dumy_i;
							$row_accountgroup['sum_price']					= 0;
					...
					foreach($accountAllCount['account4'] as $k=>$tmp_account){
						$row_accountgroup['period_count']				= $k;
						$row_accountgroup['sum_price']					= $accountAllCount['account4_sum_total_ac_price'][$k];
						$row_accountgroup['sum_feeprice']				= $accountAllCount['account4_sum_feeprice'][$k];
						$row_accountgroup['sum_commission_price']		= $tmp_account;
						$loop_accountgroup[$row_accountgroup['provider_seq']][$k] = $row_accountgroup;
					}
				}
		...
		// 각 입점사별로 저장되어있는 배열을 재구성
		$tmp_loop_accountgroup = $loop_accountgroup;
		$loop_accountgroup = array();
		foreach($tmp_loop_accountgroup as $tmp_accountgroup){
			foreach($tmp_accountgroup as $accountgroup){
				$loop_accountgroup[] = $accountgroup;
			}
		}

		// 총합 계산
		foreach($loop_accountgroup as $accountgroup){
			$tot['sum_price']					+= $accountgroup['sum_price'];
			$tot['sum_feeprice']				+= $accountgroup['sum_feeprice'];
			$tot['sum_commission_price']		+= $accountgroup['sum_commission_price'];
		}

		$this->template->assign(array(
			'loop'						=> $loop_accountgroup,
			'loopcnt'					=> count($loop_accountgroup),
			'tot'						=> $tot,						
			'sc'						=> $sc,			
		...
		switch($acinsdata['status']){//전월/당월/차월 구분
			...
			case "not-carryover":
				...
				if( $acinsdata['account_type'] == "refund" || $acinsdata['account_type'] == "rollback" ) {
					...
					if($acinsdata['ac_type'] == "cal_sales" && (!$acinsdata['out_confirm_date'] || $acinsdata['ac_ea'] != '0')){
	...
	**/
	public function accountall_setting_catalog() {
		...
		$sc = $this->input->get();
		$sc['orderby']			= (isset($sc['orderby'])) ?	$sc['orderby']:'seq';
	...
	**/
	...
	public function get_provider_for_viewer()
	{
		$result = $this->providermodel->provider_goods_list_sort();
		echo("<option value='all'>전체</option>");
		echo("<option value='1'>본사</option>");
		foreach($result as $data){
			if($data['provider_name']) echo("<option value='".$data['provider_seq']."' pay_period='".$data['calcu_count']."'>".$data['provider_name']."(".$data['provider_id'].")</option>");
		}
	}
	...
	public function get_provider_for_period()
	{
		...
		if($mode == "json"){
			$loop 		= array();
			$return_arr = array('provider_seq','provider_id','provider_name','provider_status','pgroup_name','pgroup_seq','calcu_count','mgb');
			foreach($result as $_data){
				$return = array();
				foreach($_data as $_key => $_val) if(in_array($_key,$return_arr)) $return[$_key] = $_val;
				$loop[] = $return;
			}
	...
	public function get_accountall_setting_provider()
	{
		$this->load->library('validation');

		$sc 				= array();
		$provider_period 	= $this->input->get('provider_period');
		$provider_name 		= $this->input->get('provider_name');
		$provider_search_type 	= $this->input->get('provider_search_type');
		if($provider_period){
			$sc['provider_period'] = $provider_period;
		}
		if($provider_name){
			$_POST['provider_name'] = $provider_name;
			$sc['provider_name'] 	= $provider_name;
			$this->validation->set_rules('provider_name', '입점사명','trim|required|xss_clean');

			if($this->validation->exec()===false){
				echo json_encode("provider_name_error");
				exit;
			}
		}
		if($provider_search_type){
			$type = $provider_search_type;
		}
		$resultTmp = $this->accountallmodel->get_provider_search($type,$sc);
		if($type == 'pre'){
			$result = array();
			unset($sc['provider_period']);
			$resultLast = $this->accountallmodel->get_provider_search('last',$sc);
			foreach($resultTmp as $preTmp){
				foreach($resultLast as $lastTmp){
					if($lastTmp['provider_seq'] != $preTmp['provider_seq']){
						continue;
					}else{
						$preTmp['next_calcu_count'] = $lastTmp['calcu_count'];
					}
				}
				$result[] = $preTmp;
			}
	 ...
	 * 정산데이터 정산확정 처리
	 */
	function accountall_confirm(){
		$order_seq			= $this->input->post('order_seq');
		
		try {
			$this->accountallmodel->check_confirm_account_origin_order($order_seq, 'order_export_code_list');
		} catch (Exception $exc) {
			debug('accountmodel->check_confirm_account_origin_order'.chr(10).$exc->getTraceAsString());
		}
		
		echo json_encode($order_seq);
	}
	
	/**
	 * 정산데이터 수정 SQL 처리
	 */
	function accountall_modify(){
		$mode			= $this->input->post('mode');
		$base_info		= $this->input->post('base_info');
		$modify_column	= $this->input->post('modify_column');
		$view_Ym		= $this->input->post('view_Ym');
		
		$sql = $this->accountallmodel->proc_modify_account($mode, $base_info, $modify_column, $view_Ym);
		
		$result['sql'] = $sql;
		echo json_encode($result);
	}
	
	function accountallviewerall_checker_tool_ajax(){
		$this->load->helper('accountall');
		$accountData				= $this->input->post('accountData');
		$carryover					= $this->input->post('carryover');
		$checker_tool_view_succ		= $this->input->post('checker_tool_view_succ');
		
		$accountData = unserialize(base64_decode($accountData));
	...
	public function accountallviewerall_checker_tool_download(){

		ini_set("memory_limit",-1);
		$s_year				= $this->input->post('s_year');
		$s_month			= $this->input->post('s_month');
		$arr_order_seq 		= $this->input->post('arr_order_seq');
		$filename 		= "account_error_result_".$s_year.$s_month;
		...
		echo '
				<tr>
					<td style=\' mso-number-format:"@"\'>'.$order_seq.'</td>
				</tr>
		';
		}
