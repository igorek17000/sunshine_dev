...
class popup extends admin_base {
	...
	public function zipcode_zone(){
		...
		$this->template->assign("idx",$_GET['idx']);
		$this->template->assign("shipping_cost_seq", $_GET['shipping_cost_seq']);
		$this->template->assign("shipping_group_seq", $_GET['shipping_group_seq']);
		$this->template->assign("shipping_set_seq", $_GET['shipping_set_seq']);
	...
	public function shipping_address_list(){
		...
		}elseif($_GET['tabType'] == 'scm'){ // 재고 창고
			if	(!$this->scm_cfg)	$this->scm_cfg	= config_load('scm');
			if	($this->scm_cfg['use'] == 'Y'){
				$wh_list	= $this->shippingmodel->shipping_warehouse_list($sc);
				$use_wh_seqs = array_keys($this->scm_cfg['use_warehouse']);
				if($wh_list['record']){
					foreach($wh_list['record'] as $key => $info){
						// 미사용 창고 추출
						$info['wh_use']		= 'Y';
						if(array_search($info['shipping_address_seq'], $use_wh_seqs) === false){
							$info['wh_use']	= 'N';
						}
						$list['record'][] = $info;
					}
					$list['page'] = $wh_list['page'];
				}
			}else{
				$msg	= 'All-in-One(쇼핑몰+재고관리) 버전에서만 지원되는 기능입니다.';
		...
		$this->o2oinitlibrary->init_admin_shipping_address_list($list, $sc, $_GET);
	...
	public function groupsale_choice(){
		...
		$group_sale_lists = $this->membergroupmodel->get_group_sale('all', $this->input->get('goods_seq'), $this->input->get('category_code'));

		$file_path	= $this->template_path();
		$this->template->define(array('tpl'=>$file_path));
		$this->template->assign(array('group_sale_lists'=>$group_sale_lists, 'sale_seq'=>$this->input->get("sale_seq")));
		$this->template->print_("tpl");
	}
