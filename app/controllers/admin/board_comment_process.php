...
class Board_comment_process extends admin_base {
		...
		**/
		...
		elseif($mode == 'board_comment_modify') {
			...
			if($result) {
				...
				$callback = (!empty($_POST['returnurl']))?"<script type='text/javascript'>parent.boardaddFormDialog('".$_POST['returnurl']."#cwriteform', 1200, 700, parent.getCurrentBoardName() + ' 게시글 보기','false');alert('댓글을 수정하였습니다.');</script>":"<script type='text/javascript'>parent.boardaddFormDialog('".$this->Boardmanager->realboardviewurl.BOARDID."&seq=".$boarddata['seq']."#cwriteform', 1200, 700, parent.getCurrentBoardName() + ' 게시글 보기','false');alert('댓글을 수정하였습니다.');</script>";
				echo $callback;
			}else{
		...
		elseif($mode == 'board_comment_seldelete') {
			...
			$return = array('result'=>true, 'msg'=>$num.'건의 댓글을 삭제하였습니다.');
			echo json_encode($return);
			exit;
		}
